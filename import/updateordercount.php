...
$SQL	="SELECT DISTINCT `ID_ORDER` , qty FROM `VQTY`";
...
$Base_AmountBeforeTax			=$roomrate;
		...
		$total_price		=	($roomrate*no_of_days);
 		$tarrif_price		=	$roomrate;
			 ...
			 echo "<br>".$query3 		= "SELECT * FROM fs_rate_assign_details WHERE hotel_id ='".$id_hotels."' and rate_id='".$id_rate."' " ;	
			...
			if($id_order!='' && $reference!='' && $id_shop!='' && $id_hotels!='' &&  $room_id!='' ){
				...
				//echo "id_hotels==>".$id_hotels."room_id=>".$room_id;
		...
		echo  "<br> $count Not Iserted ID= ".$in_last_id."reference=>".$reference."id_order=>".$id_order." Rate id=>".$id_rate."rate_plan_id==>".$rate_plan_id."fs_rate_assign_id==>".$fs_rate_assign_id."room_id==>".$room_id.'room_quantity'.$room_quantity;
		...
		$updateprice = executeSql("UPDATE  fs_orders  SET 
								total_price='".$total_price."',subtotal='".$subtotal."', tarrif_price='".$tarrif_price."' WHERE id_order='".$id_order."' ");
