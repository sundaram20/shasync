...
*/
	...
	if($err == 0){//No error
		...
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add
			...
			$editSql .= "	,`grade` = '".trim(addslashes($_POST['grade']))."'";
...
$addSql .=	$StatusDate;
			...
			if($_POST['status']=='1'){
				$StatusDate	=	", `status_active_date` = '".date('Y-m-d', strtotime($_POST['status_active_date']))."'";
				$StatusDate	.=	", `status_inactive_date` = '0000-00-00'";
			}else{
				$StatusDate	=	", `status_inactive_date` = '".date('Y-m-d', strtotime($_POST['status_inactive_date']))."'";
				}
			...
			$editSql .= "	,`user_custom_smtp` = '".addslashes($_POST['user_custom_smtp'])."'";
			$editSql .= "	,`smtp_email` = '".addslashes($_POST['smtp_email'])."'";
			$editSql .= "	,`smtp_password` = '".addslashes($_POST['smtp_password'])."'";
			$editSql .=	$StatusDate;
  ...
  <div class="row">
    ...
    <div class="form-group ">
      ...
      <label for="first_name">Designation <font color="#FF0000">*</font></label>
       ...
       <?php $marketDropDown = '<select class="form-control select2 input-sm" name="designation" id="designation" data-parsley-errors-container="#designationError" data-parsley-required   >
    ...
    <?php echo $err_designation;?>
     ...
     <!--grade -->
      ...
      <label for="grade">Grade </label>
       ...
       <?php $marketDropDown2 = '<select class="form-control select2 input-sm" name="grade" id="grade" data-parsley-errors-container="#designationError" data-parsley-required   >

												  <option value="">Select Grade</option>';

												 

												  $resCat = selectSql(TBL_GRADE_MASTER," where status='1' and id_shop='".addslashes($_SESSION['shop'])."'",' ORDER BY `name`');										

												  if($db->num_rows2($resCat)){

													while($resultCat = $db->fetch_object2($resCat)){

													if($_REQUEST['grade'] == $resultCat ->id){

														$selected = 'selected="selected"';

													}elseif($rowUserDetail->grade== $resultCat ->id){
														...
														$marketDropDown2 .= '<option  '.$selected.' value="'.$resultCat->id.'">'.ucfirst($resultCat->name).'</option>';

													}

												  }

													echo $marketDropDown2 .= '</select>';
    ...
    <?php echo $err_grade;?>
     ...
     <!--grade end-->
      ...
      <div class="form-group">
        ...
        <label for="company">Company<font color="#FF0000">*</font></label>
      ...
      <div class="form-group">
        ...
        <label for="address">Address 1<font color="#FF0000">*</font></label>
...
<!--Smtp Detaila--->
<div class="form-group">
                  <label for="company_credibility">User Custom SMTP <?php echo $row->user_custom_smtp; ?></label>
                  <select class="form-control select2" onChange="opencustomsmtp(this.value);" name="user_custom_smtp" id="user_custom_smtp" data-parsley-errors-container="#user_custom_smtpError" data-parsley-required>
                    <option value="1" <?php if($rowUserDetail->user_custom_smtp=='1'){echo 'selected="selected"';} ?>>Yes</option>
                    <option value="2"  <?php if($rowUserDetail->user_custom_smtp=='2'){echo 'selected="selected"';}?>>No</option>
                  </select>
                  <span id="user_custom_smtpError"><?php echo $err_user_custom_smtp;?></span> </div>
                  
    
           
      <?php 
              	if($rowUserDetail->user_custom_smtp=='2'){
              		
              		$dispalyBox ='style="display:none"';
              	}
              	?>            
                  
           <div  <?php echo $dispalyBox;?>  id="usercustomsmtp">       
                  
              <div class="form-group col-sm-6">

                  <label for="display_order">SMTP Email</label>

                  <input type="text" class="form-control" placeholder="Enter Smtp Email" id="smtp_email" name="smtp_email" value="<?php if($_POST) echo $_POST['smtp_email'];else echo stripslashes($rowUserDetail->smtp_email);?>">
                ...
                <div class="form-group col-sm-6">
                  ...
                  <label for="display_order">SMTP Password</label>

                  <input type="text" class="form-control" placeholder="Enter Smtp Password" id="smtp_password" name="smtp_password" value="<?php if($_POST) echo $_POST['smtp_password'];else echo stripslashes($rowUserDetail->smtp_password);?>">
