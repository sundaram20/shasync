...
if($_REQUEST['Download'] == 'Download'){
		...
		->setCellValue('A6', 'Executive Portfoilo - RSO Report Monthly For Year :'.$period);
		...
		$objPHPExcel->setActiveSheetIndex(0)
			...
			while($row=mysqli_fetch_object($res)){
				...
				$sqlBud = "SELECT room_nights AS budgetRN FROM ".TBL_AGENT_BUDGET." WHERE id_user='".$row->id_exe."' AND id_company='".$row->id_company."' AND `from` = '".date('Y-m-d',strtotime($date))."' AND `to` ='".date('Y-m-d',strtotime($end))."' ";

				$sqlAch =  "SELECT SUM(qty) AS achRN FROM ".TBL_AGENT_ACHIEVED." WHERE id_user='".$row->id_exe."' AND id_company='".$row->id_company."' AND month BETWEEN '".date('Y-m-d',strtotime($date))."' AND '".date('Y-m-d',strtotime($end))."' 
...
<option value="">Select Sales Executive</option>	';
				  $resUserLevel = selectSql(TBL_USERS," WHERE `status` = '1' AND `sales_status_active` = '1' ".$teamMembers." ".$UserRestriction." AND user_type!=2 AND `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
