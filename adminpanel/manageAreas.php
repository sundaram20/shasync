...
if($_REQUEST['action'] == 'change'){
	if($_REQUEST['activeId'] != ''){
		checkUserLevelPermission($_SESSION['userLevel'],TBL_AREAS,'activate');
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_AREAS,'deactivate');
	...
	if(executeSql($statusSql)){
		$err = 0;
		$_SESSION['successMsg'] = 'Areas '.selectColumn(TBL_AREAS,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';
	}else{
		$err = 1;
		$_SESSION['errorMsg'] = 'Areas '.selectColumn(TBL_AREAS,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';
	}
}else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
	checkUserLevelPermission($_SESSION['userLevel'],TBL_AREAS,'delete');
	 $delSql = "DELETE FROM `".TBL_AREAS."` WHERE `id` = '".$_REQUEST['delId']."'";
	
	$sqlDelUserLevel = selectRow(TBL_AREAS," WHERE `id` = '".$_REQUEST['delId']."'");
	if(executeSql($delSql)){		
		$err = 0;
		$_SESSION['successMsg'] = 'One Areas '.$sqlDelUserLevel["name"].' has been deleted sucessfully.';
	}else{
		$err = 1;
		$_SESSION['errorMsg'] = 'Unable to delete Areas '.$sqlDelUserLevel["name"];
	}
}
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_AREAS."` WHERE `id` IN (".addslashes($deleteIds).")";
	if(executeSql($delSql)){		
		$err = 0;
...
if($_REQUEST['secondary_user_id'] != ''){
	$sql.=  " AND FIND_IN_SET('".$_REQUEST['secondary_user_id']."',`ids_unit_user`)";//" AND `ids_unit_user` REGEXP CONCAT('(^|,)(', REPLACE('".$_REQUEST['secondary_user_id']."', ',', '|'), ')(,|$)')";
	//$sql .= " AND `ids_unit_user` = '".addslashes($_REQUEST['secondary_user_id'])."%'";
}
if($_REQUEST['order'] != ''){
	...
	<div class="box box-default">
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-body">
          ...
          <?php
		  $userSecondSql = selectSql(TBL_AREAS," where status='1' and id_shop='".addslashes($_SESSION['shop'])."'  ",' ORDER BY `id`');
											  if($db->num_rows2($userSecondSql)){
												  $ArrayOfUnitUser=array();
											  	while($resultUserCat = $db->fetch_object2($userSecondSql)){
													array_push($ArrayOfUnitUser,$resultUserCat->ids_unit_user);
													
												}
											  }
											  $ArrayOfUnitUser=implode(',',$ArrayOfUnitUser);
											  $allUnitUser= " AND FIND_IN_SET(`id`,'".$ArrayOfUnitUser."')";
											  
											 
          ?>
          <div class="col-md-6">
              <div class="form-group">
                <label>Secondary/Unit User</label>				
				 <?php $categoryDropDown = '<select class="form-control select2" name="secondary_user_id">
											    <option value="">Select Secondary/Unit User</option>';
											  $resCat = selectSql(TBL_USERS," where status='1' and id_shop='".addslashes($_SESSION['shop'])."' ".$allUnitUser." ",' ORDER BY `id`');
											  if($db->num_rows2($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													if($_REQUEST['secondary_user_id'] == $resultCat->id){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th>Area </th>
				  <th>Primary/RSO user </th>
				  <th>Secondary/Unit User </th>
                  <th>Status</th>
				  <th>Action</th>
                </tr>
                </thead>
                ...
                <tr>
                  ...
                  <td><?=$row->name;?></td>
				  <td><?php echo selectColumn(TBL_USERS,'username'," WHERE `id` = '".$row->user_id."'");   ?></td>
					<td><ol>
					<?php
						foreach (explode(",",$row->ids_unit_user) as $index => $id) {
							if($id!=''){
								echo "<li>".selectColumn(TBL_USERS,'name','WHERE id="'.$id.'" ')."</li>";
							}
						}
					?>
					</ol></td>
  		    ...
  		    if (this.readyState == 4 && this.status == 200) {
  		      ...
  		      if(this.responseText == 1){
  		      	...
  		      	if(confirm('Are you sure that you want to delete this record '+name+'?')){
  		      		window.location.href='manageAreas.php?delId='+id+'&action=delete&page=<?=$_REQUEST['page']?>';
  		      	}
  		      }
  		    }
  		  };
  		  xhttp.open("GET", "ajax/ajaxCheckCompanyDomain.php?id_cmp_area="+id, true);
  		  xhttp.send();
  	}
  </script>                                    
