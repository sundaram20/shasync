...
if($_REQUEST['action'] == 'change'){
	...
	$delSql = "DELETE FROM `".TBL_ATTRIBUTES."` WHERE `id` = '".$_REQUEST['delId']."'";
		...
		if(executeSql($delSql)){		
			$err = 0;
			$_SESSION['successMsg'] = 'One Sub Group  has been deleted sucessfully.';
		}else{
			$err = 1;
			$_SESSION['errorMsg'] = 'Unable to delete Sub Group ';
		}
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
		  <div class="btn-group  pull-right">
							  <a type="button" class="btn btn-success" href="editProductSubGroups.php" >Add Sub Group</a>
            ...
            <div class="box-body table-responsive">
		        ...
		        <tbody>  
		        	<?php 		 
					  ...
					  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageProductSubGroups.php?inactiveId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageProductSubGroups.php?activeId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 
				  <td><img src="images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editProductSubGroups.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/delete.gif" style="cursor:pointer;" title="Delete" name="<?php echo $row->name; ?>" id="<?php echo $row->id;?>" onClick="deleteMe(this.id,this.name);"/></td>
                </tr>
               <?php } ?>  
  		    ...
  		    if (this.readyState == 4 && this.status == 200) {
  		      ...
  		      if(this.responseText == 1){
  		      	...
  		      	if(confirm('Are you sure that you want to delete this record '+name+'?')){
  		      		window.location.href='manageProductSubGroups.php?delId='+id+'&action=delete&page=<?=$_REQUEST['page']?>';
  		      	}
  		      }
  		    }
  		  };
