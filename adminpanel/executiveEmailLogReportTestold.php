                       ...
                       <?php  echo $_REQUEST['teamId']; ?>
                        <!--team ends-->
                          ...
                          $resUserLevel = selectSql(TBL_USERS," WHERE `status` = '1'  AND `sales_status_active` = '1'  ".$teamMembers."  $UserRestriction AND `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
                            ...
                            //  $resUserLevel22 = "SELECT * FROM ".TBL_USERS." WHERE `status` = '1'  AND `sales_status_active` = '1'  ".$teamMembers."  $UserRestriction AND `id_shop` = '".addslashes($_SESSION['shop'])."' ORDER BY `name` ";
                               //  echo $resUserLevel22;


                            $categoryDropDown = '<select multiple="multiple" class="form-control select2" name="usernameid[]" id="usernameid[]">
                             <!-- <option value="">All</option>-->';
                          ...
                          $resUserLevel = selectSql(TBL_USERS," WHERE `status` = '1'  AND `sales_status_active` = '1'  ".$teamMembers."  $UserRestriction AND `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `name`');
                                      ...
                                      while($resultUserLevel = $db->fetch_object2($resUserLevel)){
                                      //if($_REQUEST['usernameid'] == $resultUserLevel->id){
                                   $categoryDropDown .= '<option selected="selected" value="'.$resultUserLevel->id.'">'.ucfirst($resultUserLevel->name).'</option>';
