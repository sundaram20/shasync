...
checkUserLevelPermission($_SESSION['userLevel'],'credit_report','view');
...
if($_REQUEST['Download']=='Download'){
    ...
    if($_REQUEST['usernameid'] != ''){
      ...
      if($_SESSION['userLevel'] !=1 && $_SESSION['teamMemberLevel']==''){
       $sql .= " AND B.`id` = '".$_SESSION['userId']."'";
        ...
        $sql .= "AND FIND_IN_SET(B.ids_team,'".$_SESSION['teamId']."') ";
      }
    } 
    
    $sql .= " ORDER BY A.company_credibility,A.name "; 
    //echo $sql;
    //exit;  
  ...
  $objPHPExcel->getActiveSheet()->getPageSetup()
      ...
      header('Content-Disposition: attachment;filename="Credit_Report'.date('d-m-Y H:i:s').'.xls"');
