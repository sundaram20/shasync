...
// Assign for Call handler
if($_REQUEST["act"] == "assign_call" && !empty($_REQUEST['pickup_ids']) && !empty($_REQUEST['assign_user_id'])){
    $assignUserId  = (int)$_REQUEST['assign_user_id'];
    $assignIds     = array_map('intval', $_REQUEST['pickup_ids']);
    $assignIdsStr  = implode(',', $assignIds);

    $assignSql = "UPDATE `daily_pickup`
                  SET `assign_user_id` = '".$assignUserId."'
                  ,`last_modified` = '".currenDateTime()."'
                  ,`id_mst_user_modified_by` = '".$_SESSION['userId']."'
                  WHERE `id` IN (".$assignIdsStr.")";

    if(executeSql($assignSql)){
		foreach($assignIds as $pickupId){

    // Get daily_pickup header details
    $dpRow = selectRow('daily_pickup', " WHERE `id` = '".$pickupId."'");

    // Get all items (serial numbers + products) from daily_pickup_details
    $dpdRes = selectSql('daily_pickup_details', " WHERE `id_daily_pickup` = '".$pickupId."'");

    if($db->num_rows2($dpdRes)){
        while($dpdRow = $db->fetch_object2($dpdRes)){

            $supportInsertSql = "INSERT INTO `support_details` 
                (
                    `id_shop`,
                    `id_daily_pickup`,
                    `id_support`,
                ...
                (
                    ...
                    '".$pickupId."',
                    '0',
                    '".(int)$dpdRow->id_product."',
                    '".(int)$dpRow['id_company']."',
                    '".(int)$dpdRow->serial_number."',
                    '".(int)$dpRow['id_executive']."',
                    '".$assignUserId."',
                    '',
                    'Assigned for Call',
                    '1',
            ...
            executeSql($supportInsertSql);
			
			//calendar
			$resCalCheck = executeSql("SELECT * FROM `".TBL_DAILY_CALENDER."` 
				WHERE `id_shop`  = '".$_SESSION['shop']."' 
				AND `type`       = '8'
				AND `visit_id`   = '".$pickupId."'
				AND `doc_id`     = '".(int)$dpdRow->id."'");
    ...
    $calSql = "UPDATE `".TBL_DAILY_CALENDER."` SET
        ...
        `id_user`         = '".(int)$dpRow['id_executive']."',
        `assign_user_id`  = '".$assignUserId."',
        `doc_id`          = '".(int)$dpdRow->id."',
        `visit_id`        = '".$pickupId."',
        `id_list_name`    = '0',
        `dated`           = '".date('Y-m-d')."',
        `status`          = '1'
        WHERE `visit_id` = '".$pickupId."'
        AND `doc_id`     = '".(int)$dpdRow->id."'
        AND `type`       = '8'";
} else {
    // Insert new
...
mysqli_query($connNew, $calSql);
			
        }
    }
}
		
        echo json_encode(['success' => true, 'message' => 'Records assigned successfully.']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Failed to assign records.']);
    }
    exit;
}
...
//$sql = "SELECT dp.*, 
	...
	//	JOIN 
	//	daily_pickup_details AS dpd
	//	ON
    //	dp.id = dpd.id_daily_pickup
	//	WHERE
    //	dp.id_shop = '".addslashes($_SESSION['shop'])."'";

/*$sql = "SELECT dp.*, 
       dpd.id_product, 
      dpd.serial_number
FROM daily_pickup dp
JOIN (
    SELECT * 
  FROM daily_pickup_details 
    GROUP BY id_daily_pickup
) dpd ON dp.id = dpd.id_daily_pickup
WHERE dp.id_shop = '".addslashes($_SESSION['shop'])."'";*/
...
$sql = "SELECT dp.*, 
        ...
        dpd.rate,
        dpd.sales_revenue,
        dpd.renewal_date as item_renewal_date,
        dpd.renewal_status as item_renewal_status
FROM daily_pickup dp
JOIN daily_pickup_details dpd ON dp.id = dpd.id_daily_pickup
WHERE dp.id_shop = '".addslashes($_SESSION['shop'])."'";

if($_SESSION['userId'] != 245){
    $sql .= " AND (dp.`id_executive` = '".addslashes($_SESSION['userId'])."' 
               OR dp.`assign_user_id` = '".addslashes($_SESSION['userId'])."')";
}


if(!empty($_REQUEST['search_name'])){
    // Assuming 'bill_no' is in the daily_pickup table, hence using 'dp' alias
    $sql .= " AND (dp.`bill_no` LIKE '%".addslashes($_REQUEST['search_name'])."%' )";
}

if(!empty($_REQUEST['search_serial'])){
$sql .= " AND (dpd.`serial_number` LIKE '%".addslashes($_REQUEST['search_serial'])."%' )";
}

if($_REQUEST['companyId'] != ''){
	$sql .= " AND dp.`id_company` = '".addslashes($_REQUEST['companyId'])."'";
}

if($_REQUEST['id_customer'] != ''){
	$sql .= " AND dp.`id_contacts` = '".addslashes($_REQUEST['id_customer'])."'";
}
...
if($_REQUEST['id_executive'] != ''){
	$sql .= " AND dp.`id_executive` = '".addslashes($_REQUEST['id_executive'])."' ";
}
...
if(isset($_REQUEST['check_box']) && $_REQUEST['check_box'] == 1 && !empty($_REQUEST['date_created'])){
    ...
    // Assuming 'doc_date' is in the daily_pickup table
    $sql .= " AND DATE(dp.doc_date) >= '".date('Y-m-d', strtotime($from_book))."' AND DATE(dp.doc_date) <= '".date('Y-m-d', strtotime($to_book))."' ";
}
...
if(isset($_REQUEST['renewal_check']) && $_REQUEST['renewal_check'] == 1 && !empty($_REQUEST['renewal_date'])){
    ...
    $sql .= " AND dp.id_renewal_required = '1' 
              AND DATE(dpd.renewal_date) >= '".date('Y-m-d', strtotime($from_renewal))."'
              AND DATE(dpd.renewal_date) <= '".date('Y-m-d', strtotime($to_renewal))."' ";
}
...
if(@$_REQUEST['searchFormSubmit']!='1'){  // Last 24Hours- Record
	...
	$sql .= " AND dp.`doc_date` >= '".$UniqueDateFor."'";
...
/* Assign bar styling */
#assign_user_select:disabled,
#assignCallBtn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
}
#assignCallBtn {
...
#selectedCountBadge {
  ...
  <section class="content">
    <div class="box box-default">
      ...
      <div class="box-header with-border">
        ...
        <div class="btn-group  pull-right"> <a type="button" class="btn btn-success" href="edit-daily-pickup.php">Add Daily Pickup</a>
  ...
  <label for="renewal_date">
    <input type="checkbox" name="renewal_check" value="1"
      <?php if(@$_REQUEST['renewal_check']==1) echo 'checked'; ?> />
    &nbsp;Renewal Date : From - To
  </label>
  ...
  <div class="input-group">
    ...
    <input type="text"
           ...
           value="<?php if($_REQUEST) echo $_REQUEST['renewal_date']; ?>">
  </div>
</div>
      ...
      <div class="col-xs-12"> 
        ...
        <div class="box">
			...
			<div class="box-header" >
  				<h3 class="box-title">
					Daily Pickup List(Item Wise)
				<span id="selectedCountBadge" style="display:none;">0 selected</span>
				</h3>
				
  <div class="btn-group  pull-right" style="display:flex; align-items:center; gap:8px;">
    <select id="assign_user_select" class="form-control select2" style="width:200px;" disabled>
      <option value="">-- Select User --</option>
      <?php
        $resUsers = selectSql(TBL_USERS," WHERE id_shop='".addslashes($_SESSION['shop'])."' AND status=1 ",' ORDER BY name');
        if($db->num_rows2($resUsers)){
          while($uRow = $db->fetch_object2($resUsers)){
            echo '<option value="'.$uRow->id.'">'.ucfirst($uRow->name).' '.ucfirst($uRow->last_name).'</option>';
          }
        }
      ?>
    </select>
    <button type="button" id="assignCallBtn" class="btn btn-warning" onclick="assignForCall()" disabled title="Select records first">
      <i class="fa fa-phone"></i> Assign for Call
    </button>
	  <button type="button" id="clearSelectionBtn" class="btn btn-default" onclick="clearSelection()" style="display:none;" title="Clear selection">
          ...
          <form name="listingForm" action="" method="post">
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th width="2%"><input type='checkbox' name='pickupCheckAll' id="CheckAll" title="Select All" />&nbsp;S.No.&nbsp;</th>
	...
	$roww = mysqli_fetch_object($resw);	
		...
		$TotalValue	=	 round($roww->sales_revenueTotal + $Tax);
			...
			$id_designation = selectColumn(TBL_CUSTOMER,'designation'," WHERE `id_customer` = '".$row->id_contacts."'");
					  ...
					  //$last_remark = selectColomn('support_details','support_remark',"WHERE `id_daily_pickup` = '".$row->id."' ORDER BY id DESC LIMIT 1");
					  
					   $sql = "SELECT support_remark FROM support_details WHERE id_daily_pickup = '".$row->id."' ORDER BY id DESC LIMIT 1";
					  $result = mysqli_query($connNew, $sql);
$rowRemark = mysqli_fetch_assoc($result);
$last_remark = $rowRemark['support_remark'] ?? '';
					  
					
					?>
                  <tr>
					  <td>
  <input type="checkbox" name="pickup_ids[]" class="pickup-checkbox" value="<?=$row->id;?>"/>
  <?php echo (($_REQUEST['page']-1)*$setpage)+$counter++;?>.&nbsp;
</td>
                    <td><?=$row->bill_no;?></td>
					  <td><?php echo  date('d M Y',strtotime($row->doc_date));?></td>
					  <td>
<?php 
// Show item-level renewal date if available, otherwise fall back to header renewal date
if(!empty($row->item_renewal_date) && $row->item_renewal_date != '0000-00-00'){
    echo date('d M Y', strtotime($row->item_renewal_date));
} elseif($row->id_renewal_required == 1 && !empty($row->renewal_date) && $row->renewal_date != '0000-00-00'){
    echo date('d M Y', strtotime($row->renewal_date));
} else {
    echo '-';
}
?>
                    ...
                    <td>
						...
						<img src="images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='edit-daily-pickup.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>&return_file=<?=urlencode(basename($_SERVER['PHP_SELF']))?>&searchFormSubmit=<?=urlencode($_REQUEST['searchFormSubmit'])?>&search_name=<?=urlencode($_REQUEST['search_name'])?>&search_serial=<?=urlencode($_REQUEST['search_serial'])?>&companyId=<?=urlencode($_REQUEST['companyId'])?>&id_customer=<?=urlencode($_REQUEST['id_customer'])?>&id_products=<?=urlencode($_REQUEST['id_products'])?>&id_executive=<?=urlencode($_REQUEST['id_executive'])?>&check_box=<?=urlencode($_REQUEST['check_box'])?>&filter_date_created=<?=urlencode($_REQUEST['date_created'])?>&renewal_check=<?=urlencode($_REQUEST['renewal_check'])?>&filter_renewal_date=<?=urlencode($_REQUEST['renewal_date'])?>';" />
						
						&nbsp;&nbsp;&nbsp;&nbsp; <a  href="javascript:void(0);" id="viewbtn" onclick="OpenViewPopup('<?php echo encryptor('encrypt',$row->id); ?>');"><i class="fa fa-paper-plane"></i></a> 
...
<script type="text/javascript">
	...
	function viewSupport(id){
		...
		$.ajax({
			 url: 'ajax/ajaxSupportDrawer.php',
			type: 'GET',
			data: { id: id },
			timeout: 10000,
	...
	function addSupportFollowup(makan){
...
function updateSelectionUI() {
	var checked = document.querySelectorAll('#example2 tbody .pickup-checkbox:checked');
	var count   = checked.length;
	var total   = document.querySelectorAll('#example2 tbody .pickup-checkbox').length;
    ...
    // Enable / disable assign controls
    var userSelect    = document.getElementById('assign_user_select');
    var assignBtn     = document.getElementById('assignCallBtn');
    var clearBtn      = document.getElementById('clearSelectionBtn');

    userSelect.disabled = (count === 0);
    ...
    // Highlight selected rows
    document.querySelectorAll('#example2 tbody .pickup-checkbox').forEach(function(cb){
...
// Scoped helper — only grabs checkboxes inside the listing table
function getPickupCheckboxes() {
    return document.querySelectorAll('#example2 tbody .pickup-checkbox');
}
...
document.getElementById('CheckAll').addEventListener('change', function(e){
    ...
    getPickupCheckboxes().forEach(function(cb){ cb.checked = this.checked; }, this);
    updateSelectionUI();
}, true); 

// Individual checkbox change
document.getElementById('example2').addEventListener('change', function(e){
    if (e.target && e.target.classList.contains('pickup-checkbox')) {
        updateSelectionUI();
    }
});
...
function clearSelection() {
    getPickupCheckboxes().forEach(function(cb){ cb.checked = false; });
    document.getElementById('CheckAll').checked = false;
    updateSelectionUI();
}

// Assign for Call
function assignForCall() {
    var selected = [];
    document.querySelectorAll('#example2 tbody .pickup-checkbox:checked').forEach(function(cb){
    ...
    var assignUser = document.getElementById('assign_user_select').value;
    if (!assignUser) {
        // Shake the dropdown to draw attention
        var sel = document.getElementById('assign_user_select');
        sel.style.border = '2px solid red';
        sel.focus();
        setTimeout(function(){ sel.style.border = ''; }, 2000);
        alert('Please select a user to assign the call to.');
        return;
    }

    // Get selected user name for confirm message
    var userSelect  = document.getElementById('assign_user_select');
    var userName    = userSelect.options[userSelect.selectedIndex].text;

    if (!confirm('Assign ' + selected.length + ' record(s) to "' + userName + '" for call?')) {
        return;
    }

    // Show loading state on button
    var btn = document.getElementById('assignCallBtn');
    ...
    $.ajax({
        ...
        data: {
            act: 'assign_call',
            pickup_ids: selected,
            assign_user_id: assignUser
        },
        success: function(response){
            try {
                var res = JSON.parse(response);
                if (res.success) {
                    alert('✓ ' + selected.length + ' record(s) assigned to "' + userName + '" successfully.');
                    window.location.reload();
                } else {
                    alert('Error: ' + res.message);
...
document.addEventListener('DOMContentLoaded', function(){ 
	updateSelectionUI(); 
	 ...
	 $(document).off('change', 'input[name="check_box"]');
    $(document).off('change', 'input[name="renewal_check"]');
