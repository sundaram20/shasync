...
checkUserLevelPermission($_SESSION['userLevel'],TBL_RATE_DETAILS_UNIT,'add');
/////////////////////////////////////////////////////////////////////////////////////
// ----------cate---------
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> Rate Letter Unit Manager  <small>Rate Letter Unit</small> </h1>
    <ol class="breadcrumb">
      <li><a href="home.php"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">Rate Letter Unit Master</li>
    </ol>
  </section>
  <!-- Main content -->
...
?>
          <div class="box-header with-border">
            <h3 class="box-title"><?php echo $_REQUEST['id']==''?'Add':'Edit'?> Rate Letter Unit <?php echo $row->rate_name.$subCode;  ?> </h3>
             </div>
          <!-- /.box-header -->
          <!-- form start -->		 
          ...
          <form name="rateMaster" id="rateMaster"  method="post" enctype="multipart/form-data" role="form" data-parsley-validate autocomplete="off">           
            ...
            <div class="box-header">
			  ...
			  <div class="form-group col-sm-4">
                   ...
                   <select required="required" class="form-control select2 itemName" name="company_id" id="company_id" data-parsley-errors-container="#companyError" data-parsley_required <?php //echo $disabled; ?> onChange="getCompanyGuestName(this.value,'');areaExecutive(); " ><option value="">Select Company</option>
			...
			$companyDropDown .= '<option '.$selected.' value="'.$resultCat->id_company.'">'.ucfirst($resultCat->name).'-'.ucfirst($resultCat->city).'</option>';
					...
					<span id="companyError" style="color:red; font-size: 15px;"><?php echo $err_company;?></span>	
...
<?php 		}				 ?>
	...
	//echo $resultCat->id.'|'.$resultCat->rate_level_id .'=='. $row->id.'|'.$row->rate_level_id;
			//echo "SELECT `".TBL_RATE."`.*, `".TBL_RATE_LEVEL."`.name as level_name ,`".TBL_RATE_LEVEL."`.id as rate_level_id ,`".TBL_RATE_MARKET."`.name as market_name from `".TBL_RATE."` LEFT JOIN `".TBL_RATE_LEVEL."` ON `".TBL_RATE."`.rate_level_id=`".TBL_RATE_LEVEL."`.id   LEFT JOIN `".TBL_RATE_MARKET."` ON `".TBL_RATE."`.market=`".TBL_RATE_MARKET."`.id where   `".TBL_RATE."`.id_shop='".addslashes($_SESSION['shop'])."'  and (`".TBL_RATE."`.company_id='".$row->id_company."' || `".TBL_RATE."`.company_id='0' )"		?>            
                  ...
                  <?php $seasonDropDown = '<select class="form-control select2" name="seasonId" id="seasonId" data-parsley-errors-container="#seasonError" onchange="getseasonDate(this.value); ajaxMasterRateLetterLoad(this.value); rateLetterUnitMasterFunction();" data-parsley-required '.$disabled.' >
              ...
              <div class="row">
               <?php
			   		if($row->rate_category_id!=""){		
				?>
					<input type="hidden" id="rate_level_id_edit" name="rate_level_id_edit" value="<?php echo $row->rate_category_id;?>" />
				<?php
					}
				?>
				...
				<div class="form-group col-sm-3">
                  ...
                  <select class="form-control" name="rate_level_id" id="rate_level_id" data-parsley-required data-parsley-errors-container="#rate_level_idError" onchange="updateLevelAndMarket(); rateLetterUnitMasterFunction();getGeneralTerms(this.value,2);" <?php  echo $disabled ?>>
               ...
               <!-- <div class="form-group col-sm-4">
                  <label for="rate_level_id">Master Rate Letter<font color="#FF0000">*</font></label>
                  <select class="form-control" name="rate_level_id" id="rate_level_id" data-parsley-required data-parsley-errors-container="#rate_level_idError" onchange="updateLevelAndMarket(); rateLetterUnitMasterFunction();" <?php  echo $disabled ?>>
                  ...
                  <?php $marketDropDown = '<select class="form-control  select2 input-sm" name="new_rate_level_id" id="new_rate_level_id" data-parsley-errors-container="#new_rate_level_idError" onchange="rateLetterUnitMasterFunction(); getGeneralTerms(this.value,2);"   data-parsley-required  '.$disabled.'>
												  <option value="">Select Rate Level</option>';
                ...
                <div class="form-group col-sm-3">
                  ...
                  <?php $marketDropDown = '<select class="form-control  select2 input-sm" name="market" id="market" data-parsley-errors-container="#marketError" onchange="rateLetterUnitMasterFunction();"  data-parsley-required   >
					  ...
					  <span id="marketError"><?php echo $err_market;?></span>
				            ...
				            <div class="form-group col-sm-3">
				            	<label for="id_guest" >Terms<font style="color:red;">*</font></label>
				            	<div class="input-group" id="showGuest">
				              	<select class="form-control select2" name="generalterms" id="generalterms" required="required" data-parsley-errors-container="#generaltermsError" data-parsley-required>
...
<div id="myRadioGroup">
    <input type="radio" name="HotelDuplicateInsert"  id="HotelDuplicateInsert" onClick="SelectHotelsList(1);"   value="1" disabled="disabled" />All Hotels
           ...
           <?php }  ?>        
              ...
              <div class="row">
                <div class="form-group col-sm-4">
					  ...
					  <?php $hotelDropDown = '<select class="form-control select2" name="hotelId" id="hotelId" data-parsley-errors-container="#hotelError" onchange="getRoom(this.value,1); rateLetterUnitMasterFunction(); getInclusionDetailUnit(this.value);" data-parsley-required >
												  <option value="">Select Hotel</option>';
 ...
 <?php 
          ...
          <form name="form1" id="rateUpdate"  method="post" enctype="multipart/form-data" role="form" data-parsley-validate autocomplete="off">
            ...
            <div class="box-body box-primary" id="rateMasterDetail"> </div>
                ...
                $ratePoints=array();
                if($_REQUEST['id']==''){
                	$sqlDoc = "SELECT * FROM ".TBL_DOCUMENT_CONFIG." WHERE id_shop='".$_SESSION['shop']."' AND doc_type='2' ";
            ...
            <div style="float: left;width: 1100px;">
                 ...
                 <div class="form-group col-sm-4">
                  <label for="userlevelId">Rate Points<font style="color:red;">*</font></label>
				  <select class="form-control select2" name="rate_points[]" data-parsley-required  id="rate_points" multiple="multiple" data-parsley-errors-container="#rate_pointsError">				  
                  <?php 
					...
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						$chkSql = "SELECT * FROM `".TBL_RATE_UNIT."` WHERE FIND_IN_SET('".$resUserActions->id."',rate_points ) and id='".addslashes(encryptor('decrypt',$_REQUEST['id']))."' ";
						if($db->num_rows2(executeSql($chkSql)) > 0){
							$selected = 'selected="selected"';
						}else if($_POST[$selected]){
						$selected = 'selected="selected"';
						}
						else if(in_array($resUserActions->id,$ratePoints)){
							$selected = 'selected="selected"';
						}												
						else{
							$selected = '';
           ...
           <div class="form-group col-sm-4">
             ...
             <div class="form-group col-sm-6">
                     <label for="last_modified_by">Mail Sent By</label>
              <?php $sqlUserDetail = $db->fetch_obj2(selectSql(TBL_USERS,"WHERE `id` = '".$row->id_mail_sent_by."'",''));?>
                     <input type="text" disabled="disabled" class="form-control"  id="id_mail_sent_by"  value="<?php echo stripslashes($sqlUserDetail->username);?>">       
                   </div>
                   
               <div class="form-group col-sm-6">
                     <label for="last_modified_by">Mail Sent At</label>
              <?php ?>
                     <input type="text" disabled="disabled" class="form-control"  id="id_mail_sent_by"  value="<?php echo $row->mail_sent_at;?>">       
                   </div>          
            ...
            <div class="box-footer ">
            	<input type="hidden" name="chkFlag" id="chkFlag" value="0">	
              <input type='hidden' value='<?=($_REQUEST['id']==''?'Add':'Edit')?>' class="btn btn-primary" name="Save" >
              <input type='button' value='<?=($_REQUEST['id']==''?'Add':'Edit')?>' class="btn btn-primary" onclick="submitRateLetterUnitForm();"  id="saveButton">
              &nbsp;&nbsp;&nbsp;&nbsp;
              <input type='button' value='Cancel' class="btn btn-default" onclick='location.replace("manageRateLettersUnit.php?page=<?php echo $_GET['page']; ?>"); '>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
<?php if($_REQUEST['id']!=''){ ?>
<script>
window.onload = function() {rateLetterUnitMasterFunction(); }
...
window.onload = function() {rateLetterUnitMasterFunction(); }
 function ReviseRateLetterAutoMail(hotelId,seasonId,company_id,revise_code,sub_code){
  ...
  window.location.href = "manageRateLettersUnit.php?";
  ...
  //COMPANY AUTO COMPLETE START==================================================================
	comCheck = () =>{
     ...
     $('.itemName').select2({
        ...
        ajax: {
          url: "ajax/ajaxSearchCompanyFilter.php",
          dataType: 'json',
          delay: 50,
    ...
    window.location.href='editRateLettersUnit.php?id='+id+'&action=edit&page=1';
  });
