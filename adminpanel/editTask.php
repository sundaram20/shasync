...
if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){
	 $sql = "  SELECT * FROM `".TBL_DAILY_TASK."`
  ...
  <section class="content">
    ...
    <div class="box box-primary">
      <div class="box-header with-border" id="recentAdd" style="border:1px solid #252525; border-radius:3px;display: none; ">
        <h3 class="box-title" style="color: green;">Recently Added Task :</h3>
      ...
      <div class="box-header with-border">
        <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Task</h3>
		  <br><?php echo addslashes(encryptor('decrypt',$_REQUEST['eId']))!=''?'Task ID '.addslashes(encryptor('decrypt',$_REQUEST['eId'])):'';  ?>
      </div>
      ...
      <form name="form1" id="form1" method="post" enctype="multipart/form-data" role="form" data-parsley-validate autocomplete="off">
        ...
        <input type="hidden" value="<?php echo $_REQUEST['id_calls'];?>" name="id_calls" id="id_calls" />
    ...
    // 24 * 60 * 60 = 86400 seconds 
     $DateNoDays = abs(round($diff / 86400)); 
            ...
            <div class="form-group col-md-12">
              <label for="email" style="width:100%;" >Details<font style="color:red;">*</font>  <div style="float:right;color:#888"> Words left: <span id="word_left">15</span></div></label>
       ...
       <!-- <div class="form-group  col-md-3">
          ...
          <div class="row">
            <div class="col-sm-3">
              <div class="form-group">
                <label for="image" style="float:left;">Task Assigned To &nbsp;&nbsp; </label>
                <?php if($_REQUEST['eId']==''){?>
                <a class="pull-left btn btn-success btn-xs" onclick="addEqyFollowUp(0);" <?php echo $disabled ?> type="button" id="enqFollowUp" >Assign Task </a>
                <?php }else{?>
                <a class="pull-left btn btn-success btn-xs" <?php echo $disabled ?> type="button" >Assign Task </a>
                <?php }?>
              </div>
          ...
          <div class="box">
            <?php if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){ 
	                	...
	                	$max_id=selectColumn(TBL_DAILY_TASK_DETAILS,'MAX(id)',"WHERE  enquiry_id = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");

						$FollowupSql = executeSql("SELECT * from `".TBL_DAILY_TASK_DETAILS."` where   enquiry_id = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");
            ...
            <div id="showFolowup">
              <div class="box-body table-responsive">
						...
						if($incentive_module_approved	==1){
							...
							if(num_rows($CheckEnquiry) ==0 && $row->assign_user_id==$_SESSION['userId']){
								$id_hotel_md=	selectColumn(TBL_INCENTIVE_PARTICIPATE_HOTEL, 'id' , "WHERE hotel_id = '".$FollowupSqlRow['hotel_id']."' and status=1");							  
									if($id_hotel_md>0){	
									$claimincentive	='&nbsp;<button  data="'.$FollowupSqlRow['id'].'" class="btn btn-success btn-sm" type="button" onclick="OpenPopupAddIncentive('.$FollowupSqlRow['enquiry_id'].','.$FollowupSqlRow['lead_status'].',8);"    >Claim Incentive</button>';            
									}
							}
						}
				}
			   ...
			   $DateVisitList	.='<td>'.ucwords(selectColumn(TBL_USERS,'name'," WHERE `id` = '".$FollowupSqlRow['assign_user_id']."' ")).'</td>';
				...
				$DateVisitList .= '<td id="ChangeButton_'.$FollowupSqlRow['id'].'"><button  data="'.$FollowupSqlRow['id'].'" class="btn '.$StatusEs.'" type="button" onclick="OpenPopup('.$FollowupSqlRow['lead_status'].','.$FollowupSqlRow['id'].','.$FollowupSqlRow['enquiry_id'].','.$FollowupSqlRow['lead_status'].',8);"    >'.$ActiveINactive.'</button>';   
		...
		?>
          <div class="form-group">
            ...
            <select class="form-control select2" onchange="IncentiveStatusConfirm(this.value);" data-parsley-required name="status_incentive_id" id="status_incentive_id">
             <option value="">Sales Task Award Status</option>
	...
	function savefollowupDate() { 
	  ...
	  if(form.parsley().validate()){
		...
		var nextFollowup = form.serialize(); 
		 $.ajax({
		   type: "GET",
		   url: 'ajax/ajaxfollowupCalanderTaskUpdate.php',
		   data: nextFollowup,  
		   success: function (result) {
	         ...
	         $( "#my_popup_no" ).hide();
  		     $( ".my_popup_open" ).click();	
 		     $( "#FollowUpNextUpdate" ).html(result);window.location.href='taskSummary.php';
			},
			complete: function(){
				 alert('Email-2');/*	// Forward Enquiry Mail
		  	$.ajax({
		  		type:"POST",
		  		...
		  		success:function(data){
		  			...
		  			$( ".my_popup_open" ).click();
					$('#SuccessMessageEmail').html('<div><b>Task Details sent to : </b></div><br/>'+result.msg+'<div><br/></div>');	
