	...
	$Values['per_report_date'] =date('01-04-'.$FinanceStarYear).' to '.date("31-03-".$FinanceEndYear);
			...
			$sql_team = "SELECT id,name FROM ".TBL_TEAM." WHERE id IN (".$_SESSION['teamId'].") and id!=8 ORDER BY name";
              ...
              <div class="form-group col-sm-2">
                <label for="">&nbsp;</label>
                <a  onclick="fetchPerformanceGraphData();fetchLeadPerformance();" class="btn btn-info form-control">Search</a> </div>
              <div class="form-group col-sm-2">
                <label for="">&nbsp;</label>
...
function fetchLeadPerformance(){
        ...
        $("#loading").show();
			...
			$.ajax({
				...
				type:'POST',
				data:'per_report_date='+per_report_date+'&id_team='+id_team+'&date_period='+date_period+'&ptype='+ptype,
				success:function(data){
					$("#LeadSummaryCon").show();
