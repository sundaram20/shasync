...
//echo "<pre>";print_r($order_data1);
						...
						<tr align="middle" style="color:#fff;font-color:#fff;border:1px">
						   ...
						   <th width="15%" ><b>'.date("D",strtotime($dateCheckin)).', '.dateformat_date($dateCheckin).'</b></th>
						</tr>
	...
	$availableData .= '<div ><table class="table" style="margin:0px !important;text-align:Left;margin-top:-50px;" width="800">
       ...
       <table class="table" border="1" style="border:1px solid:red;margin-top:-10px;">
		...
		$availableData .= '<tr align="middle" style="border:1px;" >
						   ...
						   <td width="20%"  ><span '.$markColor.'>'.ucwords(selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$order_data['company']."' ")).'-'.selectColumn(TBL_COMPANY,'city'," WHERE `id_company` = '".$order_data['company']."' ").'</span></td>	
...
/*Discussion Summary -End*/
	...
	if($explodeCount > 0 && $printHead ==0){//style="page-break-inside:avoid;"
	$availableData .= '<div ><table class="table" style="margin:0px !important;text-align:Left;" width="800">
						<tr>
						  <td>
...
if($total > 0){$counter = 1;
				  ...
				  while($row2 = $db->fetch_object()){
					...
					if($row2->conveyance_remarks!=''){
						$conveyance_remarks .= '<li>'.$row2->conveyance_remarks.'</li>';
	...
	$availableData .= '
				...
				if($conveyance_remarks!=''){
					$availableData .= '<tr style="padding:0px 0px 0px 0px;margin:0px 0px 0px 0px;"><td style="padding:0px 0px 0px 0px;margin:0px 0px 0px 0px;"  colspan=10> Remarks: <ul style="padding:0px 0px 0px 70px;margin:0px 0px 0px 0px;">'.$conveyance_remarks.'</ul> </td></tr>';
				}		
				$availableData .= '</table></div>';
...
if($_REQUEST['location']=='set'){
	...
	$dompdf->stream('SalesReport.pdf', array("Attachment" => false));die;
}
else{
	$dompdf->output();
	//$dompdf->stream();
	$dompdf->stream('SalesReport.pdf', array("Attachment" => true));die;
}
/*$dompdf->load_html($availableData);
