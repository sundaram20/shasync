...
if($_REQUEST['Download']=='Download'){
 ...
 function activityLogReport($checkin,$checkout,$id_user,$id_shop,$cron,$connNew,$objPHPExcel,$fileName){
      ...
      $objDrawing->setCoordinates('D2');        //set image to cell
  ...
  cellColor('A9:D9','75923c');
    ...
    while(strtotime($checkin)<=strtotime($checkout)){
       ...
       $query =" SELECT  'VISIT' AS 'DOC',count(id) AS CHK,CONCAT(count(id),' Visits') AS 'Description','Sales Calls' AS 'ACTIVITY_TYPE'
        ...
        FROM ".TBL_DAILYVISIT."
        WHERE dated='".date('Y-m-d.',strtotime($checkin))."' AND  `".TBL_DAILYVISIT."`.`id_shop` = '".addslashes($id_shop)."' ".$cond." 
        ...
        WHERE dated='".date('Y-m-d.',strtotime($checkin))."' AND `".TBL_OTHER."`.`id_shop` = '".addslashes($id_shop)."' ".$cond."   
...
if(!isset($cron)){
if($_REQUEST['Download']=='Download')  
activityLogReport($checkin,$checkout,$_REQUEST['usernameid'],$_SESSION['shop'],$cron,$connNew,$objPHPExcel,$fileName);
