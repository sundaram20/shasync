...
if($_REQUEST['Download'] == 'Download'){
		...
		$sql = "SELECT `".TBL_DAILYVISIT."`.id_user,".TBL_USERS.".ids_team,".TBL_TEAM.".name as teamname,".TBL_USERS.".user_type,
	   ...
	   //Path to signature .jpg file
		...
		$objPHPExcel->getActiveSheet()->mergeCells('J7:O7');
		$head_hotel_row = 7;
		...
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('J'.$head_hotel_row++, 'Year to date');

		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('D8', 'Visit')
			->setCellValue('E8', 'Rateletter')
			->setCellValue('F8', 'Conveyance')
			->setCellValue('G8', 'Entertainment')
			->setCellValue('H8', 'Lunch')
			->setCellValue('I8', 'Total Expense')

			->setCellValue('J8', 'Visit')
			->setCellValue('K8', 'Rate Letter')
			->setCellValue('L8', 'Conveyance')
			->setCellValue('M8', 'Entertainment')
			->setCellValue('N8', 'Lunch')
			->setCellValue('O8', 'Total Expense')
	...
	cellColor('A6:O6','254061');

	cellColor('A7:O8','75923c');




	$objPHPExcel->getActiveSheet()->getStyle('A6')->applyFromArray($styleArray);
	 $objPHPExcel->getActiveSheet()->getStyle('A6:O6')->getAlignment()->applyFromArray(
	    array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
	);

	$objPHPExcel->getActiveSheet()->getStyle('A7:O8')->applyFromArray($styleArray_1);
	 $objPHPExcel->getActiveSheet()->getStyle('A7:O8')->getAlignment()->applyFromArray(
	    array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
	);
	...
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('O')->setWidth(15);	
	$head_hotel_row++;
		...
		$conveyanceOtherMtd =0;
		$conveyanceOtherYtd =0;	
...
$sqlIssued = "SELECT 
	...
	COUNT(IF(date_created BETWEEN '".$mtdfrom."' AND '".$toDate."',`".$RateTable."`.id, NULL)) AS	RateLetterIssuedMtd,

	COUNT(IF(date_created BETWEEN '".$ytdfrom."' AND '".$toDate."',`".$RateTable."`.id, NULL)) AS	RateLetterIssuedYtd
	
					FROM `".$RateTable."`
					
					WHERE `".$RateTable."`.id_shop=".$session_shop." AND  ".$RateTable.".created_by = ".$row->id_user;		
					
			

$resIssued = mysqli_query($conn,$sqlIssued);
$RowIssued = mysqli_fetch_object($resIssued);
		...
		->setCellValue($head_order_data++ . $connew,$RowIssued->RateLetterIssuedMtd)
		...
		->setCellValue($head_order_data++ . $connew,$RowIssued->RateLetterIssuedYtd)
		...
		//->setCellValue($head_order_data++ . $connew,$row->user_type);
		
		
		$objPHPExcel->getActiveSheet()->getStyle('A6:O'.$connew)->applyFromArray($styleThinBlackBorderOutline);
		...
		$TotalRateLetterIssuedMtd	+=	$RowIssued->RateLetterIssuedMtd;
		$TotalRateLetterIssuedYtd	+=	$RowIssued->RateLetterIssuedYtd;
		...
		cellColor('A'.$connew.':O'.$connew,'cbf9b8');
		...
		$objPHPExcel->getActiveSheet(0)->setCellValue('E'.$connew,$TotalRateLetterIssuedMtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('F'.$connew,$conveyanceMtd+$conveyanceOtherMtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('G'.$connew,$entertainmentMtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('H'.$connew,$lunchMtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('I'.$connew,($conveyanceMtd+$conveyanceOtherMtd+$entertainmentMtd+$lunchMtd));
	
		$objPHPExcel->getActiveSheet(0)->setCellValue('J'.$connew,$visitYtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('K'.$connew,$TotalRateLetterIssuedYtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('L'.$connew,$conveyanceYtd+$conveyanceOtherYtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('M'.$connew,$entertainmentYTd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('N'.$connew,$lunchYtd);
		$objPHPExcel->getActiveSheet(0)->setCellValue('O'.$connew,($conveyanceYtd+$conveyanceOtherYtd+$entertainmentYTd+$lunchYtd));
		
		
		$objPHPExcel->getActiveSheet()->getStyle('A'.$connew.':O'.$connew)->applyFromArray($totalArray);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$connew.':O'.$connew)->applyFromArray($styleThinBlackBorderOutline);
