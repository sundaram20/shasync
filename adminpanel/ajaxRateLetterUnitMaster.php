...
$resRoom = executeSql("SELECT rt.name, ahr.hotel_id,ahr.inventory, ahr.room_id,ahr.double_pax_price from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.room_id = rt.id where ahr.status='1' and rt.status='1' and ahr.hotel_id='".addslashes($hotelId)."' ORDER BY ahr.display_order ");


$resInclusion = executeSql("SELECT * from `".TBL_RATE_DETAILS_UNIT."` where hotel_id='".addslashes($hotelId)."' and rate_id='".addslashes(encryptor('decrypt',$_REQUEST['id']))."'");
...
$editRowvalue = executeSql("SELECT * FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b where  a.`id` = '".addslashes(encryptor('decrypt',$_REQUEST['id']))."' AND hotel_id='".addslashes($hotelId)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id ");
...
$editsql = executeSql("SELECT * FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b where  a.`id` = '".addslashes(encryptor('decrypt',$_REQUEST['id']))."' AND hotel_id='".addslashes($hotelId)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id ");
...
$resRate = executeSql("SELECT `".TBL_RATE_DETAILS_UNIT."`.* from `".TBL_RATE_UNIT."` LEFT JOIN `".TBL_RATE_DETAILS_UNIT."` ON `".TBL_RATE_UNIT."`.id= `".TBL_RATE_DETAILS_UNIT."`.rate_id  where `".TBL_RATE_UNIT."`.id='".$id ."' and `".TBL_RATE_DETAILS_UNIT."`.room_id='".$rowRoom->room_id."' and `".TBL_RATE_DETAILS_UNIT."`.rate_plan_id='".$rowRatePlan->id."'  and `".TBL_RATE_DETAILS_UNIT."`.rate_assign_id='".$rowInclusion->id."'");
...
$resCat_rooms22 = executeSql("SELECT rt.name, ahr.hotel_id,ahr.inventory, ahr.room_id,ahr.double_pax_price from `fs_assign_hotel_room` ahr left join `fs_room_type` rt on ahr.room_id = rt.id where ahr.status='1' and rt.status='1' and ahr.hotel_id=".addslashes($hotelId)." ORDER BY ahr.display_order");											  
...
//$availableData .= '<input type="hidden" id="data_id|'.$roomId.'|'.$rate_plan_id.'|'.$planType.'" name="data_id[]" value="|'.$roomId.'|'.$rate_plan_id.'|'.$planType.'" >';	
					...
					$availableData .= '<td><span><select class="form-control"  name="room_type_id[]" id="room_type_id" data-parsley-required  '.$disabled.'>
											  ...
											  $resCat_rooms = executeSql("SELECT rt.name, ahr.hotel_id,ahr.inventory, ahr.room_id,ahr.double_pax_price from `fs_assign_hotel_room` ahr left join `fs_room_type` rt on ahr.room_id = rt.id where ahr.status='1' and rt.status='1' and ahr.hotel_id=".addslashes($hotelId)." ORDER BY ahr.display_order");
	  ...
	  $resCat = selectSql(TBL_RATE_PLAN," where status='1' and id_shop='".addslashes($_SESSION['shop'])."' ",'  order by display_order');
			...
			$editsql = executeSql("SELECT * FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b where  a.`company_id` = '".addslashes($_POST['company_id'])."' AND										
...
//$availableData .= '<input type="hidden" id="data_id|'.$roomId.'|'.$rate_plan_id.'|'.$planType.'" name="data_id[]" value="|'.$roomId.'|'.$rate_plan_id.'|'.$planType.'" >';	
 ...
 $availableData .='</table>
			...
			$availableData .='<div class="form-group col-sm-12">
                  ...
                  <textarea rows="2" class="form-control " name="hotel_remarks" id="hotel_remarks" rows="1" placeholder="Enter Remarks" automcomplete="off" >'.$edithotel_remarks.'</textarea>
                </div>';	
