...
//checkUserLevelPermission($_SESSION['userLevel'],TBL_DAILY_TASK,'view');
...
if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_DAILY_TASK,'delete');

	

	executeSql("DELETE from `".TBL_DAILY_TASK_DETAILS."` where type='8' and enquiry_id='".addslashes(encryptor('decrypt',$_REQUEST['delId']))."' ");

    executeSql("DELETE from `".TBL_DAILY_CALENDER."` where type='8' and visit_id='".addslashes(encryptor('decrypt',$_REQUEST['delId']))."' ");

	$delSql  = "DELETE from `".TBL_DAILY_TASK."` where type='8' and id='".addslashes(encryptor('decrypt',$_REQUEST['delId']))."' ";
	...
	if(executeSql($delSql)){		
		...
		$_SESSION['successMsg'] = 'One Task  has been deleted sucessfully.';
...
$assignUserSql = "SELECT * FROM ".TBL_DAILY_TASK_DETAILS." WHERE id_user='".$_SESSION['userId']."' or `assign_user_id`='".$_SESSION['userId']."'  GROUP by enquiry_id  ";
...
$sql = " SELECT  DISTINCT A.* FROM `".TBL_DAILY_TASK."` A WHERE A.`id_shop` = '".addslashes($_SESSION['shop'])."' and A.type=8 ";
...
if($_REQUEST['Download']=='Download'){
	...
	$shop_id	=	$_SESSION['shop'];
		...
		if(!isset($cron)){
			...
			->setCellValue('A6', 'Task Summary Report   '.$CreationOrBookingDate);
			...
			->setCellValue('A6', 'Task Summary Report '.date('M-Y',strtotime($checkin)).' To '.date('M-Y',strtotime($checkout)));
		...
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Task Description')
...
cellColor('A7:M7','75923c');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A')->getAlignment()->applyFromArray(
		...
		$max_id=selectColumn(TBL_DAILY_TASK_DETAILS,'MAX(id)',"WHERE  enquiry_id = '".$row->id."' ");

					$FollowupSql = executeSql("SELECT B.*,B.lead_status AS F_STATUS,B.details as lastFollowupDesc,B.followup_close_type_id,B.dated as nextlead_date from `".TBL_DAILY_TASK_DETAILS."` As B where   id = '".$max_id."' ");
		...
		$NAme	=	$resultContact->first_name.' '.$resultContact->last_name;
					...
					$LeadFollowupDesc= selectColumn(TBL_DAILY_TASK_DETAILS,'enquiry_close_summary'," WHERE `enquiry_id` = '".$row->id."'");
...
$objPHPExcel->getActiveSheet()->getStyle('A7:M7')->applyFromArray($styleThinBlackBorderOutline);
	...
	$connew++;	
		...
		$objPHPExcel->getActiveSheet()->setTitle('Task Summary Report');
...
$objPHPExcel->getActiveSheet()
	...
	$fileName = 'Task_Summary_Report '.date('d_M_Y');
    ...
    <div class="col-xs-12">
      ...
      <div class="nav-tabs-custom">
        ...
        <form name="searchForm" action="" method="get">
          ...
          <a title="Clear" class="pull-left btn btn-success" href="manageCalls.php" style="margin-left:10px;color:#fff;font-weight:bold; float:left;">&nbsp;Clear</a>   
...
<th >	Action</th>-->
		...
		$contactNameMobile	=	''.$resultContact->first_name.' '.$resultContact->last_name.' <br>'.$resultContact->mobile;
                 ...
                 <?php /*?>   <td><?php echo ($row->hotel_id!=0?selectColumn(TBL_HOTELS,'CONCAT(name,"- ",city)'," WHERE `id` = '".$row->hotel_id."'"):"Direct");  ?></td>

                    <td><?php echo ($row->id_company!=0?selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$row->id_company."'"):"Direct").' - '.$contactNameMobile;  ?></td><?php */?>
                    ...
                    <?php
                      ...
                      //$lead_status=selectColumn(TBL_DAILY_TASK_DETAILS,'lead_status'," WHERE `enquiry_id` = '".$row->id."'");
                    ...
                    ?>
                     ...
                     <td ><?php   if($FollowupSqlRow['F_STATUS']==1){
					   ...
					   echo $LeadFollowupDesc= $FollowupCloseTypeRemarks.' - '.selectColumn(TBL_DAILY_TASK_DETAILS,'enquiry_close_summary'," WHERE `enquiry_id` = '".$row->id."'");

						 }

						// echo selectColumn(TBL_DAILY_TASK_DETAILS,'enquiry_close_summary'," WHERE `enquiry_id` = '".$row->id."'");}  ?></td>
...
?>
			...
			if($incentive_module_approved	==1){
                 ...
                 <?php if($_SESSION['userLevel']=='1'){?>
                        ...
                        <a href="javascript:void(0)" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageCalls.php?delId=<?=encryptor('encrypt',$row->id)?>&action=delete&page=<?=$_REQUEST['page']?>';}" title="Delete"><i class="fa fa-remove" ></i></a> 
