...
// Set document properties
	...
	for($i = 0 ; $i <= $diff ; $i++){
		...
		$sqlGrp .= "
			,SUM(CASE WHEN MONTH(Date)=".$startMo." AND YEAR(Date)=".$startYr." AND `DOC_TYPE`='BUDGET' THEN VALUE ELSE 0 END) AS BUDGETVALUE,
			SUM(CASE WHEN MONTH(Date)=".$startMo." AND YEAR(Date)=".$startYr." AND `DOC_TYPE`='ACHIEVED' THEN ROUND(VALUE/100000,2) ELSE 0 END) AS ACHEIVEDVALUE";
	...
	// Add some data
	$sqlGrp .= " FROM `budget_achieved_revenue`
	...
	if($res){
		...
		while ($rn = mysqli_fetch_row($res)) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('A3', 'Revenue (In Lacs)')
	...
	$objPHPExcel->getActiveSheet()->setTitle('Month Wise Revenue');
	...
	header('Content-Disposition: attachment;filename="MonthWiseRevenue_report.xls"');
