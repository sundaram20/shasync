...
//adminLoginCheck();
//checkUserLevelPermission($_SESSION['userLevel'],TBL_LEADS,'view');
$id = $_GET['id'];
$sql = "Select * from `".TBL_LEADS."` where id = '".$_GET['id']."'";
$res = executeSql($sql);
...
<div  style="width:auto; margin:auto;">
  <form name="form"  method="post" enctype="multipart/form-data"  action="ajaxprocess.php">
  <input type="hidden" name="id" value="<?=$rowUsers->id?>" />
    <table >
      ...
      <tr>
        <td width="25%" valign="top" bgcolor="#f4f4f4" class="new_text" style="padding-left: 6px;">Users <font color="#FF0000">*</font></td>
        <td width="75%" valign="top" class="text"><table width="100%" border="0">
            <tr>
              <?php 
												
					$sqlUser = selectSql(TBL_USERS,'','');
					$iCounterUser = 0;
					while($resUser = mysql_fetch_object($sqlUser)){
					$chkSql = "SELECT * FROM `".TBL_LEADS."` WHERE FIND_IN_SET('".$resUser->id."',user_id) AND `id` = '".addslashes($_GET['id'])."'";	
					if(mysql_num_rows(executeSql($chkSql)) > 0){
							$selected = 'checked="checked"';
						}else{
							$selected = '';
						}
						echo '<td><input type="checkbox" '.$selected.' name="userId[]" value="'.$resUser->id.'">'.$resUser->name.'</td>';
						if($iCounterUser == '3'){
							echo '<tr>';
						}
						$iCounterUser++;
      ...
      <tr>
        ...
        <td width="75%" valign="top" class="text"><?php if($rowUsers->parent_id != 0 ){ 
			  $leadDropDown = '<select class="may_text_area1" name="leadStatus">
				  ...
				  else {$leadDropDown = '<select class="may_text_area1" name="leadStatus">
												<option value="">---Select Lead Status---</option>';
				  $resLead = selectSql(TBL_LEADS_STATUS," WHERE `status` = '1' and `lead_status` = '2'",' ORDER BY `name`');
				  if(mysql_num_rows($resLead)){
					while($resultLead = mysql_fetch_object($resLead)){
						if($_REQUEST['leadStatus'] == $resultLead->id){
							$selected = 'selected="selected"';
      ...
      <tr>
        ...
        <td align="left"  valign="middle" bgcolor="#FFFFFF" class="padding13 padding_left14 "><input type='submit' value='Edit' class="css_btns" name="Save" >
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type='button' value='Cancel' class="css_btns" onclick=' back_prev_user("manageLeads.php"); '></td>
      </tr>
    </table>
  </form>
</div>