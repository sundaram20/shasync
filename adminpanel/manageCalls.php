...
$sql = " SELECT * FROM `call_master` where `id_shop` = '".addslashes($_SESSION['shop'])."'";
...
if(@$_REQUEST['hotelId'] != ''){//FIND_IN_SET('".$resActionId."',user_actions) 
	...
	$sql .= " AND FIND_IN_SET (".addslashes($_REQUEST['hotelId']).",`call_master`.`id_hotel`)";
...
if(@$_SESSION['HotelUserPermission'] != ''){//FIND_IN_SET('".$resActionId."',user_actions) 
	 ...
	 $sql .= " AND `call_master`.`id_hotel` IN  (".addslashes($_SESSION['HotelUserPermission']).")";

}

if(@$_REQUEST['booking_status'] != ''){

	$sql .= " AND `call_master`.`booking_status` = '".addslashes($_REQUEST['booking_status'])."%'";

}

if(@$_REQUEST['company_id'] != ''){

	$sql .= " AND `call_master`.`id_company` = '".addslashes($_REQUEST['company_id'])."'";

}

if(@$_REQUEST['guest'] != ''){

	$sql .= " AND `call_master`.`id_customer` = '".addslashes($_REQUEST['guest'])."'";

}

if(@$_REQUEST['payment_status'] != ''){

	$sql .= " AND `call_master`.`payment_status` = '".addslashes($_REQUEST['payment_status'])."'";
...
if(@$_REQUEST['booking_date'] != ''){
	...
	$sql .= " AND `call_master`.`invoice_date` = '".date('Y-m-d',strtotime($_REQUEST['booking_date']))."'";
...
if(@$_REQUEST['checkin_date'] != ''){
	...
	$sql .= " AND `call_master`.`checkin` = '".date('Y-m-d',strtotime($_REQUEST['checkin_date']))."'";
...
if(@$_REQUEST['searchFormSubmit']!='1'){
	...
	$sql .= " AND `call_master`.`date_modified` >= '".$UniqueDateFor."'";
...
<?php 	
 ...
 <td>
                  ...
                  <img src="images/view.gif" style="cursor:pointer;" title="View" name="<?php echo $row->name; ?>" id="<?php echo $row->id;?>" onClick="window.location.href='manageCallsDetails.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';"/>
