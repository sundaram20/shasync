...
checkUserLevelPermission($_SESSION['userLevel'],TBL_RATE_DETAILS_UNIT,'view');
...
if($_REQUEST['action'] == 'change'){
	...
	if($_REQUEST['activeId'] != ''){		
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_RATE_DETAILS_UNIT,'activate');

		$statusId = addslashes(encryptor('decrypt',$_REQUEST['activeId']));

		$statusSql = "	UPDATE `".TBL_RATE_UNIT."`
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_RATE_DEATILS_UNIT,'deactivate');
	...
	if(executeSql($statusSql)){
		...
		$_SESSION['successMsg'] = selectColumn(TBL_RATE_UNIT,'rate_name'," WHERE `id` = '".$statusId."'").'- Rate Letter status has been changed sucessfully.';

	}else{
		$err = 1;
		$_SESSION['errorMsg'] = selectColumn(TBL_RATE_UNIT,'rate_name'," WHERE `id` = '".$statusId."'").'- status has not been changed sucessfully.';
...
if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_RATE_UNIT,'delete');
	
   $checkExisting = executeSql("SELECT * FROM `".TBL_ORDERS."` where `id_rate` = '".$_REQUEST['delId']."'");

	  $sqlDelUsers1 = selectRow(TBL_RATE_UNIT," WHERE `id` = '".$_REQUEST['delId']."'");
	...
	if(num_rows($checkExisting)>0){
	 ...
	 $_SESSION['errorMsg'] = $sqlDelUsers1['rate_name'].' - Unable to delete this Rate Letter ';
		...
		$sqlDelUsers = "DELETE FROM `".TBL_RATE_UNIT."` WHERE `id` = '".$_REQUEST['delId']."'";
...
$sql = " SELECT  `".TBL_RATE_UNIT."`.*, `".TBL_RATE_DETAILS_UNIT."`.hotel_id  FROM `".TBL_RATE_UNIT."` LEFT JOIN `".TBL_RATE_DETAILS_UNIT."` ON `".TBL_RATE_UNIT."`.id=`".TBL_RATE_DETAILS_UNIT."`.rate_id  WHERE `".TBL_RATE_UNIT."`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_RATE_UNIT."`.`company_id` != '0'  ";
...
if($_REQUEST['rate_level_id'] != ''){
	...
	$sql .= " AND `".TBL_RATE_UNIT."`.`rate_level_id` = '".addslashes($_REQUEST['rate_level_id'])."'";

}
if($_REQUEST['companyId'] != ''){

	$sql .= " AND `".TBL_RATE_UNIT."`.`company_id` = '".addslashes($_REQUEST['companyId'])."'";
...
if($_REQUEST['seasonId'] != ''){
	...
	$sql .= " AND `".TBL_RATE_UNIT."`.`seasonId` = '".addslashes($_REQUEST['seasonId'])."'";

}

if($_REQUEST['hotelId'] != ''){

	$sql .= " AND `".TBL_RATE_DETAILS_UNIT."`.`hotel_id` = '".addslashes($_REQUEST['hotelId'])."'";
...
if($_REQUEST['state'] != ''){
	...
	$sql .= " AND `".TBL_RATE_UNIT."`.`id_state` = '".addslashes($_REQUEST['state'])."'";

}



if($_REQUEST['rateletter_id'] != ''){

	$sql .= " AND `".TBL_RATE_UNIT."`.`id` = '".addslashes($_REQUEST['rateletter_id'])."'";

}

if($_REQUEST['market'] != ''){

	$sql .= " AND `".TBL_RATE_UNIT."`.`market` = '".addslashes($_REQUEST['market'])."'";

}

if($_REQUEST['created_by'] != ''){

	$sql .= " AND `".TBL_RATE_UNIT."`.`created_by` = '".addslashes($_REQUEST['created_by'])."'";
...
if($_SESSION['HotelUserPermission'] != ''){//FIND_IN_SET('".$resActionId."',user_actions) 
	 ...
	 $sql .= " AND `".TBL_RATE_DETAILS_UNIT."`.`hotel_id` IN  (".addslashes($_SESSION['HotelUserPermission']).")";

}

	$sql .= "group by `".TBL_RATE_DETAILS_UNIT."`.rate_id order by id desc";
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <h1> Unit Rate Letter<small></small> </h1>

    <ol class="breadcrumb">

      <li><a href="home.php"><i class="fa fa-dashboard"></i> Home</a></li>

      <li class="active">Unit Rate Letter</li>
    ...
    <div class="col-xs-12">
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="box-header with-border">
          ...
          <div class="btn-group  pull-right"><a type="button" class="btn btn-success" href="editRateLettersUnit.php" >Add Rate</a>
                ...
                <div class="form-group">
				 ...
				 // $sql = ;
				   ...
				   $levelDropDown = '<select class="form-control select2" name="rateletter_id">
											  ...
											  $resCat = executeSql(" SELECT  `".TBL_RATE_UNIT."`.*, `".TBL_RATE_DETAILS_UNIT."`.hotel_id  FROM `".TBL_RATE_UNIT."` LEFT JOIN `".TBL_RATE_DETAILS_UNIT."` ON `".TBL_RATE_UNIT."`.id=`".TBL_RATE_DETAILS_UNIT."`.rate_id  WHERE `".TBL_RATE_UNIT."`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_RATE_UNIT."`.`company_id` != '0' group by `".TBL_RATE_DETAILS_UNIT."`.rate_id order by id desc");
...
if($row->sub_code ==0){
	...
	$subCode=	'-'.$row->sub_code;
                  ...
                  <tr>
                    ...
                    ?>
                    <td><?php echo ucwords(selectColumn(TBL_USERS,'name'," WHERE `id` = '".$row->created_by."'")); ?></td>
					...
					<?php if($_SESSION['userLevel']==1){ ?>	
                     <td><?=$row->status=='1'?'<span onclick="location.href=\'manageRateLettersUnit.php?inactiveId='.encryptor('encrypt',$row->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageRateLettersUnit.php?activeId='.encryptor('encrypt',$row->id).'&eId='.$_GET['eId'].'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>
					<?php } ?>	
                     <td>
...
?>
 <a href="mail-template/sendRateMailUnit.php?id=<?=encryptor('encrypt',$row->id)?>" target="_blank"><i class="fa fa-paper-plane"></i></a>&nbsp 	
  <a href="pdf-template/generateRateUnitPdf.php?id=<?=encryptor('encrypt',$row->id)?>" title="Download" target="_blank"><i class="fa fa-file-pdf-o"></i></a>
                    ...
                    <?php 
					 if($row->sub_code	==0){
						$sub_code	= $row->sub_code+1; 
						 ?>
						 &nbsp;&nbsp; <a href="editRateLettersUnit.php?hotelId=<?php echo encryptor('encrypt',addslashes($_REQUEST['hotelId'])); ?>&id=<?=encryptor('encrypt',$row->id);?>&action=edit&page=<?=$_REQUEST['page']?>&revise_code=0&sub_code=0" title="Edit" name="<?php echo $hotelId; ?>"   id="<?php echo $row->id;?>" ><i class="fa fa-pencil-square-o" ></i></a>
  ...
  <div id="FeedBack_<?php echo $row->id;?>" class="well" style="display:none;">
    <form id="FeedBackpopupForm" data-parsley-validate autocomplete="off">
        ...
        <label for="room_name">Rate Letter</label>
         ...
         <a href="editRateLettersUnit.php?hotelId=<?php echo encryptor('encrypt',addslashes($_REQUEST['hotelId'])); ?>&id=<?=encryptor('encrypt',$row->id);?>&action=edit&page=<?=$_REQUEST['page']?>&revise_code=1&sub_code=<?php echo $sub_code;?>" title="Edit"   class="btn btn-default">Revise</a>
         
         
         <a href="editRateLettersUnit.php?hotelId=<?php echo encryptor('encrypt',addslashes($_REQUEST['hotelId'])); ?>&id=<?=encryptor('encrypt',$row->id);?>&action=edit&page=<?=$_REQUEST['page']?>&revise_code=0&sub_code=0" title="Edit"   class="btn btn-default">Modify</a>
      </form>
  </div>
					 ...
					 <?php }else{
					$sub_code	= $row->sub_code+1; 
?>&nbsp;&nbsp;<a href="editRateLettersUnit.php?hotelId=<?php echo encryptor('encrypt',addslashes($_REQUEST['hotelId'])); ?>&id=<?=encryptor('encrypt',$row->id);?>&action=edit&page=<?=$_REQUEST['page']?>&revise_code=1&sub_code=<?=$sub_code;?>" title="Edit"  ><i class="fa fa-pencil-square-o" ></i></a>
