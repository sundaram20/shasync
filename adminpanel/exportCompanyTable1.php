...
FROM `".TBL_COMPANY."` AS a
			...
			AND c.id = '".addslashes($_SESSION['userId'])."'";
	...
	//echo $query;	die;					
				...
				if($fileType != 'csv'){
						...
						<tr><td style='height:50px;font-weight:bold;text-align:center;font-size:1.5em;' colspan='".$fields_num."' >Company Profile Based ".$fileName.date('d-F-Y')."</td></tr>
						<tr>
						";
...
while($rowContact = mysqli_fetch_row($resultContact)){
			...
			$dataWrite .= "<tr style='background-color:#b9e0f7'>";
			$dataWrite .= "<td></td>";
