...
if(isset($_REQUEST['id_hotel']) AND $_REQUEST['id_hotel'] !=0){
	$cond = " AND HID = ".$_REQUEST['id_hotel']."";
}
...
//print_r($_REQUEST['reservation_date']);
	...
	//Title of the report
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'HOTELWISE PRODUCTIVITY REPORT');	
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B5','HOTEL NAME');
...
//Adding Data from data base into sheet
	//$sqlEx ="SELECT executive FROM budget_acheived WHERE shop_id = 4 ".$cond." GROUP BY executive";
	...
	//while($rowEx = mysqli_fetch_object($resEx)){
		...
		$sqlMain = "SELECT hotel,
					...
					FROM budget_acheived  WHERE shop_id = '".$_SESSION['shop']."'  ".$cond." GROUP BY hotel";
		...
		while($rowMain = mysqli_fetch_object($resMain)){
			...
			if($rowMain->OTHERLAST != 0){
				//$golyOther =0;
				$golyOther = ((round($rowMain->OTHERTHIS/100000,2)-round($rowMain->OTHERLAST/100000,2))/round($rowMain->OTHERLAST/100000,2))*100;
				//$totalGolyOther+=((round($rowMain->OTHERTHIS/100000,2)-round($rowMain->OTHERLAST/100000,2))/round($rowMain->OTHERLAST/100000,2))*100;
			...
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$rowCounter,$golyOther);
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$rowCounter,round(($rowMain->VALUETHIS+$rowMain->OTHERTHIS)/100000,2));
			...
			if( ($rowMain->VALUELAST+$rowMain->OTHERLAST) !=0){
				$totalRevenueGoly = 0;
				$totalRevenueGoly = ((round(($rowMain->VALUETHIS+$rowMain->OTHERTHIS)/100000,2)-round(($rowMain->VALUELAST+$rowMain->OTHERLAST)/100000,2))/round(($rowMain->VALUELAST+$rowMain->OTHERLAST)/100000,2));
		...
		$totalGolyOtherPlusValue = (($totalRevenueThis-$totalRevenueLast)/$totalRevenueLast)*100;
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCounter++.$rowCounter,round($totalOtherLast/100000,2));
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCounter++.$rowCounter,round($totalOtherThis/100000,2));
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCounter++.$rowCounter,round($totalGolyOtherPlusValue,2));
		$objPHPExcel->getActiveSheet()->getStyle("A9:V".$rowCounter++)->applyFromArray($styleThinBlackBorderOutline);
		///////////////////////////////
	//}	    
	//exit;
//Generating Report
	$objPHPExcel->getActiveSheet()->setTitle('Hotel Wise Productivity  Report');
	...
	header('Content-Disposition: attachment;filename="HotelwiseProductivity.xls"');
    ...
    <section class="content-header">
      <h1>
       Productivity Report Manager
        <small>Hotel Wise Productivity Report</small>
      </h1>
      <ol class="breadcrumb">
	...
	<div class="box box-default">
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
          <h3 class="box-title">Hotel wise productivity report &nbsp;</small> </h3>
                    ...
                    <?php 
                        $hotelDropDown = '<select class="form-control select2 " name="id_hotel">
                                      ...
                                      if($db->num_rows2($resCat)){
                                        ...
                                        if(isset($_REQUEST['id_hotel']) && $_REQUEST['id_hotel']!="" && trim($_REQUEST['id_hotel']) == $resultCat->id){
