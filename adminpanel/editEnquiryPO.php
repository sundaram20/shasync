     ...
     $DateNoDays = abs(round($diff / 86400)); 
			...
			<div class="form-group col-md-5">
				...
				<select class="form-control select2" name="id_mst_lead_source" id="id_mst_lead_source" data-parsley-errors-container="#guestError" >
                      <option value="">Select Source1</option>
            ...
            <?php
                 ...
                 <!--ITEM DETAILS START-->  
                 <?php include('enquiryItemDetails.php');?>
                 <!--ITEM DETAILS ENDS--> 
    ...
    $resCat = mysqli_query($connNew,$citySql);
          ...
          ?>
                  ...
                  <?php $areaDropDown = '<select class="form-control input-sm" name="area" id="area"  onchange="areaOnChg(this.value);"  data-parsley-required   >
                               ...
                               $resCat = selectSql(TBL_AREAS," where status='1' and id_shop='".addslashes($_SESSION['shop'])."' AND FIND_IN_SET(id,'".$_SESSION['teamMemberAreas']."') ",' ORDER BY `name`');
                                 if($db->num_rows2($resCat)){
                                 while($resultCat = $db->fetch_object2($resCat)){
...
function SaveSalesReport(){
	...
	if(form.parsley().validate()){
		...
		$.ajax({
		   type: "POST",
		   url: 'ajax/ajaxUpdateEnquiryTest.php',	   
		   data: form.serialize(), 
		   success: function (result) {
...
//Select 2  Resolve Here
  ...
  // var counetr1=1;
   //var ledger_id=1;
        ...
        cols1 += '<td><div id="hideshow_item_codes'+ counter1 +'" ><select onchange="itemget(this.id)" name="id_inv_items_po' + counter1 + '" id="id_inv_items_po' + counter1 + '" class="form-control select3"  style="width:100%"><option>Select Item Code</option><?php 
    ...
    // var newRow11 = $('<tr id="trdeletes' + counter1 + '">');
       ...
       cols1 += '<td><div id="local'+counter1+'" name="local'+counter1+'" ><select  name="id_mst_charges_purchase_local' + counter1 + '" id="id_mst_charges_purchase_local' + counter1 + '" class="form-control select3" onchange="po_locals(this.id)" style="width:100%;"><option>Select Tax Register</option><?php 
 $sql = "SELECT mst_charges.* FROM mst_charges WHERE mst_charges.id_shop = '".addslashes($_SESSION['shop'])."' AND  charges_account = '2' ";
                       $db->query($sql); 
                      while($row1 = $db->fetch_object()){ ?>
