        ...
        <div style="float: left;width: 1100px;">
          <div class="form-group col-sm-4">
            ...
            <select class="form-control select2" name="rate_points[]"  id="rate_points" multiple="multiple" data-parsley-errors-container="#rate_pointsError">
              <?php  $rateidr	=addslashes(encryptor('decrypt',$_REQUEST['id']));
					...
					while($resUserActions = $db->fetch_object2($sqlUserActions)){
						 $chkSql1 = "SELECT * FROM `".TBL_RATE."` WHERE id='".$rateidr."' AND FIND_IN_SET('".$resUserActions->id."',rate_points) and  id_shop='".$_SESSION['shop']."' ";
						$checkRatepoint=	executeSql($chkSql1);
						if(num_rows($checkRatepoint)>'0'){
							$selected = 'selected="selected"';
