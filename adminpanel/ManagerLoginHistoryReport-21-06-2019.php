...
if($_REQUEST['Download'] == 'Download'){
	  function cellColor($cells,$color){
		...
		$sql = "SELECT a.*,count(a.id_user) as last30days ,DATE_FORMAT(a.login_date, '%m/%d/%Y'), b.hotel_access,c.name as HotelName,b.city
...
if($_REQUEST['executive_user_id'] != ''){
 ...
 $sql .=  " group by a.id_user ORDER BY c.name ,b.city ,b.name ASC";
...
cellColor('A6:F6','254061');
cellColor('A7:F7','75923c');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A7:F7')->applyFromArray($styleArray_1);
	 $objPHPExcel->getActiveSheet()->getStyle('A7:F7')->getAlignment()->applyFromArray(
	    array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
	);
...
foreach($HotelAccess as $hotelids){
    ...
    $HotelAccess2[]	=	selectColumn(TBL_HOTELS,'concat(name,"-",city)'," WHERE `id` = '".$hotelids."'");
}
		
		
$UserSelectedHotelList     = implode(',',$HotelAccess2);
if($UserSelectedHotelList==''){
    $UserSelectedHotelList  ='All Hotels';
}
if($UserSelectedHotelList==$UserSelectedHotelRechech){
	 $UserSelectedHotelList2  =$UserSelectedHotelList;
	 $UserSelectedHotelList  ='';
	 $SCount	=	'';
	 
	 $objPHPExcel->getActiveSheet()->mergeCells('A'.($mergerow).':A'.($connew));
	 $objPHPExcel->getActiveSheet()->mergeCells('B'.($mergerow).':B'.($connew));
	...
	if($UserCity==$UserCityReAssin){
	 $UserCity2  =$UserCity;
	 $UserCity  ='';
	 	  $objPHPExcel->getActiveSheet()->mergeCells('C'.($mergeCityrow).':C'.($connew));
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$connew.':F'.$connew)->applyFromArray($styleThinBlackBorderOutline);
$objPHPExcel->setActiveSheetIndex(0)
->setCellValue($head_order_data++ . $connew, $SCount)
->setCellValue($head_order_data++ . $connew, $UserSelectedHotelList)
->setCellValue($head_order_data++ . $connew, ucwords($UserCity))
...
$objPHPExcel->getActiveSheet()->getStyle('A7:F7')->applyFromArray($styleThinBlackBorderOutline);
 ...
 $HotelAccess2='';
		...
		$UserSelectedHotelRechech=$UserSelectedHotelList2;
