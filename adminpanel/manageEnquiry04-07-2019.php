...
<th >	Action</th>-->
		...
		$NAme	=	$resultContact->first_name.' '.$resultContact->last_name;
                    ...
                    ?>
                     <td ><?php   if($row->F_STATUS==1){echo $row->lastFollowupDesc;}else{ echo selectColumn(TBL_DAILY_ENQUERY_DETAILS,'enquiry_close_summary'," WHERE `enquiry_id` = '".$row->id."'");}  ?></td>
