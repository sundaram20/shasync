...
$date1	=date('d-m-Y');
  ...
  <input type="text" class="form-control pickerdate_addreport" placeholder="Enter Invoice No" id="invoiceno" name="invoiceno" value="<?php echo  $row->id_invoice; ?>"  data-parsley-required>
           ...
           <div class="form-group col-md-3">
              ...
              <input type="text" class="form-control " placeholder="Enter Source " id="source" name="source" value="<?php echo  $row->source; ?>"  data-parsley-required>
       ...
       <!-- <div class="form-group  col-md-3">
          ...
          <input type="hidden" name="edit_id_invoice" id="edit_id_invoice" value="<?php echo addslashes(encryptor('decrypt',$_REQUEST['eId'])); ?>">
          <!--CONVERTION Room Nights-->
          </div>
          <div class="row">
            <div class="col-sm-3">
              <div class="form-group">
                ...
                <?php //if($_REQUEST['eId']==''){?>
                <a class="pull-left btn btn-success btn-xs" onclick="addEqyFollowUp(0);" <?php echo $disabled ?> type="button" id="enqFollowUp" >1Add Follow Ups</a>
                <?php// }else{?>
                <a class="pull-left btn btn-success btn-xs" <?php //echo $disabled ?> type="button" >2Add Follow Ups </a>
                <?php// }?>
              </div>
          ...
          <div class="box">
            <?php if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){ 
	                	...
	                	$max_id=selectColumn('invoice_details','MAX(id)',"WHERE  id_invoice = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");

						$FollowupSql = executeSql("SELECT * from `".'invoice_details'."` where   id_invoice = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");
            ...
            <div id="showFolowup">
              <div class="box-body table-responsive">
						...
						if($incentive_module_approved	==1){
							...
							if(num_rows($CheckEnquiry) ==0 && $row->assign_user_id==$_SESSION['userId']){
								$id_hotel_md=	selectColumn(TBL_INCENTIVE_PARTICIPATE_HOTEL, 'id' , "WHERE hotel_id = '".$FollowupSqlRow['hotel_id']."' and status=1");							  
									if($id_hotel_md>0){	
									$claimincentive	='&nbsp;<button  data="'.$FollowupSqlRow['id'].'" class="btn btn-success btn-sm" type="button" onclick="OpenPopupAddIncentive('.$FollowupSqlRow['id_invoice'].','.$FollowupSqlRow['lead_status'].',4);"    >Claim Incentive</button>';            
									}
							}
						}
				}
				...
				$DateVisitList .= '<td id="ChangeButton_'.$FollowupSqlRow['id'].'"><button  data="'.$FollowupSqlRow['id'].'" class="btn '.$StatusEs.'" type="button" onclick="OpenPopup('.$FollowupSqlRow['lead_status'].','.$FollowupSqlRow['id'].','.$FollowupSqlRow['id_invoice'].','.$FollowupSqlRow['lead_status'].',4);"    >eee'.$ActiveINactive.'</button>';   
...
$incentiveSql = executeSql("SELECT * from `".TBL_INCENTIVE."` where   id_invoice = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");
...
$followup_close_summary=selectColumn(TBL_INCENTIVE,'follow_up_close_summary',"WHERE  id_invoice = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");
	...
	if($status==1){
		...
		if($FollowupSqlRow['lead_status'] == 1){
				...
				if($max_id==$FollowupSqlRow['id']){
					...
					$DateVisitList	.='&nbsp;<button  data="'.$FollowupSqlRow['id'].'" class="btn '.$StatusEs.' btn-sm" type="button" onclick="OpenIncentivePopup('.$IncentiveUpdate.');"    >sdsdUpdate</button>';            
						}
				}
				$DateVisitList	.='</td>';
...
<script type="text/javascript">
	...
	function OpenPopupAddIncentive(id_invoice,hotel_id,followup_type){
	...
	var edit_id_invoice	=	$("#edit_id_invoice").val();
			...
			$.ajax({
				...
				data: 'selectType=Add&id_hotel_md='+id_hotel_md+'&enquiryDate='+enquiryDate+'&id_invoice='+id_invoice, 
				success: function (result) {
					$('#viewincPopUp').popup('show');
	...
	function OpenPopup(followup_status,followup_id,daily_Visit_id,hotel_id,followup_type){ alert('1');
...
function IncentiveStatusConfirm(id){
	...
	if (id == "none" || id== "") { 
		...
		$.ajax({
				type: "POST",
				url: 'ajax/ajaxListHotelUser.php',
				data: 'select=2'+'&id_user_created='+id_user_created+'&id_hotel_created='+id_hotel_created+'&id_invoice='+eId, 
				success: function (result) {
	...
	var checkbox = document.getElementById('ClaimIncentive');
		if (checkbox.checked == true)
		{
			...
			$.ajax({
				type: "POST",
				url: 'ajax/ajaxIncentiveAddEditForm.php',
				data: 'selectType=1'+'&id='+id+'&id_hotel_md='+id_hotel_md+'&enquiryDate='+enquiryDate+'&edit_id_invoice='+edit_id_invoice, 
				success: function (result) {
