...
/*
$sql_order = mysql_query("SELECT * FROM `".TBL_ORDERS."`");
...
<?php include_once("includes/header.php")?>
   ...
   <div class="content-wrapper"> 
    ...
    <section class="content-header">
       <h1> Hotel Booking Manager <small>Manage Reservations</small> </h1>
       <ol class="breadcrumb">
    ...
    <section class="content">
       <div class="row"> 
        ...
        <div class="col-md-12"> 
           ...
           <div class="nav-tabs-custom">
			...
			?>
               <h3 class="box-title"><?php echo $_REQUEST['eId']==''?'Add':'Edit'?> Order : <a><?php echo $row->reference.$AmendmentTotalCount; ?></a></h3>
             </div>
            <!-- /.box-header --> 
            <!-- form start -->
	  ...
	  <?php if((!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit') || ($_GET['type']=='N' || $_SESSION['editCart'][$OrderUniqueID]['series']['type'] == 'N') || ($_GET['type']=='C' || $_SESSION['editCart'][$OrderUniqueID]['series']['type'] == 'C')){ ?>
	  ...
	  $resCat = selectSql(TBL_HOTELS," where status='1' AND id_shop='".addslashes($_SESSION['shop'])."'".$_SESSION['HotelPerHotel']." ",' ORDER BY `name`');
                        ...
                        <div class="form-group col-sm-2">
                           ...
                           <select class="form-control select2" name="room_id" id="room_id" data-parsley-required data-parsley-errors-container="#roomError">
                            <?php if($_SESSION['editCart'][$OrderUniqueID]['hotel_id']){
					$resRoom = executeSql("SELECT rt.name, ahr.hotel_id, ahr.inventory, ahr.room_id from `".TBL_ASSIGN_HOTEL_ROOM."` ahr left join `".TBL_ROOM_TYPE."` rt on ahr.room_id = rt.id where ahr.status='1' and rt.status='1' and ahr.hotel_id='".addslashes($_SESSION['editCart'][$OrderUniqueID]['hotel_id'])."'");
				  ...
				  for($i =0; $i < 6; $i++){
						...
						echo $availableData = '<th>'.$checkinDate.'</th>';
                         ...
                         <?php  } ?>
                         <?php /*?>----------------------------------Series BOOKING STAR---------------------------------<?php */?>
                        <?php
						
						 if($_GET['type']=='S' && $_REQUEST['action']!='edit' ){ ?>
                        <div class="form-group col-sm-5">
                           <label for="segment" > Series Name</label>
                           <?php 
								$seriesDropDown = '<select name="series" id="series" class="form-control select2" data-parsley-errors-container="#seriesError" onchange="getSeriesBooking();getGetSeriesDate();">
									 	 <option value="">Select Series</option>';
											  $resCat = selectSql(TBL_SERIES_MASTER,"where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."'",' ORDER BY `id`');
											  if(num_rows($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													if($row->series_id == $resultCat->id){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
									 ...
									 ?>
                           <span id="seriesError"></span> </div>
                        <div class="form-group col-sm-5">
                           <label for="operator" >Operator Name</label>
                           <?php 
								$operatorDropDown = '<select name="operator" id="operator" class="form-control select2" data-parsley-errors-container="#operatorError" onchange="getSeriesBooking();getGetSeriesDate();">
									 	 <option value="">Select Operator</option>';
											  $resCat = selectSql(TBL_OPERATOR_MASTER,"where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `id`');
											  if(num_rows($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
													if($row->operator_id == $resultCat->id){
														$selected = 'selected="selected"';
													}else{
														$selected = '';
													}
									 ...
									 ?>
                           <span id="operatorError"></span> </div>
                           
                              <div style="margin-top: 10px;float: left; width:100%">
                           <div class="form-group col-sm-5">
                           <label for="id_company">Source</label>
                           <select class="form-control" name="id_company" id="id_company" onChange="ajaxAddRoommsgUpdate(); getcreditvalueNew(this.value,''); getContact(this.value,''); getRateLetter(this.value,'');  getGetSeriesDate(); getSeriesBooking();" data-parsley-errors-container="#companyError" data-parsley-required >
                        ...
                        <div class="form-group col-sm-5" >
                         ...
                         <div style="color:red; float:left; margin-left:15px; margin-top:29px;" id="getcredit_value"></div>
                          ...
                          <!-- dateRangeEdit SerieldateRangeNewEdit-->
                       ...
                       <div class="form-group col-sm-3">
                           ...
                           <div class="input-group">
                               ...
                               <div class="input-group-addon"> <i class="fa fa-calendar"></i></div>
                       <input type="text" class="form-control pull-right dateRangeEdit" id="reservation_date" name="reservation_date" value="">
                             </div>
  ...
  <?php /*?>----------------------------------Series BOOKING END---------------------------------<?php */?>
                        ...
                        <?php if((!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit') || ($_GET['type']=='N' || $_SESSION['editCart'][$OrderUniqueID]['series']['type'] == 'N') || ($_GET['type']=='C' || $_SESSION['editCart'][$OrderUniqueID]['series']['type'] == 'C')){ ?>
                         ...
                         <div class="form-group col-sm-5">
                           <label for="segment" > Series Name</label>
                           <?php 
								$seriesDropDown = '<select name="series" id="series" class="form-control select2" data-parsley-errors-container="#seriesError" >
									 	 <option value="">Select Series</option>';
											  $resCat = selectSql(TBL_SERIES_MASTER,"where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."'",' ORDER BY `id`');
											  if(num_rows($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
                        ...
                        <div class="form-group col-sm-5">
                           <label for="operator" >Operator Name</label>
                           <?php 
								$operatorDropDown = '<select name="operator" id="operator" class="form-control select2" data-parsley-errors-container="#operatorError" >
									 	 <option value="">Select Operator</option>';
											  $resCat = selectSql(TBL_OPERATOR_MASTER,"where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' ",' ORDER BY `id`');
											  if(num_rows($resCat)){
											  	while($resultCat = $db->fetch_object2($resCat)){
