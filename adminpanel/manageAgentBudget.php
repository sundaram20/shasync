...
checkUserLevelPermission($_SESSION['userLevel'],TBL_AGENT_BUDGET,'view');
...
if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){
  ...
  checkUserLevelPermission($_SESSION['userLevel'],$budgetTable,'delete');
	$delSql = "DELETE FROM `".$budgetTable."` WHERE `season_id` = '".addslashes($_REQUEST['season_id'])."' and `id_hotel` = '".addslashes($_REQUEST['id_hotel'])."' and `id_user` = '".addslashes($_REQUEST['id_user'])."'  ";

	

	

	 $sqlDelUsers = selectRow($budgetTable," WHERE `season_id` = '".addslashes($_REQUEST['season_id'])."' and `id_hotel` = '".addslashes($_REQUEST['id_hotel'])."' and `id_user` = '".addslashes($_REQUEST['id_user'])."' ");
...
$sql = " SELECT  * FROM `".$budgetTable."`  WHERE `".$budgetTable."`.`id_shop` = '".addslashes($_SESSION['shop'])."' ";
...
if($_REQUEST['searchFormSubmit'] =='1'){
  ...
  if($_REQUEST['user_id'] != ''){
  	...
  	$sql .= " AND `".$budgetTable."`.`id_user` = '".addslashes($_REQUEST['user_id'])."'";

  }
  elseif($_SESSION['teamMemberLevel']==1){

    $sql .= " AND `".$budgetTable."`.`id_user` in (".$_SESSION['teamMembers'].")";
  }
  else{
    $sql .= " AND `".$budgetTable."`.`id_user` in (".$_SESSION['userId'].")";
  }

  if($_REQUEST['season_id'] != ''){

  	$sql .= " AND `".$budgetTable."`.`season_id` = '".addslashes($_REQUEST['season_id'])."'";

  }

  if($_REQUEST['hotelId'] != ''){

  	$sql .= " AND `".$budgetTable."`.`id_hotel` = '".addslashes($_REQUEST['hotelId'])."'";
  ...
  if(!isset($_SESSION['teamMemberLevel'])){
    $sql .= " AND `".$budgetTable."`.`id_user` = '".$_SESSION['userId']."'";
  }
  else{
    $sql .= " AND `".$budgetTable."`.`id_user` IN (".$_SESSION['teamMembers'].")";
  }
...
$sql =	$sql." group by id_user,season_id ".$condHotGroup." ";
    ...
    <div class="col-xs-12">
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="box-header with-border">
          ...
          <div class="btn-group  pull-right"><a type="button" class="btn btn-success" href="editAgentBudget.php" >Add Budget</a>
            ...
            <ul class="dropdown-menu" role="menu">
              	...
              	<li><a title="Import Budget" href="" data-toggle="modal" data-target="#budgetModal"><img src="images/excel-icon.jpg" width="20" height="20" />&nbsp;Import</a></li>
								...
								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo $budgetTable;?>"><img  src="images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li>
          ...
          <div class="box-body">
            ...
            <div class="row">
              ...
              <!--<div class="form-group col-sm-6">
                ...
                <label for="season_id">Company <font color="#FF0000">*</font></label>
                ...
                <?php $hotelDropDown = '<select class="form-control select2" name="hotelId" id="hotelId" '.$disabledHotel.'>
														  ...
														  if($db->num_rows2($resCat)){
															...
															while($resultCat = $db->fetch_object2($resCat)){
																...
																if($_REQUEST['hotelId']== $resultCat->id_company){
																...
																$hotelDropDown .= '<option '.$selected.' value="'.$resultCat->id_company.'">'.ucfirst($resultCat->name).'</option>';
                ...
                <div class="form-group">
                  ...
                  <?php $levelDropDown = '<select class="form-control select2" name="user_id">
											 ...
											 if(empty($_SESSION['hotel_access'])){
												  ...
												  $resCat = selectSql(TBL_USERS," where status='1' and  id_shop='".addslashes($_SESSION['shop'])."' ".$UserRestriction." ".$teamMembers." ",' ORDER BY `name`');												}
              ...
              <div class="form-group col-sm-6">
                ...
                <label for="season_id">Season<font color="#FF0000">*</font></label>

                <?php $seasonDropDown = '<select class="form-control select2" name="season_id" id="season_id" >
											  ...
											  if($db->num_rows2($resCat)){
											  	...
											  	while($resultCat = $db->fetch_object2($resCat)){
													...
													if($resultCat->id == $_REQUEST['season_id']){
              ...
              <table id="example2" class="table table-bordered table-striped">
				...
				if($total > 0){$counter = 1;
                  ...
                  <tr>
                   ...
                   <td><?php echo selectColumn(TBL_BUDGET_YEAR,'name'," WHERE `id` = '".$row->season_id."'");   ?></td>
...
?>
                    ...
                    &nbsp;&nbsp; <a href="editAgentBudget.php?hotelId=<?php echo encryptor('encrypt',$row->id_user); ?>&season_id=<?=encryptor('encrypt',$row->season_id);?>&id=<?=encryptor('encrypt',$row->id);?>&id_hotel=<?=encryptor('encrypt',$row->id_hotel);?>&action=edit&page=<?=$_REQUEST['page']?>" title="Edit"><i class="fa fa-pencil-square-o" ></i></a>

                    &nbsp;&nbsp; <!--<a href="javascript:void(0)" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='managebudget.php?delId=<?=encryptor('encrypt',$row->id)?>&season_id=<?php echo $row->season_id; ?>&id_hotel=<?php echo $row->id_hotel;?>&id_user=<?php echo $row->id_user; ?>&action=delete&page=<?=$_REQUEST['page']?>';}" title="Delete"><i class="fa fa-remove" ></i></a>--> </td>
