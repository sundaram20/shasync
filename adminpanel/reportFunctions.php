...
error_reporting(E_ALL);
	...
	function hotelWiseContractReport($cron,$id_shopCron,$hotelIdCron,$seasonIdCron,$db,$objPHPExcel){
		...
		if(!isset($cron)){
			...
			$rateSql="SELECT `".TBL_RATE_DETAILS."`.*,`".TBL_RATE."`.*, `".TBL_RATE."`.id  as detail_id FROM `".TBL_RATE_DETAILS."` join `".TBL_RATE."` on fs_rate.id=fs_rate_details.rate_id  LEFT JOIN `fs_company` AS cmp ON cmp.id_company=".TBL_RATE.".company_id  WHERE 1=1 AND ".TBL_RATE.".company_id!=0  ".$cond."  group by ".TBL_RATE.".id  order by ".TBL_RATE.".seasonId ,cmp.name asc";	
		}
...
$city = selectColumn(TBL_COMPANY,'city'," WHERE `id_company` = '".$RatecheckCountRecords->company_id."'");
	...
	function executivePortFolioYearlyReport($cron,$id_shop,$id_user,$fy_date,$fileName="",$conn="",$objPHPExcel=""){
		...
		if($numRows > 0){
			...
			while($row = mysqli_fetch_object($res)){
				...
				$budget = selectColumn(TBL_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."'  ");
				...
				$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue($head_order_data++ . $connew, str_replace('&amp;','&', $row->name))
							...
							->setCellValue($head_order_data++ . $connew, $sumRow->rate_name)
							->setCellValue($head_order_data++ . $connew, $winRow->rate_name)
							->setCellValue($head_order_data++ . $connew, $yearRow->rate_name)
							->setCellValue($head_order_data++ . $connew,($prevAch2==''?0:$prevAch2) )
							->setCellValue($head_order_data++ . $connew,($prevAch==''?0:$prevAch))
							->setCellValue($head_order_data++ . $connew,$achieved==''?0:$achieved)
							->setCellValue($head_order_data++ . $connew,$budget==''?0:$budget);
	...
	//Feedback Report
	function feedbackReport($conn,$shop_id,$checkin,$checkout,$id_user,$feedType,$lead_status,$objPHPExcel,$teamMembers,$cron,$fileName){
		...
		if($numRows > 0){
			...
			if(!isset($cron)){
				...
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A6', 'Feed Back Report  Of Month '.date('M-Y',strtotime($checkout)));
			}	
	...
	//Followup report
	function followUpReport($conn,$objPHPExcel,$shop_id,$fileName,$cron,$teamMembers,$cond){
		if(isset($cron)){
			$cond='';
		...
		$sql = " SELECT  3 AS display_order,id,lead_status,'Sales Report' AS 'Source',hotel_id,id_user,assign_user_id,dated,date_created,follow_up_summary AS summary,followup_close_type_id AS id_close_type,followup_close_summary AS close_summary  FROM `".TBL_FOLLOWUP_DETAILS."`  WHERE follow_up_summary !='' AND   `".TBL_FOLLOWUP_DETAILS."`.`id_shop` = '".$shop_id."'  ".$cond."
			
			UNION ALL

			SELECT  2 AS display_order,id,lead_status,'Enquiry' AS 'Source',hotel_id,id_user,assign_user_id,follow_up_date AS dated,created_date AS date_created,follow_up_summary AS summary,'' AS id_close_type,'' AS close_summary  FROM `".TBL_DAILY_ENQUERY."`  WHERE follow_up_summary !='' AND   `".TBL_DAILY_ENQUERY."`.`id_shop` = '".$shop_id."' ".str_replace('dated', 'follow_up_date', $cond)."

			UNION ALL

			SELECT  1 AS display_order,id,lead_status,'Quote' AS 'Source',hotel_id,id_user,assign_user_id,follow_up_date As dated,created_date AS last_created,follow_up_summary AS summary,'' AS id_close_type,'' AS close_summary  FROM `".TBL_SALES_QUOTE."`  WHERE follow_up_summary !='' AND   `".TBL_SALES_QUOTE."`.`id_shop` = '".$shop_id."' ".str_replace('dated', 'follow_up_date',$cond)."
		";
		...
		if($numRows > 0){
			...
			if(!isset($cron)){
				...
				$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A7', 'Follow Up Report  As on '.date('t-M-Y'));
			}	
			...
			cellColor('A7:K7','254061');
			cellColor('A8:K8','75923c');
			
			$objPHPExcel->getActiveSheet()->getStyle('A7')->applyFromArray($styleArray);
			$objPHPExcel->getActiveSheet()->getStyle('A7:K7')->getAlignment()->applyFromArray(
			...
			$objPHPExcel->getActiveSheet()->getStyle('A8:K8')->applyFromArray($styleArray_1);
			$objPHPExcel->getActiveSheet()->getStyle('A8:K8')->getAlignment()->applyFromArray(
			    array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
			);
			...
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('A')->setWidth(6);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('B')->setWidth(30);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('C')->setWidth(17);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('D')->setWidth(15);
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('E')->setWidth(20);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('F')->setWidth(15);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('G')->setWidth(40);	
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('H')->setWidth(13);
			$objPHPExcel->getActiveSheet(1)->getColumnDimension('I')->setWidth(17);
			...
			while($row = mysqli_fetch_object($res)){
				...
				if($row->lead_status==1){
					...
					->setCellValue($head_order_data++ . $connew, date('d-M-Y',strtotime($row->dated)))
					->setCellValue($head_order_data++ . $connew,$row->summary)
...
$report_date= explode(" to ",$report_date);
