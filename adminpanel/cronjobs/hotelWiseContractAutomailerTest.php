...
$season =array();
$CronSql = "SELECT id FROM `fs_cron` WHERE   `run_status`='0'  AND success_status='0' ";
$resCron = mysqli_query($connNew,$CronSql);
$rowCron = mysqli_fetch_object($resCron);
$rowNum = mysqli_num_rows($resCron);

if($rowNum>0){
 $editRateDetail = "UPDATE `fs_cron` SET `run_status` = '1' , datetimei='".date('Y-m-d H:i:s')."'   WHERE `id` = '".$rowCron->id."' ";

mysqli_query($connNew,$editRateDetail);	
...
//85
 ...
 $seasonSql = "SELECT id FROM ".TBL_RATE_SEASON." WHERE id_shop=".$id_shop."  AND status=1  and id IN('89')";
...
//print_r($season);
   $hotelSql = "SELECT id FROM ".TBL_HOTELS." WHERE id_shop=".$id_shop."  AND status=1 and id IN ('".$rowCron->id."') ORDER BY display_order ";
...
$sendMail = $mail->Send(); 
	...
	$editRateDetail2 = "UPDATE `fs_cron` SET `success_status` = '1' , datetimef='".date('Y-m-d H:i:s')."'  WHERE `id` = '".$rowCron->id."' ";
	mysqli_query($connNew,$editRateDetail2);
						
					}else{ //noRates
						
									//$objWriter->setPreCalculateFormulas(false)
									$msg = 'No Rates-'.$seasonName.'-'.$hotelName.'_'.$hotelCity;
									$msg = wordwrap($msg,70);
									$From="support@roomstatushub.com";
									$sub= 'No Rates - '.$seasonName;
									$mail = new PHPMailer(); // create a new object	
									$mail->IsSMTP(); // enable SMTP
									//$mail->SMTPDebug = 2; // debugging: 1 = errors and messages, 2 = messages only
									$mail->SMTPAuth = true; // authentication enabled
