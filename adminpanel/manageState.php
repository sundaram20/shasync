...
checkUserLevelPermission($_SESSION['userLevel'],MST_TBL_STATE_ZONE,'view');

if($_REQUEST['action'] == 'change'){

	if($_REQUEST['activeId'] != ''){

		checkUserLevelPermission($_SESSION['userLevel'],MST_TBL_STATE_ZONE,'activate');

		$statusId = addslashes(encryptor('decrypt',$_REQUEST['activeId']));

		$statusSql = "	UPDATE `".MST_TBL_STATE_ZONE."`
		...
		checkUserLevelPermission($_SESSION['userLevel'],MST_TBL_STATE_ZONE,'deactivate');
	...
	if(executeSql($statusSql)){
		...
		$_SESSION['successMsg'] = 'Zone '.selectColumn(TBL_STATE,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';

	}else{

		$err = 1;

		$_SESSION['errorMsg'] = 'Zone '.selectColumn(TBL_STATE,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';

	}

}else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){

	checkUserLevelPermission($_SESSION['userLevel'],TBL_STATE,'delete');

	$delSql = "DELETE FROM `".TBL_STATE."` WHERE `id` = '".$_REQUEST['delId']."'";

	$sqlDelUserLevel = selectRow(TBL_STATE," WHERE `id` = '".$_REQUEST['delId']."'");

	if(executeSql($delSql)){		

		$err = 0;

		$_SESSION['successMsg'] = 'One State '.$sqlDelUserLevel["name"].' has been deleted sucessfully.';

	}else{

		$err = 1;

		$_SESSION['errorMsg'] = 'Unable to delete State '.$sqlDelUserLevel["name"];
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_STATE,'activate');	
	...
	checkUserLevelPermission($_SESSION['userLevel'],TBL_STATE,'deactivate');	
	...
	$delSql = "DELETE FROM `".TBL_STATE."` WHERE `id` IN (".addslashes($deleteIds).")";
...
$sql = " SELECT * FROM `".TBL_STATE."` WHERE id_country='110' and status='1' ";
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
							  ...
							  <ul class="dropdown-menu" role="menu">
								...
								<?php ?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_STATE;?>"><img src="images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>

								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_STATE;?>"><img  src="images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php ?>
            ...
            <div class="box-body table-responsive">
              ...
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?=$row->name;?></td>
                  <td><?php 
                  $id_zone = selectColumn(MST_TBL_STATE_ZONE,'id_zone','where id_state="'.$row->id_state.'" ');
                  echo selectColumn(TBL_ZONAL,'name','where id="'.$id_zone.'" ');
                  		?>
                  </td>

                  

                  <td><?=$row->status=='1'?'<span onclick="location.href=\'manageState.php?inactiveId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'manageState.php?activeId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 
