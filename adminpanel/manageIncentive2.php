...
if($_REQUEST['Download']=='Download'){
	...
	$EnqSql 	 = 	mysqli_query($conn,$sql);
		...
		//$objPHPExcel->getActiveSheet()->mergeCells('M7:O7');
			...
			->setCellValue('M7', 'Incentive Summary Details');
		$head_hotel_row = 8;
...
//
			...
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Incentive Last Remarks')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Incentive Status');
...
$objPHPExcel->getActiveSheet(1)->getColumnDimension('T')->setWidth(15);
	...
	$Incentive_status='Verified By Corporate';
...
$objPHPExcel->getActiveSheet()
	...
	$fileName = 'Incentive_Summary_Report '.date('d_M_Y');
		ob_end_clean();
		if(!isset($cron)){
...
<?php include_once("includes/header.php")?>
  ...
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1> Incentive Manager<small>Incentive Master</small> </h1>
      <ol class="breadcrumb">
...
-->
	  ...
	  $resCat = selectSql(TBL_HOTELS," where status='1' AND id_shop='".addslashes($_SESSION['shop'])."'".$_SESSION['HotelPerHotel']." ",' ORDER BY `name`');
                ...
                <div class="col-md-5">
				  ...
				  <option '.$selected1.' value="1">Verified By Corporate</option>
...
?>
  ...
  <div id="viewincPopUp" class="well" style="display:none; background-color:#fff;">
    ...
    <div class="box-header with-border">
              <h3 class="box-title">Incentive Payment </h3>
