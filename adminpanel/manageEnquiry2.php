...
echo $assignUserSql = "SELECT eq.id , case when eqd.lead_status='1' then 'OPEN' when eqd.lead_status='0' then 'CLOSE' end as leadStatus, hotel.name as hotelName, eqd.created_by, eqd.assign_user_id,
...
SELECT eq.id ,
	...
	case when eqd.lead_status='1'  then 'OPEN' 
		when eqd.lead_status='0'  then 'CLOSE'
		 end
		as leadStatus,
	...
	created_by, 
	assign_user_id
				FROM ".TBL_DAILY_ENQUERY_DETAILS." as eqd  
				INNER JOIN fs_enquiry as eq ON eq.id=eqd.enquiry_id AND eqd.id IN(SELECT max(id) from `".TBL_DAILY_ENQUERY_DETAILS."` group by enquiry_id )
				
				INNER JOIN fs_hotels as hotel ON hotel.id=eq.hotel_id
				INNER JOIN fs_users usr on usr.id=eqd.created_by
				LEFT  JOIN fs_incentive as inc  ON inc.enquiry_id=eqd.enquiry_id
 ...
 ";
					 ...
					 $FollowupSql = "SELECT max(id) as ids_enquiry_details from `".TBL_DAILY_ENQUERY_DETAILS."` group by enquiry_id ";

					
					
					$assignUserQuery= mysqli_query($connNew,$FollowupSql);
$assignUserEnquiryId=array();
while($assignUserResult=mysqli_fetch_object($assignUserQuery)){
array_push($assignUserEnquiryId,$assignUserResult->ids_enquiry_details);
}
//print_r($assignUserEnquiryId);
      ...
      <div class="nav-tabs-custom">
        ...
        <form name="searchForm" action="" method="get">
          ...
          <div class="box-footer">
            ...
            <a title="Clear" class="pull-left btn btn-success" href="manageEnquiry.php" style="margin-left:10px;color:#fff;font-weight:bold; float:left;">&nbsp;Clear</a> </div>
        </form>
        <div class="box">
