...
checkUserLevelPermission($_SESSION['userLevel'],TBL_BUDGET_MASTER,'delete');
	...
	$_SESSION['successMsg'] = 'One Hotel Wise Monthly Achieved has been deleted sucessfully'; //.selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$sqlDelUsers['hotel_id']."'").'-'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$sqlDelUsers['room_id']."'").' has been deleted sucessfully.';
...
$err = 1;
		...
		$_SESSION['errorMsg'] = 'Unable to delete Hotel Wise Monthly Achieved ';//.selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$sqlDelUsers['hotel_id']."'").'-'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$sqlDelUsers['room_id']."'");
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <h1> Hotel Wise Monthly Achieved  <small>Hotel Budget Achieved Master</small> </h1>
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Hotel Wise Monthly Achieved</li>
    ...
    <div class="col-xs-12">
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="box-header with-border">
          ...
          <div class="btn-group  pull-right"><a type="button" class="btn btn-success" href="editHotelBudgetAchieved.php" >Add Achieved</a>
        ...
        <div class="box">
          ...
          <div class="box-header">
            ...
            <h3 class="box-title">Hotel wise Monthly Achieved List</h3>
...
?>
                    ...
                    &nbsp;&nbsp; <a href="editHotelBudgetAchieved.php?id_user=<?php echo encryptor('encrypt',$row->id_user); ?>&seasonId=<?=encryptor('encrypt',$row->seasonId);?>&id=<?=encryptor('encrypt',$row->id);?>&action=edit&page=<?=$_REQUEST['page']?>" title="Edit"><i class="fa fa-pencil-square-o" ></i></a>



                    &nbsp;&nbsp; <a href="javascript:void(0)" onClick="if(confirm('Are you sure that you want to delete this record <?=$row->name;?>?')){window.location.href='manageHotelBudgetAchieved.php?delId=<?=encryptor('encrypt',$row->id)?>&seasonId=<?php echo $row->seasonId; ?>&type=<?php echo $row->type;?>&id_user=<?php echo $row->id_user; ?>&action=delete&page=<?=$_REQUEST['page']?>';}" title="Delete"><i class="fa fa-remove" ></i></a> 

 &nbsp;&nbsp;<a href="manageHotelBudgetAchieved.php?method=Download&id_user=<?php echo encryptor('encrypt',$row->id_user); ?>&seasonId=<?=encryptor('encrypt',$row->seasonId);?>>&type=<?php echo $row->type;?>" title="Download" ><i class="fa fa-file-excel-o"></i></a>
