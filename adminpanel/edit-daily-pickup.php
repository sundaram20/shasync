	...
	if($err == 0){//No error
		if(($_REQUEST['Save'] == 'Add')){    
			...
			$addSql = "   	INSERT INTO `daily_pickup` SET
							...
							`assign_user_id` = '".$_REQUEST['user_id']."',
		...
		//die;
			...
			foreach($_REQUEST['dailypickup'] as $datapickup){
				...
				$addSql = " INSERT INTO `daily_pickup_details` SET
							...
							`serial_number` = '".$datapickup['serial_number']."',
							`qty` = '".$datapickup['qty']."',
							...
							`renewal_date` = '".date('Y-m-d', strtotime($datapickup['renewal_date']))."',
							`renewal_status` = '".$datapickup['renewal_status']."'
			 ...
			 $returnFile = !empty($_REQUEST['return_file']) ? basename($_REQUEST['return_file']) : 'ManagerDailyPickup.php';

$filterParams = http_build_query([
    'page'              => $_REQUEST['page'],
    'searchFormSubmit'  => $_REQUEST['searchFormSubmit'],
    'search_name'       => $_REQUEST['search_name'],
    'search_serial'     => $_REQUEST['search_serial'],
    'companyId'         => $_REQUEST['companyId'],
    'id_customer'       => $_REQUEST['id_customer'],
    'id_products'       => $_REQUEST['id_products'],
    'id_executive'      => $_REQUEST['id_executive'],
    'check_box'         => $_REQUEST['check_box'],
    'date_created'      => $_REQUEST['filter_date_created'],
    'renewal_check'     => $_REQUEST['renewal_check'],
    'renewal_date'      => $_REQUEST['filter_renewal_date'],
	'Search'            => 'Search',
]);

header("location:" . $returnFile . "?" . $filterParams);
exit;
...
<div class="content-wrapper"> 
    ...
    <div class="box box-default" style="margin-bottom : 0!important;"> 
      ...
      <form name="DailyPickupForm" id="DailyPickupForm" action="" method="get">
        ...
        <input type="hidden" value="<?php echo $_REQUEST['eId'];?>" name="eId" />
		  <input type="hidden" name="return_file"     value="<?php echo htmlspecialchars(basename($_REQUEST['return_file'] ?? 'ManagerDailyPickup.php')); ?>" />
    <input type="hidden" name="search_name"     value="<?php echo htmlspecialchars($_REQUEST['search_name'] ?? ''); ?>" />
    <input type="hidden" name="search_serial"   value="<?php echo htmlspecialchars($_REQUEST['search_serial'] ?? ''); ?>" />
    <input type="hidden" name="companyId"       value="<?php echo htmlspecialchars($_REQUEST['companyId'] ?? ''); ?>" />
    <input type="hidden" name="id_customer"     value="<?php echo htmlspecialchars($_REQUEST['id_customer'] ?? ''); ?>" />
    <input type="hidden" name="id_products"     value="<?php echo htmlspecialchars($_REQUEST['id_products'] ?? ''); ?>" />
    <input type="hidden" name="id_executive"    value="<?php echo htmlspecialchars($_REQUEST['id_executive'] ?? ''); ?>" />
    <input type="hidden" name="check_box"       value="<?php echo htmlspecialchars($_REQUEST['check_box'] ?? ''); ?>" />
    <input type="hidden" name="filter_date_created"    value="<?php echo htmlspecialchars($_REQUEST['filter_date_created'] ?? ''); ?>" />
    <input type="hidden" name="renewal_check"   value="<?php echo htmlspecialchars($_REQUEST['renewal_check'] ?? ''); ?>" />
    <input type="hidden" name="filter_renewal_date"    value="<?php echo htmlspecialchars($_REQUEST['filter_renewal_date'] ?? ''); ?>" />
 ...
 ?>
			  ...
			  <div class="form-group col-sm-4">
    <label for="id_vendor">Vendor Name</label>
    <div class="input-group enquirypage" id="showvendorby">
        <select class="form-control select2 vendorName" name="id_vendor" id="id_vendor">
            <option value="">Select Vendor</option>
            <?php
            if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){
                if($row->id_vendor != ''){
                    $vendorName = selectColumn('fs_vendors', 'name', " WHERE `id` = '".$row->id_vendor."'");
                    echo '<option selected="selected" value="'.$row->id_vendor.'">'.$vendorName.'</option>';
                }
            }
            ?>
        </select>
        <!-- Show pencil if vendor selected, plus if not -->
        <div class="input-group-addon vendorby_open" id="vendorby_btn">
            <i class="fa fa-plus" id="vendorby_icon"></i>
        </div>
    </div>
</div>
...
<div id="addCon" class="input-group-addon bookedby_open"><i class="fa fa-plus"></i></div>
  ...
  <li> <i class="fa fa-window-maximize bg-green"></i>
	...
	$sql2 = " SELECT dpd.*, fh.serial_number_applicable 
          FROM `daily_pickup_details` dpd
          LEFT JOIN `fs_hotels` fh ON fh.id = dpd.id_product
          WHERE dpd.`id_daily_pickup` = '".addslashes(encryptor(decrypt,$_REQUEST['eId']))."' ";
								 $db->query($sql2); 
$k=1;
...
?>
			   ...
			   <?php 
				$is_required = ($rowsID->serial_number_applicable == '1') ? 'data-parsley-required="true" required' : '';
				?>

			    <td><input type="text"  autocomplete="off"  name="dailypickup[<?php echo $k;?>][serial_number]" id="sr_no<?php echo $k;?>" placeholder="Enter Serial no" onkeyup="qtycalc_rows(this.id,<?php echo $k;?>)" class="form-control"  <?php echo $is_required; ?> value="<?php echo $rowsID->serial_number; ?>"/></td>
...
<td class="renewal_date_col_<?php echo $k;?>" style="<?php echo ($rowsID->serial_number_applicable == '1') ? '' : 'display:none'; ?>">
  <input type="text" autocomplete="off" 
    name="dailypickup[<?php echo $k;?>][renewal_date]" 
    id="renewal_date_item<?php echo $k;?>" 
    placeholder="DD-MM-YYYY" 
    class="form-control pickerdate_addreport" 
    value="<?php echo ($rowsID->renewal_date != '0000-00-00' && !empty($rowsID->renewal_date)) ? date('d-m-Y', strtotime($rowsID->renewal_date)) : ''; ?>" />
</td>
<td class="renewal_status_col_<?php echo $k;?>" style="<?php echo ($rowsID->serial_number_applicable == '1') ? '' : 'display:none'; ?>">
  <select name="dailypickup[<?php echo $k;?>][renewal_status]" 
    id="renewal_status_item<?php echo $k;?>" 
    class="form-control select2">
    <option value="">--Select--</option>
    <option value="1" <?php echo ($rowsID->renewal_status == '1') ? 'selected="selected"' : ''; ?>>Recurring</option>
    <option value="2" <?php echo ($rowsID->renewal_status == '2') ? 'selected="selected"' : ''; ?>>Refused</option>
  </select>
</td>
...
?>
				  ...
				  <td><input type="text"  autocomplete="off"  name="dailypickup[<?php echo $k;?>][serial_number]" id="sr_no<?php echo $k;?>" placeholder="Enter Serial No" onkeyup="qtycalc_rows(this.id,<?php echo $k;?>)" class="form-control"  ?> </td>
...
<td class="renewal_date_col_<?php echo $k;?>" style="display:none;">
  <input type="text" autocomplete="off" 
    name="dailypickup[<?php echo $k;?>][renewal_date]" 
    id="renewal_date_item<?php echo $k;?>" 
    placeholder="DD-MM-YYYY" 
    class="form-control pickerdate_addreport" value="" />
</td>
<td class="renewal_status_col_<?php echo $k;?>" style="display:none;">
  <select name="dailypickup[<?php echo $k;?>][renewal_status]" 
    id="renewal_status_item<?php echo $k;?>" 
    class="form-control select2">
    <option value="">--Select--</option>
    ...
    <option value="2">Refused</option>
          ...
          <div class="box-footer">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
            <input type='button' value='Cancel' class="btn btn-default" onclick='location.replace("<?php echo htmlspecialchars(basename($_REQUEST['return_file'] ?? 'ManagerDailyPickup.php')); ?>");'>
          </div>
        </div>
      </form>
    </div>
...
<div id="companyby" class="well" style="width:50%;">
  ...
  ?>
    ...
    <div id="vendorby" class="well" style="width:50%; display:none;">
    <h3>Add Vendor</h3>
    <form id="vendorbypopupform" data-parsley-validate autocomplete="off" method="post">
        <input type="hidden" id="EditVendorID" name="EditVendorID" value="">
        
        <div class="form-group col-sm-6">
            <label>Name <font color="#FF0000">*</font></label>
            <input type="text" class="form-control" placeholder="Enter Vendor Name" id="vendor_name" name="vendor_name" data-parsley-required>
        </div>
        
        <div class="form-group col-sm-6">
            <label>Mobile <font color="#FF0000">*</font></label>
            <input type="text" class="form-control" placeholder="Enter Mobile" id="vendor_mobile" name="vendor_mobile" data-parsley-type="digits" data-parsley-length="[10,10]" data-parsley-required>
        </div>
        
        <div class="form-group col-sm-6">
            <label>Email</label>
            <input type="email" class="form-control" placeholder="Enter Email" id="vendor_email" name="vendor_email" data-parsley-type="email">
        </div>
        
        <div class="form-group col-sm-6">
            <label>Type <font color="#FF0000">*</font></label>
            <select class="form-control" id="vendor_type" name="vendor_type" data-parsley-required>
                <option value="">Select Type</option>
                <option value="1">Hard Vendor</option>
                <option value="2">Tally Partner</option>
        ...
        <div class="form-group col-sm-12">
			<label>Company Name</label>
			<input type="text" class="form-control" placeholder="Enter Company Name" id="vendor_company" name="vendor_company">
		</div>
        
        <div class="form-group col-sm-12" align="left">
            <input type="button" class="btn btn-primary" onclick="saveVendorPopupform();" value="Save">
            <button class="vendorby_close btn btn-default">Close</button>
        </div>
    </form>
</div>
  ...
  <form id="bookedbypopupform" data-parsley-validate autocomplete="off" method="post" >
    ...
    <div class="form-group col-sm-4">
      <label for="email" >Email Id <font color="#FF0000">*</font></label>
      <input type="email" name="email" id="email" class="form-control" placeholder="Enter Email Id" data-parsley-type="email" automcomplete="off" data-parsley-required required>
    </div>
    <div class="form-group col-sm-4">
...
$(document).on('select2:select', '.select2-results__option', function() {
    $(this).closest('.select2-container').prev('select').select2('close');
		...
		cols1 += '<td><input  type="text"  autocomplete="off" placeholder="Enter Serial Number" class="form-control" value=""   name="dailypickup[' + counter1 + '][serial_number]" id="sr_no' + counter1 + '" required onkeyup="qtycalc_rows(this.id,' + counter1 + ')" /></td>';
...
cols1 += '<td><input  type="text"  autocomplete="off" placeholder="0" class="form-control"  value="0"   name="dailypickup[' + counter1 + '][other_expenses]" id="other_expenses' + counter1 + '" required onkeyup="qtycalc_rows(this.id,' + counter1 + ')"/></td>';  
 	  ...
 	  cols1 += '<td class="renewal_date_col_' + counter1 + '" style="display:none;"><input type="text" autocomplete="off" placeholder="DD-MM-YYYY" class="form-control pickerdate_addreport" value="" name="dailypickup[' + counter1 + '][renewal_date]" id="renewal_date_item' + counter1 + '" /></td>';

cols1 += '<td class="renewal_status_col_' + counter1 + '" style="display:none;"><select name="dailypickup[' + counter1 + '][renewal_status]" id="renewal_status_item' + counter1 + '" class="form-control select2"><option value="">--Select--</option><option value="1">Recurring</option><option value="2">Refused</option></select></td>';
		 ...
		 $.ajax({
                ...
                document.getElementById("cost" + mydata.clicked_id).type = "hidden";
				 ...
				 document.getElementById("orginal_cost_active"+mydata.clicked_id).value = '0';
						...
						// Show/hide renewal date and status based on serial_number_applicable
            if (mydata.serial_number_applicable == '1') {
                $('.renewal_date_col_' + mydata.clicked_id).show();
                $('.renewal_status_col_' + mydata.clicked_id).show();
            } else {
                $('.renewal_date_col_' + mydata.clicked_id).hide();
                $('.renewal_status_col_' + mydata.clicked_id).hide();
                // Clear values when hidden
                $('#renewal_date_item' + mydata.clicked_id).val('');
                $('#renewal_status_item' + mydata.clicked_id).val('');
            }
...
$(document).on('click', '.vendorby_open', function(){
    var vendorId = $('#id_vendor').val();
    
    if(vendorId && vendorId != '0'){
        // Pencil mode — load existing vendor data
        $.ajax({
            type: 'POST',
            url: 'ajax/ajaxGetVendorData.php',
            data: { id: vendorId },
            dataType: 'json',
            success: function(data){
                $('#EditVendorID').val(data.id);
                $('#vendor_name').val(data.name);
                $('#vendor_mobile').val(data.mobile);
                $('#vendor_email').val(data.email);
                $('#vendor_type').val(data.vendor_type);
                $('#vendor_company').val(data.company_name);
                $('#vendorby').popup('show');
            }
        });
    } else {
        // Plus mode — clear form and open fresh
        $('#EditVendorID').val('');
        $('#vendorbypopupform')[0].reset();
        $('#vendorby').popup('show');
    }
});

$(document).on('click', '.vendorby_close', function(){
    $('#vendorby').popup('hide');
    $('#vendorbypopupform')[0].reset();
});
...
$('.vendorName').select2({
	...
	allowClear: true,
    ajax: {
        url: "ajax/ajaxSearchVendorName.php",
        dataType: 'json',
        delay: 50,
        processResults: function(data) {
            return { results: data };
	...
	// Toggle vendor button icon based on selection
$('#id_vendor').on('change', function(){
    var val = $(this).val();
    if(val && val != '0'){
        $('#vendorby_icon').removeClass('fa-plus').addClass('fa-pencil');
    } else {
        $('#vendorby_icon').removeClass('fa-pencil').addClass('fa-plus');
    }
});

// On page load, set icon if vendor already selected (edit mode)
$(document).ready(function(){
    if($('#id_vendor').val()){
        $('#vendorby_icon').removeClass('fa-plus').addClass('fa-pencil');
    }
...
// Save vendor popup form
function saveVendorPopupform() {
    var form = $("#vendorbypopupform");
    if (form.parsley().validate()) {
        $.ajax({
            type: "POST",
            url: 'ajax/ajaxSaveVendor.php',
            data: form.serialize(),
            success: function(result) {
                var data = JSON.parse(result);
                if (data.status == 1) {
                    var editId = $('#EditVendorID').val();
                    if(editId){
                        // Update existing option text
                        $('#id_vendor option:selected').text(data.name);
                    } else {
                        // Add new vendor to dropdown and select it
                        var newOption = new Option(data.name, data.id, true, true);
                        $('#id_vendor').append(newOption).trigger('change');
                    }
                    $('#vendorby').hide();
                    $('#vendorbypopupform')[0].reset();
                } else {
