...
//echo $date.'===='.$end;
	...
	$sql .=" SELECT  a.id_company,a.id_default_group,a.name,b.name AS area,b.id as id_area,c.id as id_user,c.name AS executive,c.id AS id_executive  FROM `".TBL_COMPANY."` AS a
			...
			WHERE a.`id_shop` = '".addslashes($id_shop)."' AND a.status=1
...
$objPHPExcel->getActiveSheet(1)->getColumnDimension('M')->setWidth(7);
	...
	function conveyanceReport($cron,$id_shop,$idUser,$report_date,$approval_status){
       ...
       $design = selectColumn(TBL_USERS,'designation','WHERE id="'.$idUser.'" ');
       $design_name = selectColumn(TBL_DESIGNATION_MASTER,'name','WHERE id IN ('.$design.')');

      $grade = selectColumn(TBL_USERS,'grade','WHERE id="'.$idUser.'" ');
       //$grade_name = selectColumn(TBL_GRADE_MASTER,'name','WHERE id IN ('.$grade.')');
		...
		$content.="	<div style='border:1px solid black;'><table  style='width:100%;margin:0px 0px 0px 0px;padding:0px 0px 0px 0px;' cellspacing='0'>
						...
						<tr>
							...
							<td colspan='3'><b>Designation <span style='margin-left:15px;'>:</span> </b></td>

							<td colspan='5' style='text-align:left;'><b>".ucwords($design_name)."</b></td>
						...
						<tr>
							...
							<td colspan='2' style='text-align:left;'><b>".ucwords($team)."</b></td>";
					          if($grade!=''){
					      	 $grade_name = selectColumn(TBL_GRADE_MASTER,'name','WHERE id IN ('.$grade.')');
					       $content.="<td colspan='3'><b>Grade <span style='margin-left:15px;'>:</span> </b></td>
												
      	                     <td colspan='5' style='text-align:left;'><b>".ucwords($grade_name)."</b></td>";
                                 } 
						...
						<tr>
							...
							<td colspan='3'><b>Location <span style='margin-left:15px;'>:</span> </b></td>

							<td colspan='2' style='text-align:left;'><b>".ucwords(selectColumn(TBL_USERS,'city','WHERE id="'.$idUser.'" '))."</b></td>
...
$report_date1= explode(" to ",$report_date);
	...
	$checkout = $report_date1['1'];
		...
		exit;
			...
			file_put_contents('/var/www/vhosts/roomstatushub.in/httpdocs/sync/adminpanel/mailattach/'.$Filename.'.pdf', $gen);
			...
			//$companyselect2==1;
			 //$companyselect3==1;
				...
				//	$budget = selectColumn(TBL_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."'   ");  Commented ON 13-02-2021
					...
					$budget = selectColumn(TBL_UNIT_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."' AND id_user='".$id_user."'  "); 
					...
					$achieved =selectColumn(TBL_UNIT_AGENT_ACHIEVED,'sum(qty)'," WHERE  id_shop='".$id_shop."' and id_company='".$row->id_company."' AND id_user='".$id_user."'  "); 
...
//Removed uSER ID 
//$budget = selectColumn(TBL_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."' AND `from`= '".$date."' AND `to`= '".date('Y-03-31',strtotime($end))."' AND id_user='".$id_user."' ");
                $budget = selectColumn(TBL_UNIT_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."' AND `from`= '".$date."' AND `to`= '".date('Y-03-31',strtotime($end))."'  ");
...
$sqlPIckupBase = " SELECT 
		...
		where `".TBL_UNIT_AGENT_ACHIEVED."`.`id_shop` = '".$id_shop."'";
		$resultPIckupBase = mysqli_query($conn,$sqlPIckupBase);
