...
error_reporting(E_ALL);
	...
	function loginLogoutReport($id_shop, $startDate, $endDate, $id_executive, $cron=false){
	    ...
	    $sql = "SELECT DATE(created_at) AS date, (CASE WHEN type=1 THEN TIME_FORMAT(MIN(created_at), '%h:%i %p')  ELSE 'Not logged in' END ) AS login,(CASE WHEN  type=2 THEN TIME_FORMAT(MAX(created_at), '%h:%i %p')  ELSE 'Not logged out' END ) AS logout
		...
		*/
		...
		if($numRows > 0){
			...
			while($row = mysqli_fetch_object($res)){
				...
				$budget = selectColumn(TBL_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."' AND `from`= '".$date."' AND `to`= '".$end."'");
		...
		*/
		...
		if($numRows > 0){
			...
			while($row = mysqli_fetch_object($res)){
				...
				$achieved =selectColumn(TBL_UNIT_AGENT_ACHIEVED,'sum(qty)'," WHERE month between '".$date."' and '".$end."' and id_shop='".$id_shop."' and id_company='".$row->id_company."'  ");
				$budget = selectColumn(TBL_UNIT_AGENT_BUDGET,'room_nights'," WHERE `id_company` = '".$row->id_company."'  ");
				$prevAch2 = selectColumn(TBL_UNIT_AGENT_ACHIEVED,'sum(qty)'," WHERE month between '".$prevYear2."' and '".$prevYear2End."' and id_shop='".$id_shop."' and id_company='".$row->id_company."'  ");
				$prevAch = selectColumn(TBL_UNIT_AGENT_ACHIEVED,'sum(qty)'," WHERE month between '".$prevYear."' and '".$prevYearEnd."' and id_shop='".$id_shop."' and id_company='".$row->id_company."'  ");
