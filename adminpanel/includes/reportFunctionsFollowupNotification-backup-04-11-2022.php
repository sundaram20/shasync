...
while($row = mysqli_fetch_object($res)){
			...
			$summary	=	$row->summary;
				...
				if($row->lead_status==1){
					...
					<td style="border:1px solid black;">'.selectColumn(TBL_HOTELS,'CONCAT(name,",",city)','WHERE id="'.$row->hotel_id.'" ').'</td>
					...
					<td style="border:1px solid black;">'.date('d-M-Y',strtotime($row->dated)).'</td>
