...
$SheetUniq = $objPHPExcel->getActiveSheet();
 $SheetUniq->setTitle('Hotel Wise Contract Report');
$SheetUniq->getStyle('A7:N7')->applyFromArray( $style_header );
$SheetUniq->getStyle('A8:N8')->applyFromArray( $style_header );
		$SheetUniq->getStyle('A7:N7')->getFont()->setBold(true)
	                                ->setName('Calibri')
	                                ->setSize(14)
	                                ->getColor()->setRGB('ffffff');
		$SheetUniq->getStyle('A8:N8')->getFont()->setBold(true)
	                                ->setName('Calibri')
	                                ->setSize(14)
	                                ->getColor()->setRGB('ffffff');							
		$cond='';
		...
		if(file_exists($signature)){
			...
			$objDrawing->setWorksheet($SheetUniq);
		}
		...
		if(!isset($cron)){
			...
			WHERE 1=1 AND `".TBL_RATE."`.status=1 ".$cond."   order by ".TBL_RATE.".seasonId ,cmp.name,".TBL_ASSIGN_HOTEL_ROOM.".display_order asc LIMIT 0,10";
...
//echo '=='.$rateSql;
//die;
//AND ".TBL_RATE.".company_id IN ('33335') 
...
$reportDataArray['Hotel Contract'][$Company]['company_id']=$RatecheckCountRecords->company_id;
$reportDataArray['Hotel Contract'][$Company]['company Name']=$Company;
$reportDataArray['Hotel Contract'][$Company]['Market Name']=$Market;
$reportDataArray['Hotel Contract'][$Company]['season Name']=$seasonName;
$reportDataArray['Hotel Contract'][$Company]['credTxt']=$credTxt;
$reportDataArray['Hotel Contract'][$Company]['hotel_remarks']=$RatecheckCountRecords->hotel_remarks;
$reportDataArray['Hotel Contract'][$Company]['sales_person']=ucwords($userName);
$linkid=$RatecheckCountRecords->room_id.'_'.$RatecheckCountRecords->rate_plan_id;
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['room_type']=selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = '".$RatecheckCountRecords->room_id."' ");
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['plan']=selectColumn(TBL_RATE_PLAN,'name'," WHERE `id` = '".$RatecheckCountRecords->rate_plan_id."'");
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['single']=$singlePriceTxt;
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['double']=$doublePrixeTxt;
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['extra_bed']=($RatecheckCountRecords->extra_bed_price!=0?$RatecheckCountRecords->extra_bed_price:'Rate on Request');
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['lunch']=($RatecheckCountRecords->lunch_price!=0?$RatecheckCountRecords->lunch_price:'Rate on Request');
$reportDataArray['Hotel Contract'][$Company]['rooms'][$linkid]['dinner']=($RatecheckCountRecords->dinner_price!=0?$RatecheckCountRecords->dinner_price:'Rate on Request');
...
$SheetUniq->getStyle('A'.$con.':N'.$con)->getFont()->setBold(true);	
					...
					$SheetUniq->getRowDimension(20)->setRowHeight(-1); 
			$SheetUniq->getStyle('A'.$con.':N'.$con)->getFont()->setBold(true);	
			
				//$con++;	
foreach($reportDataArray as $maintitle=>$mainDatalist){$con++;
	foreach($mainDatalist as $subDataList1){
	//	cellColor('A'.$con.':N'.$con,'c6c6c6');
				$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue('A'.$con, $InCount++)
						->setCellValue('B'.$con, $subDataList1['company Name'])
						->setCellValue('C'.$con, $subDataList1['Market Name'])
						->setCellValue('D'.$con, $subDataList1['season Name'])
						->setCellValue('L'.$con,$subDataList1['credTxt'])
						->setCellValue('N'.$con,$subDataList1['hotel_remarks'])
						->setCellValue('M'.$con,$subDataList1['sales_person']);
			
					foreach($subDataList1 as  $pp=>$subDataList5){
						
						foreach($subDataList5 as  $pp5=>$subDataList){
							
							
					$objPHPExcel->setActiveSheetIndex(0)
					
					
					->setCellValue('E'.$con, $subDataList['room_type'])
					->setCellValue('F'.$con, $subDataList['plan'])
					->setCellValue('G'.$con,$subDataList['single'])
					->setCellValue('H'.$con,$subDataList['double'])
					->setCellValue('I'.$con, $subDataList['extra_bed'])
					->setCellValue('J'.$con, $subDataList['lunch'])
					->setCellValue('K'.$con, $subDataList['dinner']);
						//$SheetUniq->getStyle('B'.$con.':N'.$con)->getAlignment()->setWrapText(true);             
						//$SheetUniq->getStyle('E')->getAlignment()->setWrapText(true); 
					
				
					/*$SheetUniq->getStyle('A'.$con.':N'.$con)->getAlignment()->applyFromArray(
...
//===========Rate UNIT END===================================================================>	
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$con, 'Note : ')
		                                          ->setCellValue('B'.$con, 'Season Description')
		                                          ->setCellValue('C'.$con, 'From')
		                                          ->setCellValue('D'.$con, 'To');
		
		cellColor('A'.$boldCon.':D'.$con,'c6c6c6');
		$con++;
		if($seasonIdCron=='65,66'){
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$con, 'SUMMER 2019')
			                                          ->setCellValue('C'.$con, '01-Apr-2019')
			                                          ->setCellValue('D'.$con++, '30-Sep-2019')
			                                          ->setCellValue('B'.$con, 'WINTER 2019')
			                                          ->setCellValue('C'.$con, '01-Oct-2019')
			                                          ->setCellValue('D'.$con++, '31-Mar-2020');
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->getFont()->setBold(true);
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->applyFromArray($styleThinBlackBorderOutline);
			...
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$con, 'SUMMER 2019')
			                                          ...
			                                          ->setCellValue('C'.$con, '01-Oct-2019')
			                                          ->setCellValue('D'.$con++, '31-Mar-2020')
			                                          ->setCellValue('B'.$con, 'YEAR 2019')
			                                          ->setCellValue('C'.$con, '01-Jan-2019')
			                                          ->setCellValue('D'.$con++, '31-Dec-2020');
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->getFont()->setBold(true);
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->applyFromArray($styleThinBlackBorderOutline);
			...
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$con,$seasonDeatilsArr[0])
			                                          ->setCellValue('C'.$con,date('d-M-Y',strtotime($seasonDeatilsArr[1])))
			                                          ->setCellValue('D'.$con++,date('d-M-Y',strtotime($seasonDeatilsArr[2])));
		}
		else{
			...
			while($rowSea = $db->fetch_object2($resSea)){
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$con, $rowSea->name)
				                                          ->setCellValue('C'.$con, date('d-M-Y',strtotime($rowSea->start_date)))
				                                          ->setCellValue('D'.$con, date('d-M-Y',strtotime($rowSea->end_date)));
				$con++;
			}
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->getFont()->setBold(true);	
			//$SheetUniq->getStyle('A'.$boldCon.':D'.($con-1))->applyFromArray($styleThinBlackBorderOutline);
		...
		//} //Hotel Sql End
		$col2	=	$con-1;	
		$SheetUniq->getStyle('A7:N9')->applyFromArray($styleThinBlackBorderlatest);
		/*$SheetUniq->getStyle('A8:N8')->applyFromArray($styleThinBlackBorderlatest);
		$SheetUniq->getStyle('A9:N9')->applyFromArray($styleThinBlackBorderlatest);*/
		
		$SheetUniq->getStyle('A7:N9')->getAlignment()->applyFromArray(
			array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
		);
		
		/*$SheetUniq->getStyle('C'.$setcellcount.':K'.$col2)->getAlignment()->applyFromArray(
			array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
		);
		
		$SheetUniq->getStyle('I'.$setcellcount.':I'.$col2)->getAlignment()->applyFromArray(
			array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
		);
		$SheetUniq->getStyle('J'.$setcellcount.':J'.$col2)->getAlignment()->applyFromArray(
			array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
		);
		$SheetUniq->getStyle('K'.$setcellcount.':K'.$col2)->getAlignment()->applyFromArray(
			array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
		);*/
        ...
        $SheetUniq->getProtection()->setPassword('FreeBlocking');
        $SheetUniq->getProtection()->setSheet(true);
        // This should be enabled in order to enable any of the following!
        $SheetUniq->getProtection()->setSort(true);
        $SheetUniq->getProtection()->setInsertRows(true);*/			
		
		//Protecting File End
		/*$SheetUniq->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT);
		$SheetUniq->getPageSetup()->setPaperSize(PHPExcel_Worksheet_PageSetup::PAPERSIZE_A4);
		$SheetUniq->getPageSetup()->setFitToPage(true);
		$SheetUniq->getPageSetup()->setFitToWidth(1);
		$SheetUniq->getPageSetup()->setFitToHeight(0);
