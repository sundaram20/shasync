...
if(isset($_REQUEST['renewal_check']) && $_REQUEST['renewal_check'] == 1 && !empty($_REQUEST['renewal_date'])){
    ...
    $sql .= " AND dp.id_renewal_required = '1' 
              AND DATE(dp.renewal_date) >= '".date('Y-m-d', strtotime($from_renewal))."'
              AND DATE(dp.renewal_date) <= '".date('Y-m-d', strtotime($to_renewal))."' ";
}
...
$resw = mysqli_query($connNew,$sqlQuery);
	...
	$roww = mysqli_fetch_object($resw);	
		...
		$TotalValue	=	 round($roww->sales_revenueTotal + $Tax);
					  ...
					  $item_name =  selectColumn(TBL_HOTELS,'name'," WHERE `id` = '".$roww->id_product."'");
			$first_name =  selectColumn(TBL_CUSTOMER,'first_name'," WHERE `id_customer` = '".$row->id_contacts."'");
			$last_name = selectColumn(TBL_CUSTOMER,'last_name'," WHERE `id_customer` = '".$row->id_contacts."'");  
...
$last_remark = $rowRemark['support_remark'] ?? '';
                  ...
                  <tr>
                    ...
                    <td><?=$row->bill_no;?></td>
					  ...
					  <td>
<?php 
if($row->id_renewal_required == 1 && !empty($row->renewal_date)){
