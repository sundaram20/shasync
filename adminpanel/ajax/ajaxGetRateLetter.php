...
$resCatUnit = executeSql("SELECT `".TBL_RATE_UNIT."`.*, `".TBL_RATE_LEVEL."`.name as level_name ,`".TBL_RATE_MARKET."`.name as market_name from `".TBL_RATE_UNIT."` LEFT JOIN `".TBL_RATE_LEVEL."` ON `".TBL_RATE_UNIT."`.rate_level_id=`".TBL_RATE_LEVEL."`.id   LEFT JOIN `".TBL_RATE_MARKET."` ON `".TBL_RATE_UNIT."`.market=`".TBL_RATE_MARKET."`.id  LEFT JOIN `fs_rate_details_unit` ON `fs_rate_unit`.id=`fs_rate_details_unit`.rate_id
	...
	where    `fs_rate_details_unit`.hotel_id='".addslashes($_REQUEST['hotel_id'])."' AND  `".TBL_RATE_UNIT."`.id_shop='".addslashes($_SESSION['shop'])."'  AND `".TBL_RATE_UNIT."`.status='1' and (`".TBL_RATE_UNIT."`.company_id='".$id_company."' && `".TBL_RATE_UNIT."`.company_id!='0' ) and (( `".TBL_RATE_UNIT."`.start_date <=  '".date('Y-m-d',strtotime($checkin_date))."' and  `".TBL_RATE_UNIT."`.end_date >= '".date('Y-m-d',strtotime($checkout_date))."') OR (  `".TBL_RATE_UNIT."`.start_date between '".date('Y-m-d',strtotime($checkin_date))."' and '".date('Y-m-d',strtotime($checkout_date))."') OR (  `".TBL_RATE_UNIT."`.end_date between '".date('Y-m-d',strtotime($checkin_date))."' and '".date('Y-m-d',strtotime($checkout_date))."'))  group by `".TBL_RATE_UNIT."`.rate_name" );	
