<?php
	...
	if($_REQUEST['flag']==1){
		...
		$sql = "SELECT DISTINCT 1 AS 'id',A.id_user AS id_user,A.dated,'Sales Report' AS 'DOC_TYPE',A.conveyance_approved,supervisor_remarks FROM ".TBL_DAILYVISIT." A  
			...
			WHERE A.id_shop='".$_SESSION['shop']."' AND (A.conveyance_approved=0 || A.conveyance_approved=2) AND Total !=0 ".$teamMembers."
		...
		if($res){
			...
			while($row = mysqli_fetch_object($res)){
				...
				$content .='<tr style="border 1px solid;">
								<td style="padding:10px;">'.date('d-M-Y',strtotime($row->dated)).'</td>
