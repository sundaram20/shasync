...
$support_id = intval($_GET['id'] ?? 0);


if ($support_id <= 0) {
    echo '<div>Error: Invalid Pickup ID '.$support_id.' </div>';
    exit;
}

//$sql = "SELECT extra_data FROM `call_details` WHERE call_id = '$callId' ORDER BY id desc LIMIT 1";

$sql = "SELECT id_company, id_contacts, serial, id_product FROM support WHERE id = '".$support_id."' AND id_shop='".$_SESSION['shop']."'";
...
if (!$details) {
    echo '<div>Error: No data found for this Support</div>';
    exit;
}

$supportDetailSql = "SELECT id_user, date_created, assign_user_id, internal_remark, support_remark, support_status, followup_date FROM support_details WHERE id_support = '$support_id'";
...
<input readonly type="text" name="contact_person_display" value="<?php echo htmlspecialchars($row->first_name . ' ' . $row->last_name . ' | ' . $row->mobile); ?>" class="form-control" />
                    ...
                    <div class="form-group">
                      ...
                      <input readonly type="text" name="serial_number" id="serial_number" value="<?php echo htmlspecialchars($details['serial']); ?>" class="form-control" />
                    </div>
                  </div>
...
<!--<div class="col-md-12">
  <div class="form-group">
	...
	<button class='btn btn-success btn-sm' onclick='addSupportFollowup1(<?php echo $support_id?>)'>Assign Support</button>
