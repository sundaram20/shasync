...
if($_REQUEST['id']=='' && $_REQUEST['Save']=='Add' ){
	$chkSql = "SELECT * FROM ".TBL_UNIT_AGENT_ACHIEVED." WHERE id_user='".$_REQUEST['hotelId']."' AND id_hotel='".$_REQUEST['id_hotel']."' AND seasonId='".$_REQUEST['seasonId']."' AND id_shop='".$_SESSION['shop']."' ";
		...
		if(($_POST['Save'] == 'Add') && empty($_REQUEST['id'])){//add
			...
			echo '<p class="help-block">User Budget has been updated sucessfully.</p>';
				...
				foreach($data_id as $data =>$value){
					...
					$EditId	=selectColumn(TBL_UNIT_AGENT_ACHIEVED,'id',"WHERE `id_company`='".addslashes($value)."' AND  seasonId = '".addslashes($_POST['seasonId'])."' AND id_hotel='".$_REQUEST['id_hotel']."' ");
					...
					if($EditId>0){
						...
						$addRate .= "	
							,`last_modified` = '".currenDateTime()."'
							,`last_modified_by` = '".$_SESSION['userId']."'
							,`status` = '1' WHERE `id_company`='".addslashes($value)."'  AND seasonId = '".addslashes($_POST['seasonId'])."'
							AND month='".$DateValue."' AND id_hotel='".$_REQUEST['id_hotel']."'";	
