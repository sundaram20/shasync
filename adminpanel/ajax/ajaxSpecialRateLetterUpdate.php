...
$specailArrayLength= count($_POST['special_room_type_id']);
...
if($_POST['Save']){		
	...
	$inclusionDetail = json_encode(array_combine($_POST['inclusion_id'],$_POST['inclusion_detail']));
		...
		if(($_POST['Save'] == 'Add') && empty($_POST['id'])){//add
			...
			$chkSql = "SELECT * FROM `".TBL_RATE."` as a join `".TBL_RATE_DETAILS_SPECIAL."` as b LEFT JOIN 

			".TBL_COMPANY." c ON a.company_id=c.id_company  where UPPER(c.name)='".strtoupper($companyName)."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and (a.seasonId='".addslashes($_POST['seasonId'])."' ".$seaCond." ) and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."' AND a.status=1  AND a.id=b.rate_id AND b.rate_plan_id='".addslashes($_POST['rate_plan_id'][$i])."'";



		$chkSqlUnit = "SELECT DISTINCT a.id,a.rate_name,b.hotel_id FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b LEFT JOIN 

			".TBL_COMPANY." c ON a.company_id=c.id_company  where UPPER(c.name)='".strtoupper($companyName)."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and (a.seasonId='".addslashes($_POST['seasonId'])."' ".$seaCond.") and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."' AND a.status=1  AND a.id=b.rate_id ";	
			...
			while($rowUnit = mysqli_fetch_object($rateNoUnit)){
				...
				$rateNameUnit.=$rowUnit->rate_name.'-'.selectColumn(TBL_HOTELS,'CONCAT(name,", ",city)','WHERE id="'.$rowUnit->hotel_id.'" ').'<br>';
			...
			'.($rateNameUnit!=""?"-----Rate Letter By Unit -----<br>".$rateNameUnit."<br><button class='btn btn-primary' onclick='submitRateLetterForm();'>Countinue</button>":"").'
			...
			elseif(num_rows($checkExisting)> 0){
				...
				-----Rate Letter By Unit -------<br>".$rateNameUnit."<br><button class='btn btn-primary' onclick='location.replace(\"manageRateLetters.php\")'>Ok, Redirect me to rate letters lists</button>":"").'
...
if(num_rows($checkRatePlanExisting)>0){
	...
	$rowRatePlanExisting = $db->fetch_object2($checkRatePlanExisting);
	 ...
	 $rate_id	=	$rowRatePlanExisting->id;
		...
		for($y=1;$y<=$specailArrayLength;$y++){
	
			foreach($_POST['special_room_type_id'][$y] as $Datalist =>$value){

								$status = isset($_POST['status'][$data]) ? 1 : 0;

								$start_date	  = $_POST['special_date_start'][$y];
								$end_date		= $_POST['special_date_end'][$y];

								$addRateDetail   = "INSERT INTO `".TBL_RATE_DETAILS_SPECIAL."` SET 

													`rate_id` = '".addslashes($rate_id)."',
													
													`special_group` = '".$y."',
													`special_rate_name` = '".addslashes($_POST['special_rate_name'][$y])."',
													`rate_assign_id` = '".addslashes($rateAssignId)."',

													`room_id` = '".addslashes($_POST['special_room_type_id'][$y][$Datalist])."',

													`rate_plan_id` = '".addslashes($_POST['special_rate_plan_id'][$y][$Datalist])."',
								...
								if($_POST['discount_type'.$value]==2){
								 ...
								 $addRateDetail .=	",`room_price` = '".addslashes($_POST['special_room_price'][$y][$Datalist])."',

													`extra_bed_price` = '".addslashes($_POST['special_extra_bed'][$y][$Datalist])."',

													

													`hotel_remarks`='".addslashes($_POST['special_hotel_remarks'][$y][0])."',

													`single_pax_price` = '".addslashes($_POST['special_single_pax_price'][$y][$Datalist])."',

													`double_pax_price` = '".addslashes($_POST['special_double_pax_price'][$y][$Datalist])."',	

													`weekend_single_pax_price` = '".addslashes($_POST['special_weekend_single_pax_price'][$y][$Datalist])."',

													`weekend_double_pax_price` = '".addslashes($_POST['special_weekend_double_pax_price'][$y][$Datalist])."',												

													`breakfast_price` = '".addslashes($_POST['special_breakfast_price'][$y][$Datalist])."',

													`lunch_price` = '".addslashes($_POST['special_lunch_price'][$y][$Datalist])."',

													`dinner_price` = '".addslashes($_POST['special_dinner_price'][$y][$Datalist])."',

													`tax_room` = '".addslashes($_POST['special_tax_room'][$y][$Datalist])."',							
...
//`rate_level_id` = '".addslashes($_POST['new_rate_level_id'])."',						
			...
			$lastRecordRes = executeSql("Select sub_code as maxId from `".TBL_RATE."` where `id` = '".addslashes(encryptor('decrypt',$_POST['id']))."'");
...
/*$checkExisting = executeSql("SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where rate_id='".addslashes(encryptor('decrypt',$_POST['id']))."' and hotel_id='".addslashes($_POST['hotelId'])."'");
...
*/
		...
		for($y=1;$y<=$specailArrayLength;$y++){
			...
			foreach($_POST['special_room_type_id'][$y] as $Datalist =>$value){
				...
				$status 		  = isset($_POST['special_detail_status'][$y][$Datalist]) ? 1 : 0;
						
				$start_date	  = $_POST['special_date_start'][$y];
			
				$end_date		= $_POST['special_date_end'][$y];
			
				//$_POST['special_room_type_id'][$y][$Datalist]
			
			

				

				$checkExistingRateDetailRoomWise = executeSql("SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where id='".$_POST['edit_id'][$y][$Datalist]."' ");
...
//echo "SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where id!='".$_POST['data_id'][$data]."'  AND `room_id` = '".addslashes($_POST['room_type_id'][$data])."' AND  `rate_plan_id` = '".addslashes($_POST['rate_plan_id'][$data])."' and hotel_id='".addslashes($_POST['hotelId'])."' ";
...
$checkExistingRateDetailRoomWise1 = executeSql("SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where id='".$_POST['edit_id'][$y][$Datalist]."'  AND  hotel_id='".addslashes($_POST['hotelId'])."' and rate_id='".$rate_id."' and special_group='".$_POST['special_group'][$y][$Datalist]."' ");                                 



$people = $_REQUEST['special_detail_status'];
...
//echo '==============='.$_POST['special_hotel_remarks'][$y][0];
													
								$start_date	  = $_POST['special_date_start'][$y];
								$end_date		= $_POST['special_date_end'][$y];
								$editRateDetail = "UPDATE `".TBL_RATE_DETAILS_SPECIAL."` SET ";
								$editRateDetail   .= "
							...
							/*	if($_POST['discount_type'.$value]==2){
								 ...
								 $editRateDetail .=	",`discount` = '".addslashes($_POST['discountPercent'.$value])."'";	

									}else{					

								 $editRateDetail .=	",`discount` = '".addslashes($_POST['discountFlat'.$value])."'";			
...
*/
								 $editRateDetail .=	",
						   		  ...
						   		  $editRateDetail .= ",`detail_status` = '".$status."'

													 WHERE `id` = '".$_POST['edit_id'][$y][$Datalist]."' and 
...
//echo '<br/><br/><br/><br/>'.$editRateDetail;
			...
			//	echo "===SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where `room_id` = '".addslashes($_POST['special_room_type_id'][$y][$Datalist])."' AND  `rate_plan_id` = '".addslashes($_POST['special_rate_plan_id'][$y][$Datalist])."' and hotel_id='".addslashes($_POST['hotelId'])."' and rate_id='".$rate_id."' and special_group='".$_POST['special_group'][$y][$Datalist]."' ";

				$checkExistingRateDetailRoomWise1 = executeSql("SELECT * FROM `".TBL_RATE_DETAILS_SPECIAL."` where `room_id` = '".addslashes($_POST['special_room_type_id'][$y][$Datalist])."' AND  `rate_plan_id` = '".addslashes($_POST['special_rate_plan_id'][$y][$Datalist])."' and hotel_id='".addslashes($_POST['hotelId'])."' and rate_id='".$rate_id."' and special_group='".$_POST['special_group'][$y][$Datalist]."' ");
...
if(num_rows($checkExistingRateDetailRoomWise1) == 0){
	   ...
	   $seasonId=selectColumn(TBL_RATE,'seasonId'," WHERE `id` = '".addslashes(encryptor('decrypt',$_POST['id']))."'");

	   $company_id=selectColumn(TBL_RATE,'company_id'," WHERE `id` = '".addslashes(encryptor('decrypt',$_POST['id']))."'");
