...
$editThis="SELECT a.*,b.* FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b 
	LEFT JOIN ".TBL_ASSIGN_HOTEL_ROOM." as ahr on b.room_id=ahr.room_id AND ahr.hotel_id='".addslashes($hotelId)."'
	where  a.`id` = '".addslashes(encryptor('decrypt',$_REQUEST['id']))."' AND b.hotel_id='".addslashes($hotelId)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id order by ahr.display_order ";	

$editsql = executeSql($editThis);
