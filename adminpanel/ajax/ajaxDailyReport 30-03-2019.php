...
$TotalCountrow 	= $db->fetch_assoc2($resSql);
		...
		(SELECT id,type,dated,lead_status,visit_id,hotel_id,assign_user_id from `fs_follow_up_details` where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and lead_status=1 and  dated='".addslashes($row['dated'])."' $UserAssignId )
		UNION ALL
		(SELECT id,type,dated,lead_status,visit_id,hotel_id,assign_user_id from `fs_feedback_details`where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and lead_status=1 and  dated='".addslashes($row['dated'])."' $UserAssignId )
		UNION ALL
		(SELECT id,type,dated,lead_status,id_quote,hotel_id,assign_user_id from `sales_quote_followup` where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  lead_status=1 and dated='".addslashes($row['dated'])."' $UserAssignId )
		UNION ALL
		(SELECT id,type,dated,lead_status,enquiry_id,hotel_id,assign_user_id from `fs_enquiry_details` where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  lead_status=1 and dated='".addslashes($row['dated'])."' $UserAssignId )" );
		...
		$k='<div class="box-body table-responsive" style="height:200px; overflow-x: hidden !important; overflow: scroll;;">
	...
	if($row123['type'] == '4'){
		...
		$id_company	= selectColumn(TBL_DAILY_ENQUERY,'id_company'," WHERE `id` = '".$row123[$VisiteID]."'");	
	...
	if($row123['type'] == '5'){
		...
		$id_company	= selectColumn(TBL_SALES_QUOTE,'id_company'," WHERE `id` = '".$row123[$VisiteID]."'");	
...
$k .= '<td ><a href="'.$FileName.'?eId='.encryptor('encrypt',$row123[$VisiteID]).'&action=edit&page=1" >'.selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$id_company."'").'</a>
