...
if($_POST['Save']){		
	...
	$inclusionDetail = json_encode(array_combine($_POST['inclusion_id'],$_POST['inclusion_detail']));
		...
		$chkSqlUnit = "SELECT * FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b LEFT JOIN 
			".TBL_COMPANY." c ON a.company_id=c.id_company  where UPPER(c.name)='".strtoupper($companyName)."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and a.seasonId='".addslashes($_POST['seasonId'])."' and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id AND b.rate_plan_id='".addslashes($_POST['rate_plan_id'][$i])."'";	
			...
			$checkExisting = executeSql($chkSql);
				...
				if(num_rows($checkExisting)>0 || mysqli_num_rows($rateNoUnit)){
					$err++;
					echo '<p>!! Duplicate Found !!<br>

					-----Rate Letter By Company -----<br>'.$rateNo->rate_name.' <br>
					-----Rate Letter By Unit -------<br>'.$rateNameUnit.'
