...
$availableData .= '<div class="clearfix" style="font-weight:bold;font-size:15px;">'.selectColumn(TBL_HOTELS,'name','WHERE id="'.$hotel_id.'" ').'-'.selectColumn(TBL_HOTELS,'city','WHERE id="'.$hotel_id.'" ').'</div>
...
$availableData .= ' <div class="form-group col-sm-12"> 
	  <label>Remarks</label> 
	  <div>'.selectColumn(TBL_RATE_DETAILS,'hotel_remarks'," WHERE hotel_id='".$hotel_id."' AND `rate_id` = '".$row->id."'").'</div> 
  </div>';		
$availableData .= '</div>';
...
$availableData .= '	 <td > <div><i class="fa fa-inr"></i> '.$doublePriceTxt.' <button class="pull-left btn btn-success btn-xs" type="button" onclick="ajaxAddRoom('.$row->id.','.$row->rate_assign_id.','.$rowRoom->room_id.','.$rowRoom->rate_plan_id.',1);" ><i class="fa fa-plus-circle"></i></button></div></td>';				  
				  
$availableData .= '				  <td > <div><i class="fa fa-inr"></i> '.$singlePriceTxt.' <button class="pull-left btn btn-success btn-xs" type="button" onclick="ajaxAddRoom('.$row->id.','.$row->rate_assign_id.','.$rowRoom->room_id.','.$rowRoom->rate_plan_id.',0);" ><i class="fa fa-plus-circle"></i></button></div></td>';
