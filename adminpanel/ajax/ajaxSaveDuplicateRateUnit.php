...
$chkSql = "SELECT * FROM `".TBL_RATE."` AS a LEFT JOIN `".TBL_COMPANY."` ON a.company_id=`".TBL_COMPANY."`.id_company where  rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and (seasonId='".addslashes($_POST['seasonId'])."' ".$seaCond.") and market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.`status` = '1' AND UPPER(`".TBL_COMPANY."`.name)='".strtoupper($companyName)."' ";
		
$chkSqlUnit = "SELECT DISTINCT a.id,a.rate_name FROM `".TBL_RATE_UNIT."` as a join `".TBL_RATE_DETAILS_UNIT."` as b LEFT JOIN 
			".TBL_COMPANY." c ON a.company_id=c.id_company  where UPPER(c.name)='".strtoupper($companyName)."' and a.rate_level_id='".addslashes($_POST['new_rate_level_id'])."' and (a.seasonId='".addslashes($_POST['seasonId'])."' ".$seaCond.") and a.market='".addslashes($_POST['market'])."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id=b.rate_id AND b.hotel_id IN ('".$_SESSION['hotel_access']."') ";	 	
