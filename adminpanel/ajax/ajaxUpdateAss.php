<?php include_once("../../config/auto_loader.php");
	...
	if($err == 0){//No error
		...
		if(($_POST['Save'] == 'Add') && empty($_POST['eId'])){//add	
			...
			if(!empty($_SESSION['followup_description'])||empty($_SESSION['followup_description'])){
				foreach($_SESSION['followup_description'] as $dataCode =>$value){
					...
					$followupstat = $_SESSION['followupstatus'][$dataCode];
					$followup_desc = $_SESSION['followup_description'][$dataCode];
				}
			}
			
			
	$addEnq ="  			INSERT INTO `call_details` SET 							
							`id_shop` = '".addslashes($_SESSION['shop'])."',
							
							 `id_user` = '".addslashes($_SESSION['userId'])."',	
							`call_status` = '".$followupstat."',
							`assign_user_id`='".$assign_user."',
							`call_remark` = '".addslashes($_POST['remark'])."',
							`internal_remark` = '".$followup_desc."'
