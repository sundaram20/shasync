...
if($_REQUEST['id']=='' && $_REQUEST['Save']=='Add' ){
	$chkSql = "SELECT * FROM ".TBL_AGENT_BUDGET." WHERE id_user='".$_REQUEST['hotelId']."' AND season_id='".$_REQUEST['seasonId']."' AND id_shop='".$_SESSION['shop']."' ";
...
if(($_REQUEST['Save']=='Add') || ($_REQUEST['Save'] == 'Edit')){		
	$err = 0;		
	//echo '1111';
	if($err == 0){//No error	
		if(($_REQUEST['Save'] == 'Add') && empty($_REQUEST['id'])){//add
				...
				foreach($_REQUEST['buget_qty|'.$value] as $data2 =>$value2){
					...
					if($EditId>0){
						...
						$editRate = " UPDATE `".TBL_BUDGET_MASTER."` SET 
									...
									`seasonId`	= '".addslashes($_REQUEST['seasonId'])."',		
						...
						$addRate = "INSERT INTO `".TBL_AGENT_BUDGET."` SET 
									`id_shop` = '".addslashes($_SESSION['shop'])."',		
									...
									AND id=".addslashes($_REQUEST['seasonId'])." ",' ORDER BY `name`')."',
									`to` = '".selectColumn(TBL_BUDGET_YEAR,'end_date'," where id_shop='".addslashes($_SESSION['shop'])."' AND id=".addslashes($_REQUEST['seasonId'])." ",' ORDER BY `name`')."',
									`season_id`	= '".addslashes($_REQUEST['seasonId'])."',
		...
		else if(($_REQUEST['Save'] == 'Edit') && !empty($_REQUEST['id'])){
			
			foreach($data_id as $data =>$value){

				foreach($_REQUEST['buget_qty|'.$value] as $data2 =>$value2){

					$EditId		=	selectColumn(TBL_AGENT_BUDGET,'id','WHERE id_company="'.$value.'" AND season_id="'.$_REQUEST['seasonId'].'" AND id_user="'.$_REQUEST['hotelId'].'" ');

						if($EditId>0){
							$addRate = "UPDATE `".TBL_AGENT_BUDGET."` SET 
										`room_nights` = '".addslashes($value2)."',
										`value` = '".addslashes($_REQUEST['buget_value|'.$value][$data2])."'";
							$addRate .= "	,`date_created` = '".currenDateTime()."'
												,`last_modified` = '".currenDateTime()."'
												,`last_modified_by` = '".$_SESSION['userId']."'
												,`status` = '1'
												WHERE id_shop=".$_SESSION['shop']." AND id_company=".$value." AND id_user=".$_REQUEST['hotelId']." AND season_id ='".$_REQUEST['seasonId']."' ";
							//exit;					
							executeSql($addRate);	
							$addRate_id= $db->insert_id();	
