...
echo 'hi'; die();
	...
	$id_companysql=" AND `id_company` = '".addslashes($id_company)."'";
	 $SQL	= "
select *  from ".TBL_COMPANY." where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  city !=' ' ".$id_companysql."  order BY name LIMIT 0,25
";
}else{	 
		 
   $SQL	= "
select *  from ".TBL_COMPANY." where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and city !='' AND name LIKE '{$key}%'  order BY name LIMIT 0,25
";
	}
    ...
    while($row=mysqli_fetch_assoc($query))
    {
	  ...
	  $response[] = array('id'=>$row['id_company'], 'text'=>$city);
