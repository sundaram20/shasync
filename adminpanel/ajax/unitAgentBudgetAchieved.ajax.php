...
if($_REQUEST['hotelId']!=''){
	$editRowvalue = executeSql("SELECT * FROM `".TBL_UNIT_AGENT_ACHIEVED."` as a
			LEFT JOIN ".TBL_COMPANY." B ON a.id_company=B.id_company
			LEFT JOIN ".TBL_AREAS." C ON B.area=C.id
			where  a.`id_shop` = '".addslashes($_SESSION['shop'])."' and  a.`month`='".$start_date."' and id_hotel = '".$_REQUEST['id_hotel']."' and FIND_IN_SET('".$_REQUEST['hotelId']."',C.ids_unit_user) ");
}
...
while($rowHotelResult = mysqli_fetch_object($resCat_rooms1)){
	...
	$start_date	=	selectColumn(TBL_BUDGET_YEAR,'start_date'," WHERE `id` = '".$_REQUEST['seasonId']."'");		
	$end_date	=selectColumn(TBL_BUDGET_YEAR,'end_date'," WHERE `id` = '".$_REQUEST['seasonId']."'");								 
	$sqlqu = "SELECT * FROM `".TBL_UNIT_AGENT_ACHIEVED."` as a LEFT JOIN 
				".TBL_COMPANY." B ON a.id_company=B.id_company
				LEFT JOIN ".TBL_AREAS." C ON B.area=C.id
				 where  a.`id_shop` = '".addslashes($_SESSION['shop'])."' and   a.`seasonId`='".$_REQUEST['seasonId']."' and a.`id_company`='".$rowHotelResult->id_company."' and id_hotel='".$_REQUEST['id_hotel']."' and FIND_IN_SET('".$_REQUEST['hotelId']."',C.ids_unit_user) ORDER BY a.month";
...
$availableData .='</tr>';
	...
	while($fromVer <= $tillVer){
		$sqlTotalVer = "SELECT sum(qty)AS qty FROM `".TBL_UNIT_AGENT_ACHIEVED."` as a
			LEFT JOIN ".TBL_COMPANY." B ON a.id_company=B.id_company
			LEFT JOIN ".TBL_AREAS." C ON B.area=C.id
		  where  a.`id_shop` = '".addslashes($_SESSION['shop'])."' and FIND_IN_SET('".$_REQUEST['hotelId']."',C.ids_unit_user) and id_hotel='".$_REQUEST['id_hotel']."'   and a.`seasonId`='".$_REQUEST['seasonId']."' and month='".date('Y-m-d',strtotime($fromVer))."'  ";
