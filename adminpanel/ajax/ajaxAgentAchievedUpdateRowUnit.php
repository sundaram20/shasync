...
if($id_company!='' && $seasonId!='' && $userId!='' && $_REQUEST['id_hotel']!=''){

//echo "WHERE `id_company`='".addslashes($id_company)."' AND id_hotel='".$_REQUEST['id_hotel']."' AND  seasonId = '".addslashes($seasonId)."' and  `id_user` = '".addslashes($userId)."' and  `month` = '".addslashes($DateValue)."'  ";
 $EditId		=	selectColumn(TBL_UNIT_AGENT_ACHIEVED,'id',"WHERE `id_company`='".addslashes($id_company)."' AND id_hotel='".$_REQUEST['id_hotel']."' AND  seasonId = '".addslashes($seasonId)."' and  `id_user` = '".addslashes($userId)."' and  `month` = '".addslashes($DateValue)."'  ");

if($EditId>0){
			 $editAchieved = " UPDATE `".TBL_UNIT_AGENT_ACHIEVED."` SET 						
			...
			$editAchieved .= "
						...
						WHERE `id` = '".$EditId."' and `id_hotel`='".$_REQUEST['id_hotel']."'";	
			 // echo $editAchieved;
	...
	while (strtotime($dateStart) <= strtotime($dateEnd)) {
                //echo "$dateStart\n";
                if(strtotime($dateStart)==strtotime($DateValue)){
						$recordvalue = $value;
					}else{
						$recordvalue = 0;
						}
				$addAchieved = "INSERT INTO `".TBL_UNIT_AGENT_ACHIEVED."` SET 
...
$userTypeTable	= TBL_UNIT_AGENT_ACHIEVED;	
	$SqlConn		  = "and a.id_hotel = '".$_REQUEST['id_hotel']."' and FIND_IN_SET('".$_REQUEST['userId']."',C.ids_unit_user)";
...
$DateValueMonth	 =	date('m', $_REQUEST['DateValue']);
 $rowTotal=	selectColumn(TBL_UNIT_AGENT_ACHIEVED,'sum(qty)',"WHERE `id_company`='".addslashes($id_company)."' AND id_hotel='".$_REQUEST['id_hotel']."' AND  seasonId = '".addslashes($seasonId)."' and  `id_user` = '".addslashes($userId)."'  ");

	echo '<p class="help-block">User Budget has been updated sucessfully.</p>####'.$rowTotal.'####'.$objTot->qty;	
}else{
