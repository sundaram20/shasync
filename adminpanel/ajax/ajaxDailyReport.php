...
$TotalCountrow 	= $db->fetch_assoc2($resSql);
		...
		UNION ALL
		(SELECT id,type,dated,lead_status,enquiry_id,hotel_id,assign_user_id, 'fs_enquiry_details' as `TableName` from `fs_enquiry_details` where status='1' and `id_shop` = '".addslashes($_SESSION['shop'])."' and  lead_status=1 and dated='".addslashes($row['dated'])."' $UserAssignId )" );
