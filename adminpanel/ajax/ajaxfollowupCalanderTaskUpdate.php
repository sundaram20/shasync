...
if($_REQUEST['followup_type']	== 8){
	...
	$checkFollowExisting = executeSql(" SELECT  `".TBL_DAILY_TASK_DETAILS."`.*  FROM `".TBL_DAILY_TASK_DETAILS."`  WHERE `".TBL_DAILY_TASK_DETAILS."`.`id` = '".addslashes($_REQUEST['followup_id'])."'");

	$row = $db->fetch_object2($checkFollowExisting);

	$updateInventory = executeSql("UPDATE  `".TBL_DAILY_TASK."`  SET 
									`follow_up_date`='".addslashes(date('Y-m-d',strtotime($_REQUEST['followup_date'])))."',
									`assign_user_id`='".$_REQUEST['assign_user_id']."',
									`follow_up_summary`='".$_REQUEST['followup_description']."',
									`follow_up_date`='".date('Y-m-d',strtotime($_REQUEST['followup_date']))."'
	...
	$updateSQL = "UPDATE  `".TBL_DAILY_CALENDER."`  SET 
									...
									`assign_user_id`='".$_REQUEST['assign_user_id']."',
									`doc_id`='".$insertId."'
									where   visit_id	='".addslashes($row->enquiry_id)."' AND `enquiry_details`='1' and type	='8'
									";	
