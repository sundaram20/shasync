...
$ext = pathinfo($path, PATHINFO_EXTENSION);
           ...
           {  
                ...
                $sourcePath = $_FILES['files']['tmp_name'];  
                 $target = "/var/www/vhosts/roomstatushub.in/httpdocs/sync/uploaded_files/payment_invoice/".$new_name;  
                ...
                {  
					...
					//Use whatever path to an Excel file you need.
					  ...
					  //"/var/www/vhosts/roomstatushub.in/httpdocs/sync/uploaded_files/payment_invoice/".$_FILES["files"]["name"];
    ...
    //Replace this with whatever you want to do with the data.
	 ...
	 ///echo $sheet->setCell('H'.$row, PHPExcel_Cell_DataType::TYPE_STRING); 
	  ...
	  if($sheet->getCell('A'.$row)->getValue()!=''){
		  ...
		  $rowData22[]=$sheet->getCell('E'.$row)->getValue();
		  $rowData22[]=$sheet->getCell('F'.$row)->getValue();
		  $rowData22[]= $Invoice_date;
		  $rowData22[]=$sheet->getCell('H'.$row)->getValue();
		  $rowData22[]=$sheet->getCell('I'.$row)->getFormattedValue();
		  $rowData22[]=$sheet->getCell('J'.$row)->getFormattedValue();
		  $rowData22[]=$sheet->getCell('K'.$row)->getValue();
		  $rowData22[]=$sheet->getCell('L'.$row)->getValue();
