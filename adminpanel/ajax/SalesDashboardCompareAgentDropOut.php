...
if(($_REQUEST['pdf']==1) || ($_REQUEST['summaryReportType'] == '61')){ //Agentwise Summary
 
 
  
 if($_REQUEST['reportType']==2){       
$sql = "SELECT `fs_agent_achieved`.id_company,`fs_users`.name as name_executive,`fs_company_group`.name as name_company_group, 
        `mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_USERS."`.ids_team,`".TBL_COMPANY."`.id_default_group
...
,sum(case when  ( ( `fs_agent_achieved` .month BETWEEN '".$FromLastYearDate."' And '".$ToLastYearDate."')) then `fs_agent_achieved`.qty else 0 end) as `LastYeartentative_revenue`
   ...
   //  echo '================>'.$sql;
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  ...
	  $companypincode = $companypincode!=''?'-'.$companypincode:'';     
	     if($_REQUEST['reportType']==1){
           ...
           $newConfirmednewTentative=($rowList->newConfirmed+$rowList->newTentative);
             ...
             $LastYearnewConfirmednewTentative=($rowList->LastYearnewConfirmed+$rowList->LastYearnewTentative);
             $LastYearnewConfirmednewTentative_revenue=($rowList->LastYearconfimed_revenue+$rowList->LastYeartentative_revenue);
        ...
        if($_REQUEST['reportType']==2){
            ...
            $newConfirmednewTentative=($rowList->newConfirmed);
             ...
             $LastYearnewConfirmednewTentative=($rowList->LastYearnewConfirmed);
             $LastYearnewConfirmednewTentative_revenue=($rowList->LastYearconfimed_revenue);
            
            }
	    
	     $SumTotalGolyRoomNights=round((($newConfirmednewTentative-$LastYearnewConfirmednewTentative)/$LastYearnewConfirmednewTentative) *100,2);
	  ...
	  //  array_push($mtdRoomRevenue,($rowList->confimed_revenue==''?0:round($rowList->confimed_revenue)));
	    ...
	    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['name_executive']=$rowList->name_executive;
	    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['CityPinCode']=$companyCity.$companypincode;
	    
	    
	$reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearroomnights']=$LastYearnewConfirmednewTentative==''?'0':$LastYearnewConfirmednewTentative;
    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearconfimed_revenue']=$LastYearnewConfirmednewTentative_revenue==''?0:round($LastYearnewConfirmednewTentative_revenue);
    $reportArray['Drop-Out'][strtolower($companyname==''?$emptytext7:$companyname)]['GROWTH']=$SumTotalGolyRoomNights;
	}  
  }
...
.table {
    ...
    margin-bottom: 20px;	   
    width:100%; border: 1px important;
} 
table {
	font-size:11px !important; 
    background-color: transparent;
    border-collapse: collapse;
    border-spacing: 0;border: 1px important;
	}
...
if($_REQUEST['pdf']==1){
 $content .= '<table class="table" border=1 style=" margin-bottom: 0px;border: 1px;  ">
						<tr>					
						  <th>
...
$GROWTHnights  = array_column($mainDatalist, 'GROWTH');
...
array_multisort($GROWTHnights, SORT_ASC, $lastYearroomnights, SORT_DESC, $mainDatalist);
