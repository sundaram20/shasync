...
if($_REQUEST['FollowupClose']=='close'){ 
	...
	$companyName = selectColumn(TBL_COMPANY,'name','WHERE id_company="'.$_POST['id_company'].'" ')
             .' - '.
             selectColumn(TBL_COMPANY,'city','WHERE id_company="'.$_POST['id_company'].'" ');
		...
		$leadSource = selectColumn(TBL_LEAD_SOURCE_MASTER,'name','WHERE id="'.$row->id_mst_lead_source.'" ');
...
//incentive remarkes
	...
	$companyName = selectColumn(TBL_COMPANY,'name','WHERE id_company="'.$row->id_company.'" ') 
              .' - '.
              selectColumn(TBL_COMPANY,'city','WHERE id_company="'.$row->id_company.'" ');

   $leadSource=selectColumn(TBL_LEAD_SOURCE_MASTER,'name','WHERE id="'.$_POST['id_mst_lead_source'].'" ');
...
if($ccCreatedByEmail!='' && $ccCreatedByEmail!='roomstatushublogs@gmail.com'){
			$CreatedByEmailcc	=	','.$ccCreatedByEmail;
	}
if($ccEmail!='roomstatushublogs@gmail.com' && $ccEmail!=$ccCreatedByEmail){
    $ccEmailID	=	','.$ccEmail;
}
if($ccHeadEmail!='' && $ccHeadEmail!='roomstatushublogs@gmail.com' && $ccHeadEmail!=$ccCreatedByEmail && $ccHeadEmail!=$ccEmail){
    $ccHeadEmailID	=	','.$ccHeadEmail;
}
if(addslashes($_SESSION['shop'])==6){
   // $MohitEmailId   ='mohit@fernhotels.com,noshir@fernhotels.com';
    $MohitEmailId   ='roomstatushublogs@gmail.com';
	...
	if($ccHotelByEmailArr!=''){
	  $ccHotelByEmailArr	=	','.$ccHotelByEmailArr;
		...
		//$cc='roomstatushublogs@gmail.com';
		 ...
		 $sendMail->sendMailContent('support1@roomstatushub.com', $to, $subject,$mailContent, trim($cc,','),$ReplyToName,$addReplyTo);
