...
$reportArray=array();	
 ...
 if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==1) || ($_REQUEST['summaryReportType'] == '7' && $_REQUEST['reportType']==1)){//Team Wise  Summary PDF reportType Pickup Report
    ...
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       ".$cond." AND  ".$allUser."  GROUP BY `".TBL_USERS."`.myownteam_id,`".TBL_COMPANY."`.id_default_group Order BY `mst_team`.id_group,`fs_users`.myownteam_id";
 ...
 if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==2) || ($_REQUEST['summaryReportType'] == '7' && $_REQUEST['reportType']==2)){//Team Wise  Summary PDF reportType BOB Report
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) $condhotelAccess GROUP BY `".TBL_USERS."`.myownteam_id,`".TBL_COMPANY."`.id_default_group Order BY `mst_team`.id_group,`fs_users`.myownteam_id";
...
//die;
if($_REQUEST['summaryReportType'] == '6' || $_REQUEST['pdf']==12){//Business Source Summary
      
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_COMPANY."`.id_default_group,
       ...
       LEFT JOIN `".TBL_COMPANY."` ON `".TBL_ORDERS."`.id_company = `".TBL_COMPANY."`.id_company
       LEFT JOIN `".TBL_AREAS."` ON `".TBL_COMPANY."`.area = `".TBL_AREAS."`.id
       LEFT JOIN `".TBL_USERS."` ON `fs_areas_assign`.user_id=".TBL_USERS.".id  ".$cond." AND  ".$allUser."  GROUP BY `".TBL_COMPANY."`.id_default_group" ;
       
       
    //   echo $sql;
      //die;
       
       $SummaryHedding='Business Source';
       $TaleName='Business Source';
       $resultList = mysqli_query($connNew,$sql);
       $empty6=0;
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= selectColumn(TBL_GROUP,'name'," WHERE `id_group` = '".$rowList->id_default_group."'");
	    $exeNameArr[]=ucwords(strtolower($companyname));
	    ...
	    $emptytext6 ='empty_'.$empty6++;
	    $reportArray['Business Source']['name'][strtolower($companyname==''?$emptytext6:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray['Business Source']['name'][strtolower($companyname==''?$emptytext6:$companyname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	    }
	    
	    
	}  
  }
  if($_REQUEST['summaryReportType'] == '5' || $_REQUEST['pdf']==12){//Booking Source Summary
      
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`".TBL_ORDERS."`.id_booking_source,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_COMPANY."`.id_default_group,
       ...
       LEFT JOIN `".TBL_COMPANY."` ON `".TBL_ORDERS."`.id_company = `".TBL_COMPANY."`.id_company
       LEFT JOIN `".TBL_AREAS."` ON `".TBL_COMPANY."`.area = `".TBL_AREAS."`.id
       LEFT JOIN `".TBL_USERS."` ON `fs_areas_assign`.user_id=".TBL_USERS.".id  ".$cond." AND  ".$allUser."  GROUP BY `".TBL_ORDERS."`.id_booking_source" ;
       
       
    //   echo $sql;
      //die;
     //  
       $SummaryHedding='Booking Source';
       $TaleName='Booking Source';
       $resultList = mysqli_query($connNew,$sql);
       $empty5=0;
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= selectColumn('fs_booking_source_master','name'," WHERE `id` = '".$rowList->id_booking_source."'");
	    $exeNameArr[]=ucwords(strtolower($companyname==''?'empty':$companyname));
	    $newConfirmednewTentative=($rowList->newConfirmed+$rowList->newTentative);
	    array_push($mtdThisValues,($newConfirmednewTentative==''?'0':$newConfirmednewTentative));
	    array_push($mtdRoomRevenue,($rowList->confimed_revenue==''?0:round($rowList->confimed_revenue)));
	   
	    $emptytext5 ='empty_'.$empty5++;
	    $reportArray['Booking Source']['name'][strtolower($companyname==''?$emptytext5:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	     $reportArray['Booking Source']['name'][strtolower($companyname==''?$emptytext5:$companyname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	}  
  }
  
if($_REQUEST['summaryReportType'] == '1' || $_REQUEST['pdf']==12){
	//booking _date end
    if($_REQUEST['reportType']==1){
	...
	while($rowList = mysqli_fetch_object($resultList)){
	    ...
	    array_push($mtdRoomRevenue,($rowList->confimed_revenue==''?0:round($rowList->confimed_revenue)));
	     ...
	     $emptytext1 ='empty_'.$empty1++;
	   
	    $reportArray['Executivewise']['name'][strtolower($rowList->name_executive==''?$emptytext1:$rowList->name_executive)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	     $reportArray['Executivewise']['name'][strtolower($rowList->name_executive==''?$emptytext1:$rowList->name_executive)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	
	}

       
}   
if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==1) || ($_REQUEST['summaryReportType'] == '1' && $_REQUEST['reportType']==1)){//Executivewise  Summary PDF
    ...
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       ".$cond." AND  ".$allUser."  GROUP BY `".TBL_USERS."`.myownteam_id,`fs_users`.id,`".TBL_ORDERS."`.id_company Order BY `mst_team`.id_group,`fs_users`.myownteam_id";
      //echo $sql;
      //die;
     //  LEFT JOIN  `".TBL_TEAM."` ON `".TBL_TEAM."`.ids_team REGEXP CONCAT('(^|,)(', REPLACE('".$id_teams."', ',', '|'), ')(,|$)') AND  FIND_IN_SET(myownteam_id,'".$id_teams."') 
       $SummaryHedding='Executivewise  ';
...
if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==2) || ($_REQUEST['summaryReportType'] == '1' && $_REQUEST['reportType']==2)){//Executivewise  Summary PDF Bob
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) $condhotelAccess GROUP BY `".TBL_USERS."`.myownteam_id,`fs_users`.id,`".TBL_ORDERS."`.id_company Order BY `mst_team`.id_group,`fs_users`.myownteam_id";
  ...
  if($_REQUEST['summaryReportType'] == '2' || $_REQUEST['pdf']==12){ // HOTEL WISE View Table
    ...
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_COMPANY."`.id_default_group,
       ...
       LEFT JOIN `".TBL_USERS."` ON `fs_areas_assign`.user_id=".TBL_USERS.".id  ".$cond." AND  ".$allUser."  GROUP BY `".TBL_ORDERS."`.id_hotel" ;
       
       //LEFT JOIN `".TBL_HOTELS."` ON `".TBL_HOTELS."`.id = `".TBL_ORDERS."`.id_hotel and `".TBL_HOTELS."`.status='1'
       //echo $sql;
      //die;
     //  
       $SummaryHedding='Hotelwise ';
       $TaleName='Hotel Name';
       $resultList = mysqli_query($connNew,$sql);
       $empty2=0;
	while($rowList = mysqli_fetch_object($resultList)){
	    //echo $rowList->newConfirmed;
	  $hotelname= selectColumn(TBL_HOTELS,'name','WHERE id='.$rowList->id_hotel.' ');
	    $exeNameArr[]=ucwords(strtolower($hotelname));
	     $newConfirmednewTentative=($rowList->newConfirmed+$rowList->newTentative);
	    array_push($mtdThisValues,($newConfirmednewTentative==''?'0':$newConfirmednewTentative));
	    array_push($mtdRoomRevenue,($rowList->confimed_revenue==''?0:round($rowList->confimed_revenue)));
    
	     $emptytext2 ='empty_'.$empty2++;
	   
	    $reportArray['Hotelwise']['name'][strtolower($hotelname==''?$emptytext2:$hotelname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	     $reportArray['Hotelwise']['name'][strtolower($hotelname==''?$emptytext2:$hotelname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
	
	}  
  }
    
 if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==1) || ($_REQUEST['summaryReportType'] == '2' && $_REQUEST['reportType']==1)){//Hotel Wise  Summary PDF PICKUP Report
    ...
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       ".$cond." AND  ".$allUser."  GROUP BY `".TBL_USERS."`.myownteam_id,`".TBL_ORDERS."`.id_hotel,`".TBL_USERS."`.name Order BY `mst_team`.id_group,`fs_users`.myownteam_id";
      //echo $sql;
      //die;
     //  LEFT JOIN  `".TBL_TEAM."` ON `".TBL_TEAM."`.ids_team REGEXP CONCAT('(^|,)(', REPLACE('".$id_teams."', ',', '|'), ')(,|$)') AND  FIND_IN_SET(myownteam_id,'".$id_teams."') 
 ...
 if(($_REQUEST['pdf']==1 && $_REQUEST['reportType']==2) || ($_REQUEST['summaryReportType'] == '2' && $_REQUEST['reportType']==2)){//Hotel Wise  Summary PDF BOP
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) $condhotelAccess GROUP BY `".TBL_USERS."`.myownteam_id,`".TBL_ORDERS."`.id_hotel,`".TBL_USERS."`.name Order BY `fs_users`.myownteam_id";
 ...
 if($_REQUEST['summaryReportType'] == '3' || $_REQUEST['pdf']==12){ //Agentwise Summary
    ...
    $sql = " SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_COMPANY."`.id_default_group,
       ...
       LEFT JOIN `".TBL_COMPANY."` ON `".TBL_ORDERS."`.id_company = `".TBL_COMPANY."`.id_company
       LEFT JOIN `".TBL_AREAS."` ON `".TBL_COMPANY."`.area = `".TBL_AREAS."`.id
       LEFT JOIN `".TBL_USERS."` ON `fs_areas_assign`.user_id=".TBL_USERS.".id  ".$cond." AND  ".$allUser."  GROUP BY `".TBL_ORDERS."`.id_company" ;
...
foreach($reportArray as $maintitle=>$mainDatalist){
	...
	$contentTeam .='<tr><th colspan="4" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>'.$maintitle.' Breakup For Period '.$reportPeriod.'</b></th></tr>';
    $contentTeam .='<tr><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Name</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Room Nights</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">Revenue</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">ARR</th></tr>';
	 $TotalRoomNight='';
...
//Sorting Order Number Height to Low
      ...
      foreach($subDataList as $list=> $DataList){
           if($DataList['roomnights']>0){
                 if($DataList['confimed_revenue']>0){
                        ...
                        $contentTeamBody .='<tr ><td style="text-align:left;">'.strtoupper($name).'</td><td style="text-align:center;">'.$DataList['roomnights'].'</td><td style="text-align:center;">'.$DataList['confimed_revenue'].'</td><td style="text-align:center;">'.round($mtrArr).'</td></tr>';
                    }
            ...
            if($TotalTeamRoomNight>0){
                $contentTeam .='<tr><th  style="vertical-align:central;text-align:Left;color:#000;background-color:#e2f7b8; font-size:12px !important"><b>'.strtoupper($TeamName).'</b></th><th  style="vertical-align:central;text-align:center;color:#000;background-color:#e2f7b8; font-size:12px !important"><b>' .$TotalTeamRoomNight.'</b></th><th  style="vertical-align:central;text-align:center;color:#000;background-color:#e2f7b8; font-size:12px !important"><b> '.$TotalTeamConfimedRevenue.'</b></th><th  style="vertical-align:central;text-align:center;color:#000;background-color:#e2f7b8; font-size:12px !important"><b> '.$SumTeamTotalArray.'</b></th></tr>';
            }
    ...
    if($teamGroup!='name'){$contentTeam .='<tr style="font-weight:bold;background-color:#3C8DBC;color:white;"><td style="text-align:center;background-color:#c2d69a;">'.ucwords($teamGroup).' Total</td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$TotalRoomNight.'</td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$TotalConfimedRevenue.'</td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$SumTotalArray.'</td></tr>';
    }
    ...
    $TotalRoomNight='';
     ...
     $contentTeam .= '</table><br/><br/>';
         ...
         if($maintitle=='Team Wise'){
        	 ...
        	 $contentGroup .='<tr style="vertical-align:central;text-align:"><th colspan="4" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>'.$maintitle0ch.' Groupwise Summary For Period '.$reportPeriod.'</b></th></tr>	';
             $contentGroup .=    '<tr ><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a; ">Office</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Room Nights</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">Revenue</th><th style="vertical-align:central;text-align:center;color:#000;background-color:#c2d69a;">ARR</th></tr>';
                 foreach($GroupArray as $name => $GroupNameArray){
                    $contentGroup .='<tr ><td style="text-align:left;">'.strtoupper($name).'</td><td style="text-align:center;">'.$GroupNameArray['RoomNights'].'</td><td style="text-align:center;">'.$GroupNameArray['RoomRevenue'].'</td><td style="text-align:center;">'.round($GroupNameArray['Arr']).'</td></tr>';
             ...
             $contentGroup .='<tr style="font-weight:bold;background-color:#3C8DBC;color:white;"><td style="text-align:center;background-color:#c2d69a;">Total </td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$TotalTeamWiseRoomNight.'</td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$TotalTeamWiseConfimedRevenue.'</td><td style="border-left:1px solid #fff;background-color:#c2d69a;text-align:center;">'.$SumTotalTeamWiseArray.'</td></tr>';
