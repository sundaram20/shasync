...
$RateRecordRow = $db->fetch_object2($RateSQL);
	  	...
	  	if($_POST['rate_level_id']!='' && $_POST['market']!='' && $_POST['seasonId']!='' && $start_date !='' && $end_date!=''){
			 ...
			 $end_date		=	selectColumn(TBL_RATE_SEASON,'end_date'," WHERE `id` = '".addslashes($_POST['seasonId'])."'");					
				...
				$checkExisting = executeSql("SELECT * FROM `".TBL_RATE."` where  rate_level_id='".addslashes($_POST['rate_level_id'])."' and seasonId='".addslashes($_POST['seasonId'])."' and market='".addslashes($_POST['market'])."' AND company_id=0 AND `id_shop` = '".addslashes($_SESSION['shop'])."'");
...
//echo "ALL HOTEL INSERT";
			...
			$lastRecordRes = executeSql("SELECT MAX(`rate_name`) as maxId FROM `fs_rate` WHERE  `id_shop` = '".addslashes($_SESSION['shop'])."' AND id=(SELECT MAX(id) FROM `fs_rate` WHERE id_shop='".$_SESSION['shop']."' ) ");
