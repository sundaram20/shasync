...
//jump
	$sql= "SELECT A.total_products AS PRODUCT,A.total_price AS PRICE,A.id_order,A.reference,B.name AS Hotel,CONCAT(C.first_name,' ',C.last_name) AS Guest,D.name AS Source,E.name AS Executive,G.name AS paymentStatus,A.booking_status,A.invoice_date,A.checkin,A.checkout,A.no_of_days
			FROM fs_orders A
			LEFT JOIN fs_hotels B ON A.id_hotel = B.id
			LEFT JOIN fs_customer C ON A.id_customer = C.id_customer
			LEFT JOIN fs_company D ON A.id_company = D.id_company
			LEFT JOIN fs_areas_assign F ON D.area = F.id
			LEFT JOIN fs_users E ON F.user_id = E.id
			LEFT JOIN fs_payment_status_master G ON A.payment_status = G.id_order_state 
			WHERE A.checkin ='".$flash_date."' ".$condhotelAccess." AND (A.booking_status = 1 OR A.booking_status = 2) AND C.type = 1 AND A.id_shop = '".$_SESSION['shop']."' ORDER BY B.name ASC";
	...
	if($_POST['Download'] == 'Generate'){
		...
		->setCellValue('A2', 'EXPECTED ARRIVALS ON  '.date("d-m-Y",strtotime($flash_date)));
		$objPHPExcel->getActiveSheet()->mergeCells('A2:N2');
		$head_hotel_row = 3;
		$head_cntr_column = "A";$head_hotel_column = "A";
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue($head_cntr_column++.$head_hotel_row, 'S.No.')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Reservation Id')
			...
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Booking Status')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Booking Date')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Checkin - Checkout')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Booking Type')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Room Details')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'Revenue')
			->setCellValue($head_cntr_column++.$head_hotel_row, 'No. of Nights');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A3:N3')->applyFromArray($styleArray_1);
	 $objPHPExcel->getActiveSheet()->getStyle('A3:N3')->getAlignment()->applyFromArray(
	    array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,)
	);
	...
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('C')->setWidth(33);	
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('D')->setWidth(33);
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('E')->setWidth(33);	
	...
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('J')->setWidth(25);	
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('K')->setWidth(20);
	$objPHPExcel->getActiveSheet(1)->getColumnDimension('L')->setWidth(25);
	...
	->setCellValue($head_order_data++ . $connew, $row->reference)
	->setCellValue($head_order_data++ . $connew, $row->Hotel)
	->setCellValue($head_order_data++ . $connew, $row->Guest)
	->setCellValue($head_order_data++ . $connew, $row->Source)
	->setCellValue($head_order_data++ . $connew, $row->Executive)
	->setCellValue($head_order_data++ . $connew, $row->paymentStatus)
	->setCellValue($head_order_data++ . $connew, $bookingStatus)
	->setCellValue($head_order_data++ . $connew, date('d-M-Y',strtotime($row->invoice_date)))
	->setCellValue($head_order_data++ . $connew, date('d-M-Y',strtotime($row->checkin))." - ".date('d-M-Y',strtotime($row->checkout)));
//-------------------Adding booking type here------------
	if($row->no_of_days > 0){
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue($head_order_data++ . $connew, 'Room');
	}
	else{
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue($head_order_data++ . $connew, 'Lunch');
	}

//--------------------fetching room types here---------------
	
	echo $sqlRoom = "SELECT a.room_id AS RID,b.name AS ROOM,sum(a.room_quantity) AS QUANT  FROM `fs_order_detail` a
				LEFT JOIN `fs_room_type` b ON 	a.room_id = b.id
				WHERE id_order =".$row->id_order." AND a.dated = '".$flash_date."' GROUP BY a.room_id ";
			
	$resRoom = mysqli_query($conn,$sqlRoom);

	if($resRoom){
		$room_quant = 0;
		while ($roomData = mysqli_fetch_object($resRoom)) {

			$detail.=$roomData->ROOM."[".$roomData->QUANT."] ";
			$room_quant += $roomData->room_quantity;
		}
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue($head_order_data++ . $connew, $detail);
		$detail = "";	
	}	
...
//---------------------END------------------------------------
	...
	$objPHPExcel->setActiveSheetIndex(0)
	->setCellValue($head_order_data++ . $connew, "Rs. ".round($row->PRICE,2));

	$objPHPExcel->setActiveSheetIndex(0)
	->setCellValue($head_order_data . $connew, $row->no_of_days);
	...
	$objPHPExcel->getActiveSheet()->getStyle('L4:L'.$connew)
	...
	$connew++;	
		...
		$objPHPExcel->getActiveSheet()->setTitle('Expected Arrival');
		...
		header('Content-Disposition: attachment;filename="Expected_Arrival_report.xls"');
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       Expected Arrivals 
        <small>Expected Arrivals Report</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="home.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Report Manager</li>
        <li class="active">Expected Arrivals Report</li>
      </ol>
    </section>
    <!-- Main content -->
	...
	<div class="box box-default">
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
          <h3 class="box-title">Expected Arrivals Report &nbsp;</small> </h3>          
        </div>
        <!-- /.box-header -->
        ...
        <div class="box-body">
		  ...
		  <!--<div class="form-group col-sm-4">
			...
			<div class="col-md-4">
              <div class="form-group">
                  <label for="start_date">Arrival Day</label>
