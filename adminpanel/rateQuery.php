                   ...
                   <li> <i class="fa fa-angle-double-right bg-red"></i>
                    <div class="timeline-item">
                       ...
                       <div class="form-group col-md-2">
                           <label for="reservation_date">Season</label>
                           <div class="input-group">
                            <?php 
                              
                              $sqlSea="SELECT * FROM ".TBL_RATE_SEASON." WHERE id_shop='".$_SESSION['shop']."' AND status=1" ;
                              $resSea = mysqli_query($connNew,$sqlSea);


                             ?>                           
                            <select onchange="fetchSeasonDate(this.value)" name="id_season" id="id_season" class="form-control select2">

                              <option value="">Select Season</option>
                               <?php while($rowSea=mysqli_fetch_object($resSea)){
                                echo '<option value="'.$rowSea->id.'">'.$rowSea->name.'</option>';
                                }?>
                            </select>                         
...
function fetchSeasonDate(id){

  if(id!=''){
    $.ajax({
      url:"ajax/fetchSeasonDate.php",
      data:'id_season='+id,
      type:'POST',
      success:function(data){
        $("#reservation_date").val(data);
