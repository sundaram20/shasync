...
$content .= '<table class="table" style=" margin-bottom: 0px;border: 0px;  ">
						<tr>					
						  <th>';
   if($logo!=''){
	   $content .='<img src="../../uploaded_files/shop/'.$logo.'" class="img-responsive" alt="logo" title="logo"   />&nbsp;&nbsp;&nbsp; </th>';
	   }
...
$content .= '			 
    ...
    $headerDescription = selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE type="1" AND id_shop="'.$_SESSION['shop'].'"  ');
    $content .=strip_tags($headerDescription,"<style><b><br><strong><img><table><p><tr><td><span><ul><li><ol>");

}	
 $content .= '<table class="table"  style="page-break-before:always; margin-bottom: 0px;border: 1px; width:100%">';
...
if (strpos($FindSearonWord, 'WINTER') !== false) {
	$BackgroundColor	='background-color:#981b1e;';
...
$content .= '<tr style="'.$BackgroundColor.'color:#fff !important;font-size:16px !important;">
			<th colspan="16" ><b>'.$rowShop->name.' : '.selectColumn(TBL_RATE_MARKET,'name'," WHERE `id` = '".$RateTitle->market."'").'-'.selectColumn(TBL_RATE_SEASON,'name'," WHERE `id` = '".$RateTitle->seasonId."'").'</b></th>
						...
						<tr  style="background-color:#c1934c;color:#fff !important;font-size:11px !important">
...
$previous = $current;
 ...
 if ($resultHotelDetail->excel_display_weekday  =='1'){		 
					 $SingleAndWeekday	='<th ><b>WEEKDAYS</b></th>
											<th><b>WEEKENDS (FRI-SUN)</b></th>';
		 }else{
			  		 $SingleAndWeekday	='<th ><b>SINGLE</b></th>
										<th ><b>DOUBLE</b></th>';
			  }
...
if(strpos($FindSearonWord, 'SUMMER') !== false) {
 ...
 //  $pageBreaksWinSum	='';
  ...
  $content	.='<table   style="'.$pageBreaksWinSum.'background-color:#f6d273;color:#fff;vertical-align:central;text-align:center;" width="100%">
  <tr  style="background-color:#981b1e;color:#fff;vertical-align:central;text-align:center;">  
...
*/
  ...
  //$resCat_rooms = executeSql("SELECT *  from `fs_rate_details` where status='1' and hotel_id=".addslashes($resultHotelDetail->id));
			...
			while($rowInclusion = $db->fetch_object2($resCat_rooms)){
				...
				if($Lable	==1){										
				$content .='			
				<tr style="text-align:left;vertical-align:middle;background-color:#f6d273;">
			...
			<th width="15.5%" style="font-weight: 100" align="left">'.strtoupper(selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = ".$rowInclusion->room_id)).'</th>
...
if ($resultHotelDetail->excel_display_weekday  =='1'){
				...
				if($rowInclusion->single_pax_price>0){	
				$single_pax_price_week	=	'INR S : '.$rowInclusion->single_pax_price.' / D : '.$rowInclusion->double_pax_price.' '.$t;
				...
				$double_pax_price_week	=	'INR S : '.$rowInclusion->weekend_single_pax_price.' / D : '.$rowInclusion->weekend_double_pax_price.' '.$t;
