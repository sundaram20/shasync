...
$content .= '
			...
			<th colspan="7" ><b>'.$rowShop->name.'-'.selectColumn(TBL_RATE_MARKET,'name'," WHERE `id` = '".$RateTitle->market."'").'-'.selectColumn(TBL_RATE_SEASON,'name'," WHERE `id` = '".$RateTitle->seasonId."'").'</b></th>
			</tr>
						<tr  style="background-color:#4f6228;color:#fff;">
						  ...
						  $content .= '</b></th>
						   <th colspan="2" align="left"><b>RATES VALIDITY:';
...
<th width="22%"><b>'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = ".$rowInclusion->room_id).'</b></th>
	...
	$single_pax_price	=	'INR '.$rowInclusion->single_pax_price;
	}else{
	$single_pax_price	=	"-";
		}
if($rowInclusion->double_pax_price>0){
	
	$double_pax_price	=	'INR '.$rowInclusion->double_pax_price;
	}else{
	$double_pax_price	=	"-";
		}
if($rowInclusion->extra_bed_price>0){
	
	$extra_bed_price	=	'INR '.$rowInclusion->extra_bed_price;
	}else{
	$extra_bed_price	=	"-";
		}	
...
$Count	=	explode(',',$RateHotelDetail->rate_points);
foreach($Count as $value){
	
	$resRatepoint = executeSql("SELECT * FROM `".TBL_RATE_POINTS."` as a  where   a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND a.id='".$value."'") ;
	$RowRatepoint = $db->fetch_object2($resRatepoint);


	/*$content .=	'
   		 <tr><td colspan="7">'.$RowRatepoint->description;
$content .='</td></tr>';
*/
	$Desc	=	$RowRatepoint->description;
...
//$y	=	strip_tags($Desc, "<table><tr><td><b><i><p><span><strong><br>");
/*$content .=	'
   		 <tr><td colspan="7">'.$additional_points;
$content .='</td></tr>';*/

$additional_points	=	strip_tags($additional_points, "<table><tr><td><b><i><p><span><strong><br>");
$content .=	'
   		<tr><td colspan="7">'.$additional_points;
$content .='</td></tr></table>';
