...
//customer details/////////////////////////
$customer = selectColumn(TBL_CUSTOMER,'CONCAT(title," ",first_name," ",last_name)','WHERE id_customer="'.$RateTitle->id_contacts.'" ');
$customerLastName = selectColumn(TBL_CUSTOMER,'CONCAT(title," ",last_name)','WHERE id_customer="'.$RateTitle->id_contacts.'" ');
$id_desgination=selectColumn(TBL_CUSTOMER,'designation','WHERE id_customer="'.$RateTitle->id_contacts.'" ');
$designation=selectColumn(TBL_DESIGNATION_MASTER,'name','WHERE id="'.$id_desgination.'" ');
////customer company details/////////////////
$id_company = selectColumn(TBL_CUSTOMER,'id_company','WHERE id_customer="'.$RateTitle->id_contacts.'" ');
...
$content .= '<div class="header">
						   ...
						   <img src="../../uploaded_files/shop/sayaji-head-logo.PNG" class="img-responsive" style="width:200px;" alt="logo" title="logo"    />'; 
						   //.$logo.'" class="img-responsive" alt="logo" title="logo"    />';
...
//=====================Cover Letter=====================================
/*
$content .= "<table style='margin-top:0px;'>
				<tr><td>Date ".date('d/m/Y')."<br/></td></tr></table>";	

$content .= "<table ><tr><td><b>".$customer."</b></td></tr><tr><td>".$designation."<br/><br/></td></tr><tr><td><b>".$custCompany."</b></td></tr>
				".($addressComp!=''?'<tr><td>'.wordwrap($addressComp,40,"<br>\n").'</td></tr>':'')."".($cityComp!=''?'<tr><td>'.$cityComp.'</td></tr>':'')."".($phoneComp!=''?'<tr><td>Contact Number: '.$phoneComp.'</td></tr>':'')."".($emailComp!=''?'<tr><td>Email: '.$emailComp.'</td></tr>':'')."</table>";			
...
$content .= "<table style='margin-top:20px;text-align:justify;'><tr><td>
						 Dear ".$customerLastName.",<br/>
						</td></tr></table>";
		

*/
$content .="<span style='margin-top:5px !important;text-align:justify !important;'>".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND id="'.$mailContent.'" AND type=2')."</span>";




$content .= "<table style='margin-bottom:-50px;text-align:justify;'><tr><td>
						 ".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND type=3 AND id="'.$mailFooter.'" ')."<br></td></tr></table>";

//$content .= "<br><br>";
$customer = selectColumn(TBL_CUSTOMER,'CONCAT(title," ",first_name," ",last_name)','where id_customer="'.$RateTitle->id_contacts.'" ');

  $customerTitle = selectColumn(TBL_CUSTOMER,'title','where id_customer="'.$RateTitle->id_contacts.'" ');

  $customerLastName = selectColumn(TBL_CUSTOMER,'last_name','where id_customer="'.$RateTitle->id_contacts.'" ');

  $id_designation = selectColumn(TBL_CUSTOMER,'designation','where id_customer="'.$RateTitle->id_contacts.'" ');
  $id_company_cust =selectColumn(TBL_CUSTOMER,'id_company','where id_customer="'.$RateTitle->id_contacts.'" ');
  ...
  $customerDesignation = $designation = selectColumn(TBL_DESIGNATION_MASTER,'name','where id="'.$id_designation.'" ');
	...
	$to = selectColumn(TBL_CUSTOMER,'email','where id_customer="'.$RateTitle->id_contacts.'" ');

	$company = selectColumn(TBL_COMPANY,'name','where id_company="'.$RateTitle->company_id.'" ');
	
	$id_area = selectColumn(TBL_COMPANY,'area','where id_company="'.$RateTitle->company_id.'" ');
	...
	$season = selectColumn(TBL_RATE_SEASON,'name','where id="'.$RateTitle->seasonId.'" ');
...
/* $content.="<table>
              <tr>
                  <td  ><b>".ucwords($handeledby)."</b></td>
                  
              </tr>
              <tr>
                <td  ><b>".ucwords($designation)."</b></td>
              </tr>
              <tr><td ><span style='color:green;font-weight:bold;'>".$companyNameUser."</span>";
      
           $content .=    "<br>".$address1.','.$address2.','.$city.'-'.$zip."<br>
...
//style='page-break-before:always;margin-top:50px;'

//==========================================================page-break-before:always;
 $content .= '<table class="table"  style="margin-top:50px; margin-bottom: 0px;border: 1px; width:100%">';
...
$content .= '<tr style="'.$BackgroundColor.'color:#fff !important;font-size:16px !important;">
			<th colspan="16" ><b>'.$rowShop->name.' '.selectColumn(TBL_RATE_MARKET,'name'," WHERE `id` = '".$RateTitle->market."'").'-'.selectColumn(TBL_RATE_SEASON,'name'," WHERE `id` = '".$RateTitle->seasonId."'").'</b></th>
						...
						<tr  style="color:#000 !important;font-size:11px !important">
...
if($DesignationBy !=''){
	$DesignationByName	=	' / '.$DesignationBy;
			     ...
			     $content .= '</b></th><th colspan="2" align="left"><b>DATE:&nbsp; '.date("d-m-Y",strtotime($RateTitle->date_created)).'</b></th>
						...
						<!--<tr style="background-color:#a3c33c;color:#fff !important;font-size:11px !important">
...
$previous = $current;
 ...
 if ($resultHotelDetail->excel_display_weekday  =='1'){		 
					 $SingleAndWeekday	='<th width="100px;"><b>WEEKDAYS</b></th>
											<th width="100px;"><b>WEEKENDS (FRI/SAT)</b></th>';
		 }else{
...
if(strpos($FindSearonWord, 'SUMMER') !== false) {
 ...
 //  $pageBreaksWinSum	='';
  ...
  <tr  style="background-color:#595959;color:#fff;vertical-align:central;text-align:center;border-color:#fff !important;">  
...
<tr width="100%" style="border:1px solid #fff;"> 


<td colspan="4" style="float: left!important;vertical-align: top !important; width:250px;border-bottom:1px; border-right:0px;">
		<table border="0px ;" width="100%">
			<tr style="background-color:#AEB6BF;color:#000; ">
			<th class="fitwidth" style="text-align:left;vertical-align: left !important;color:#000;"><b>'.strtoupper($resultHotelDetail->city).' - </b>			
			<b style="font-weight:bold;">'.strtoupper($resultHotelDetail->name).' </b>
			</th>
			</tr>
...
*/
  ...
  //$resCat_rooms = executeSql("SELECT *  from `fs_rate_details` where status='1' and hotel_id=".addslashes($resultHotelDetail->id));
			...
			while($rowInclusion = $db->fetch_object2($resCat_rooms)){
				...
				if($Lable	==1){										
				$content .='			
				<tr style="text-align:left;vertical-align:middle;background-color:#AEB6BF ;color:#000 !important;">
						<th width="90px;"><b>ROOM  CATEGORY</b></th>												
						<th width="40px;"><b>PLAN</b></th>';										
						$content	.=$SingleAndWeekday;						
						$content	.='
						<th width="80px;"><b>EXTRA BED</b></th>	';
...
$content .='<th width="150px;" style="font-weight: 100;display:none!important;">'.$HotelBLD.'</th>
			</tr>';
	 ...
	 $content .='<tr><td colspan="5">'.$RateHotelDetail->hotel_remarks.'</td></tr>';
	 }
	 ...
	 $resCat_rooms = executeSql("SELECT * FROM `".TBL_RATE."` as a join `".TBL_RATE_DETAILS_SPECIAL."` as b where  hotel_id='".addslashes($resultHotelDetail->id)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND b.detail_status=1 AND a.id=b.rate_id AND b.rate_id='".$Newrate_id."'");
...
//^^^^^^^^^^^^^^^^^^^^^^^^^State And Zone^^^^^^^^^^^^^^^^^^^^^
		 ...
		 $current = $resultHotelDetail->zonal;
		if ($current != $previous) {			
		$zonalName	=	selectColumn('fs_state','name'," WHERE `id` = '".$resultHotelDetail->zonal."'");
			if($zonalName	!=''){
				$zonal	=	$zonalName;
				}
//				$City	=	" - ".selectColumn(TBL_STATE,'name'," WHERE `id_state` = '".$resultHotelDetail->state."'");
...
$resCatspecial = executeSql("SELECT a.* ,b.*,c.room_id,c.hotel_id,c.display_order,c.status FROM `fs_rate` as a join `fs_rate_details` as b left join `fs_assign_hotel_room` as c ON c.hotel_id=b.hotel_id and c.room_id=b.room_id where b.hotel_id='".addslashes($resultHotelDetail->id)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."' AND b.detail_status=1 AND a.id=b.rate_id AND b.rate_id='".$Newrate_id."' order by c.display_order asc ");			
$specialrow	=	num_rows($resCatspecial);
if($specialrow>0){
	$content	.='<table width="100%" style="border:1px solid #000;">	
			<tr width="100%" style="border:1px solid #fff;">
<td colspan="4" style="float: left!important;vertical-align: top !important; width:250px;border-bottom:1px; border-right:0px;">
...
<td colspan="4" style="float: left!important;vertical-align: top !important; width:250px;border-bottom:1px; border-right:0px;">
		<table border="0px ;" width="100%">
			...
			<th class="fitwidth" style="text-align:left;vertical-align: left !important;color:#fff;"><b>'.strtoupper($resultHotelDetail->city).' - </b>			
	...
	$content	.='<td colspan="9" style="vertical-align: top !important; margin:0px !important;"><table class="table" style="margin-bottom:0px">';
...
$resCat_rooms = executeSql("SELECT a.* ,b.*,c.room_id,c.hotel_id,c.display_order,c.status FROM `fs_rate` as a join  `".TBL_RATE_DETAILS_SPECIAL."` as b left join `fs_assign_hotel_room` as c ON c.hotel_id=b.hotel_id and c.room_id=b.room_id where b.hotel_id='".addslashes($resultHotelDetail->id)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."' AND b.detail_status=1 AND a.id=b.rate_id AND b.rate_id='".$Newrate_id."' order by b.start_date asc  ");			
...
*/
  ...
  //$resCat_rooms = executeSql("SELECT *  from `fs_rate_details` where status='1' and hotel_id=".addslashes($resultHotelDetail->id));
			...
			while($rowInclusion = $db->fetch_object2($resCat_rooms)){
				//echo TBL_RATE_DETAILS_SPECIAL.'count(special_group)'." WHERE `rate_id` = '".$rowInclusion->rate_id."' and special_group = '".$rowInclusion->special_group;
				$Lable++;
				
				if($rowInclusion->special_group !=$special_group){
				$special_groupcount	=	selectColumn(TBL_RATE_DETAILS_SPECIAL,'count(id)'," WHERE `rate_id` = '".$rowInclusion->rate_id."' AND  `start_date` = '".$rowInclusion->start_date."' and special_group = '".$rowInclusion->special_group."'");
				
				$groupcount=1;
				$special_rate_name	=	selectColumn('fs_event_calender','name'," WHERE `id` = '".$rowInclusion->special_rate_name."'");
				$content .='
				<tr style="text-align:left;vertical-align:middle;background-color:#3c8dbc;color:#fff !important;">
				<th width="90px;" colspan="6" style="text-align:left;vertical-align:middle;"><b>RATE FOR DATES  '.strtoupper($special_rate_name).' FROM '.date('d-m-Y',strtotime($rowInclusion->start_date)).' To '.date('d-m-Y',strtotime($rowInclusion->end_date)).'</b></th>																		
				</tr>';
				
			}
				$special_group=$rowInclusion->special_group;									
				if($Lable	==1){
...
$content .='<th width="150px;" style="font-weight: 100">'.$HotelBLD.'</th>
			...
			//$content .='<tr><td colspan="6">'.$rowInclusion->hotel_remarks.'</td></tr>';
		
		if($groupcount==$special_groupcount){	
			$groupcount=0;
		   ...
		   $content .='<tr><td colspan="6">'.$rowInclusion->hotel_remarks.'</td></tr>';
			...
			$contenthotel_remarks='';
			}*/
		//$content .=$contenthotel_remarks;
		//$groupcount++;*/
		$groupcount++;
	  }
...
$content .=	'
   		 <table class="table"  style="margin-bottom: 0px;border: 1px; width:100%"><tr><td colspan="13">'.$additional_points;
$content .='</td></tr></table>';
...
if($description!=''){
$content .=	'
   		 <table class="table"  style=" margin-bottom: 0px;border: 1px; width:100%"><tr><td colspan="13" style=" color:#000;">'.$description;
$content .='</td></tr></table>';
}
	}
 } 
...
$content .='<table class="table page_break0"      style="page-break-before:always; margin-bottom: 0px;border: 1px; width:500px!important;font-weight:normal !important;"></table>';
...
$content.=strip_tags($Desc,"<style><header><br><b><div><table><strong><tr><td><p><span><img><style><head><html><body><ul><li><br><ol><link><strong><font>");
