...
$resHotelDetail =  executeSQl("SELECT * FROM `".TBL_HOTELS."` where status='1' ".$sql."  ORDER BY `state`");
...
$content .= '
			...
			<th colspan="10" ><b>'.$rowShop->name.'-'.selectColumn(TBL_RATE_MARKET,'name'," WHERE `id` = '".$RateTitle->market."'").'-'.selectColumn(TBL_RATE_SEASON,'name'," WHERE `id` = '".$RateTitle->seasonId."'").'</b></th>
			</tr>
						<tr  style="background-color:#4f6228;color:#fff;">
						   ...
						   $content .= 'Template Rate';}else {
							   $content .= selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$RateTitle->company_id."'"); 
							   $content .= "( GST: ".selectColumn(TBL_COMPANY,'fax'," WHERE `id_company` = '".$RateTitle->company_id."'"); 
							  
							   $CreditValue =	selectColumn(TBL_COMPANY,'company_credibility'," WHERE `id_company` = '".$RateTitle->company_id."'");
							   if($CreditValue == '1'){ 
							   $content .= " - Credit Allowed )";
							   }
							   if($CreditValue == '2'){ 
							   $content .= " - Credit Not Allowed )";
							   }
			     ...
			     $content .= '</b></th>
				 <th colspan="3" align="left"><b>Ref:';
						    $content .=  $RateTitle->rate_name.' Date:'.date("d-m-Y"); 
$resContact = executeSql("SELECT * from `".TBL_CUSTOMER."` where status='1' and id_customer='".addslashes($RateTitle->company_id)."' and type='2'");
...
$BookerName	=	$rowContact->first_name." ".$rowContact->last_name." ".$rowContact->email;			   
			     ...
			     $content .= '</b></th>
						...
						<tr style="background-color:#4f6228;color:#fff;">
						   <th colspan="3" align="left"><b>RATES ISSUED TO: '.$BookerName.'</b></th>
						   <th colspan="7" align="left"><b>RATES ISSUED BY:';
...
while($resultHotelDetail = $db->fetch_object2($resHotelDetail)){ 
		...
		while($RateHotelDetail = $db->fetch_object2($resCat_rooms)){
		 ...
		 $current = $resultHotelDetail->state;
		if ($current != $previous) {
  $content	.='<tr align="middle" style="background-color:#75923c;color:#fff">
				   <th width="15%" colspan="10"><b>'.selectColumn(TBL_STATE,'name'," WHERE `id_state` = '".$resultHotelDetail->state."'").'</b></th>
				</tr>';
}
...
$previous = $current;
		 ...
		 $SingleAndWeekday	='<th width="25%"><b>WEEKDAYS</b></th>
						<th width="25%"><b>WEEKENDS(FRI/SAT)</b></th>';
		 	
				
		  }else{
			  
			  		 $SingleAndWeekday	='<th width="25%"><b>SINGLE</b></th>
						<th width="25%"><b>DOUBLE</b></th>';
			  
			  
			  }
						
$content	.='<tr style="text-align:left;vertical-align:top;background-color:#c2d69a;">
<th class="fitwidth" style="text-align:left;vertical-align:top;width:80px !important;"><b>'.$resultHotelDetail->city.'</b></th>

<th class="fitwidth" style="text-align:left;vertical-align:top;width:80px !important;"><b>'.$resultHotelDetail->name.'</b></th>
<th class="fitwidth" style="text-align:left;vertical-align:top; width:80px !important;"><b>'.$rowRoomCount->roomcount.' Rooms</b></th>
						...
						$content	.='<th><b>EXTRA BED</b></th>	   
						<th><b>INCLUSIONS</b></th>
						<th><b>MEALS/TAXES</b></th>
</tr>
<tr style="text-align:left;vertical-align:top;">
<th class="fitwidth" style="text-align:left;vertical-align:top;width:80px !important;font-weight: 100">'.$resultHotelDetail->address.'</th>

				<th class="fitwidth" style="text-align:left;vertical-align:top;width:80px !important;font-weight: 100">'.$resultHotelDetail->special_notes.'</th>
				<th class="fitwidth" style="text-align:left;vertical-align:top; width:80px !important;"><b></b></th>
				
				<th width="25%" colspan="7">				
					<table class="table" border="1px solid red;font-weight:normal">
				';
...
$resCat_rooms = executeSql("SELECT * FROM `".TBL_RATE."` as a join `".TBL_RATE_DETAILS."` as b where  hotel_id='".addslashes($resultHotelDetail->id)."' AND a.`id_shop` = '".addslashes($_SESSION['shop'])."'  AND b.detail_status=1 AND a.id=b.rate_id AND b.rate_id='".$Newrate_id."'");
  ...
  //$resCat_rooms = executeSql("SELECT *  from `fs_rate_details` where status='1' and hotel_id=".addslashes($resultHotelDetail->id));
			...
			$content .='<tr>
			<th width="20%" style="font-weight: 100">'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = ".$rowInclusion->room_id).'</th>
			<th width="15%" style="font-weight: 100">'.selectColumn(TBL_RATE_PLAN,'name'," WHERE `id` = ".$rowInclusion->rate_plan_id).'</th>';
...
<th style="font-weight: 100">'.$extra_bed_price.'</th>
<th style="font-weight: 100">'.selectColumn(TBL_RATE_PLAN,'remarks'," WHERE `id` = ".$rowInclusion->rate_plan_id).'</th>';
...
$content .='<th width="25%" style="font-weight: 100">'.$BreakFast.$lunch.$Dinner.$TaxVal.'</th>
			</tr>';
...
$content .=	'
   		 <tr><td colspan="10">'.$additional_points;
$content .='</td></tr>';
