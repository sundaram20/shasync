...
////////////////////Hotel Details///////////////////
$sqlRtl = "SELECT rtd.*,rt.* FROM ".TBL_RATE_UNIT." rt RIGHT JOIN 
			".TBL_RATE_DETAILS_UNIT." rtd ON rt.id=rtd.rate_id 
		WHERE rt.id= '".$id_rate."' AND rt.id_shop='".$_SESSION['shop']."' ";
...
$weekdayAllowed=selectColumn(TBL_HOTELS,'excel_display_weekday','WHERE id="'.$objRtl->hotel_id.'" ');
...
$sqlRoom = "SELECT A.breakfast_price,A.lunch_price,A.dinner_price,A.extra_bed,A.room_id,A.single_pax_price,A.rate_plan_id,A.double_pax_price,A.weekend_single_pax_price,A.weekend_double_pax_price,A.detail_status FROM ".TBL_RATE_DETAILS_UNIT." A LEFT JOIN ".TBL_ASSIGN_HOTEL_ROOM." B ON A.room_id=B.room_id AND B.hotel_id='".$objRtl->hotel_id."' WHERE A.rate_id='".$rowData->id."'  ORDER BY B.display_order ASC ";
...
$content .= "<table style='margin-top:30px;'>
				<tr><td>Date ".date('d/m/Y')."<br/><br/></td></tr>
				
			</table>";	

$content .= "<table >
				<tr><td><b>".$customer."</b></td></tr>
				<tr><td>".$designation."<br/><br/></td></tr>
				<tr><td><b>".$custCompany."</b></td></tr>
				".($addressComp!=''?'<tr><td>'.wordwrap($addressComp,20,"<br>\n").'</td></tr>':'')."
				".($cityComp!=''?'<tr><td>'.$cityComp.'</td></tr>':'')."
				".($phoneComp!=''?'<tr><td>Contact Number: '.$phoneComp.'</td></tr>':'')."
				".($emailComp!=''?'<tr><td>Email: '.$emailComp.'</td></tr>':'')."
			</table>";			

$content .= "<table style='margin-top:30px;text-align:justify;'>
				<tr>
					<td>
						<b> Dear ".$customerLastName.",</b><br><br>
						Greetings from <b>".$hotelWithCity."</b>, a member of ".ucwords(strtolower(str_replace(':','',$shopName))).", India’s leading environmentally sensitive hotel chain.<br><br>

						We look forward to welcoming your clients and guests for their business, leisure and corporate MICE stays at our hotel.<br><br>
					</td>
				</tr>
			</table>";
...
$content .= "<table style='margin-bottom:-50px;text-align:justify;'>
				<tr>
					<td>
						With reference to our discussions, we are pleased to offer your company <b>Special Rates</b> at our hotel for ".ucfirst(strtolower($season)).", as per the annexures attached. Kindly acknowledge your acceptance within 7 days for us to extend these special rates to your company.<br><br>

						You may make your room reservations by calling or emailing me or our Reservations team directly, by contacting our Central Reservations on ".$crsBrandPhone." / ".$emailBrand."<br><br>

						We thank you for your support and value your patronage, and assure you of our best services and personalized attention at all times.<br><br>
...
$content .= "<table style='width:100%;margin-top:30px;'>
				<tr>
					<td style='width:50%;'>
						<b>".$sqlUserDetail->name."</b><br/>
                  		<i>".$handeledByDesignation."</i><br/>
                           <b>".$companyNameUser."</b><br/>
                          ".($address1!=''?$address1.', ':'').($address2!=''?$address2.' ':'').'<br/>'.$city.'-'.$zip."<br/>
                          M: ".$sqlUserDetail->mobile." <br/ >Email : ".$sqlUserDetail->email."
					<br><br></td>
				</tr>
			</table>";
//start 
/*$content .= "<table style='page-break-after:always;border-top:1px solid black;border-bottom:1px solid black;width:100%;margin-top:150px;'>
				<tr>
					<td style='width:50%;text-align:left;'>
						".$handeledByName." (".$handeledByDesignation."),".$handeledByMobile." ".$handeledByEmail."
						- Hotel Reservations: ".$crsPhone." ".$crsEmail." 
					</td>
				</tr>
			</table>";	*/
//end			
$content .="<table style='page-break-before:always;margin-top:50px;'><tr><td>YOUR PREFERRED CORPORATE RATES:</td></tr></table>";
$content .= "<table cellspacing='0' cellpadding='0' border='1px' style='text-align:center;width:100%;margin-top:30px;'>
				<tr style='border:1px solid black;background-color:#272727;color:white;'>
					<td colspan='5'  >
						".$hotelWithCity."<br>
						Rates validity: ".date('d-F-Y',strtotime($rowData->start_date))." – ".date('d-F-Y',strtotime($rowData->end_date))."
					</td>
				</tr>
				<tr style='border:1px solid black;background-color:#595959;color:white;'>
					<td rowspan='2' width='26%'><b>Room Type</b></td>
					<td colspan='4' rowspan='1'>Preferred Corporate Rates</td>
				</tr>
...
if($room->dinner_price==0){
		    $empty='-';
		}
		$content .= "<tr style='border:0.5px solid black;background-color:#F9F9F9;color:black;'>
					<td style='text-align:left'>".selectColumn(TBL_ROOM_TYPE,'name','WHERE id="'.$room->room_id.'" ')."</td>
					<td >".selectColumn(TBL_RATE_PLAN,'name','WHERE id="'.$room->rate_plan_id.'" ')."</td>
					<td >".$singlePriceTxt."</td>
					<td >".$doublePriceTxt."</td>
					<td >".($room->lunch_price!=0?"L: INR ".$room->lunch_price.' + TAXES':'').($room->dinner_price!=0?" | D : INR ".$room->dinner_price.' + TAXES':'').$empty." </td>
					
				</tr>";
	}
}	
$content .= "<tr style='border:0.5px solid black;background-color:#F9F9F9;color:black;'>
					<td style='text-align:left;'>Extra Person/Extra Bed</td>
					<td colspan='4' style='text-align:left;'>".($objRtl->extra_bed_price==0?'Rate on request':'INR '.$objRtl->extra_bed_price)."</td>
					
				</tr>";

$content .= "<tr style='border:0.5px solid black;background-color:#F9F9F9;color:black;'>
					<td style='text-align:left;'>Note: </td>
					<td style='text-align:left;' colspan='4'>".$objRtl->hotel_remarks."</td>
					
				</tr>";

$content .= "</table>";

//if($objRtl->hotel_remarks!='')
//$content .="<table style='margin-top:20px;margin-bottom:20px;'><tr><td><b>Inclusions:</b> ".$objRtl->hotel_remarks."</td></tr></table>";
...
/*$content .= "<footer>
				Corporate Office: ".$addressBrand." T:".$phoneBrand."
				&nbsp; &nbsp; F:".$faxBrand." &nbsp; &nbsp; Web:".$webBrand."
			</footer>";*/
...
$dompdf->set_paper('potarit', 'potarit');
$dompdf->load_html(strip_tags($content,"<style><header><br><b><div><table><strong><tr><td><p><span><img><ul><li><ol><link><strong>"));
...
$dompdf->get_canvas()->page_text(550, 18, "Page: {PAGE_NUM} of {PAGE_COUNT}", $font, 6, array(0,0,0));

$market = selectColumn(TBL_RATE_MARKET,'name','WHERE id="'.$rowData->rate_level_id.'" ');
