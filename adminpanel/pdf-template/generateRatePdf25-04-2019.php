...
$content .= '<table class="table" style=" margin-bottom: 0px;border: 0px;  ">
						<tr>					
						  <th>
						  <img src="http://crs.roomstatushub.com/uploaded_files/shop/'.$logo.'" class="img-responsive" alt="logo" title="logo"   />
						  ';
$content .= '			 </th>
				</tr>
			</table>
...
$content .= '<tr style="'.$BackgroundColor.'color:#fff !important;font-size:16px !important;">
			<th colspan="13" ><b>'.$rowShop->name.' '.selectColumn(TBL_RATE_MARKET,'name'," WHERE `id` = '".$RateTitle->market."'").'-'.selectColumn(TBL_RATE_SEASON,'name'," WHERE `id` = '".$RateTitle->seasonId."'").'</b></th>
			</tr>
...
if($DesignationBy !=''){
	$DesignationByName	=	' / '.$DesignationBy;
			     ...
			     $content .= '</b></th>
						...
						<tr style="background-color:#4f6228;color:#fff !important;font-size:11px !important">
						   <th colspan="6" align="left"><b>RATES ISSUED TO: &nbsp;'.strtoupper($rowContact->title).' '.strtoupper($BookerName).$DesignationToName.'</b></th>
						   ...
						   $content .= '</b></th>
						   <th colspan="3" align="left"><b>DATE:&nbsp; '.date("d-m-Y").'</b></th>
						   </tr></table>';   
...
*/
  ...
  //$resCat_rooms = executeSql("SELECT *  from `fs_rate_details` where status='1' and hotel_id=".addslashes($resultHotelDetail->id));
			...
			<th width="15.5%" style="font-weight: 100">'.selectColumn(TBL_ROOM_TYPE,'name'," WHERE `id` = ".$rowInclusion->room_id).'</th>
