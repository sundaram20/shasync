     ...
     $DateNoDays = abs(round($diff / 86400)); 
            ...
            <div class="col-md-5" >
              <div class="form-group">
                <label for="start_date">Lead For Hotel<font style="color:red;">*</font></label>
                <?php 
                     	if($row->hotel_id==0){
            ...
            <div class="form-group col-md-5">
              ...
              <?php
			           ...
			           $categoryDropDown = '<select  class="form-control select2 " onchange="getExecutiveName(this.value,\'\');" name="id_company" id="id_company" data-parsley-errors-container="#idcompanyError"  data-parsley-required>
            ...
            <div class="form-group col-md-12">
              <label for="email" >Details<font style="color:red;">*</font></label>
              <textarea class="form-control" name="discussion_summary" data-parsley-required id="en_discussion_summary"    rows="2" placeholder="Enter Discussion Summary" automcomplete="off" <?php echo $TextAreaReadOnly; ?>><?php if($_POST) echo $_POST['details'];else echo stripslashes($row->details);?>
                </textarea>
            </div>
        ...
        <div class="form-group  col-md-3">
                  <label for="name">Expected Revenue (In Lacs)</label>
...
?>
          ...
          <?php 
            ...
            <div class="col-sm-3">
              <div class="form-group">
                <label for="image" style="float:left;">Incentive &nbsp;&nbsp; </label>
              </div>
            </div>
          </div>
          <div class="box">
                ...
                <table id="example2" class="table table-bordered table-striped">
                  <thead>
                    <tr> 
                      ...
                      <th>Forward To</th>
                      <th>Action Date</th>
                      <th>Incentive Internal Remarks</th>
                      <th>Status</th>
                      <th>Action</th>
                    </tr>
                  </thead>
...
$forward_for_approval=selectColumn(TBL_USERS,'name'," WHERE `id` = '".$FollowupSqlRow['id_forward_for_approval']."' ");
	if($status==1){
		...
		if($FollowupSqlRow['lead_status'] == 1){
			    ...
			    $IncentiveUpdate = "'".$revenue."','".$FollowupSqlRow['id_forward_for_approval']."','".$FollowupSqlRow['id_user']."','".$FollowupSqlRow['id_incentive']."','".$id_user_created."','".$id_hotel_created."'";
				$DateVisitList	='<tr>';
...
<script type="text/javascript">
	...
	function OpenIncentivePopup(approved_amount,id_forward_for_approval,id_user,id_incentive,id_user_created,id_hotel_created){ //Incentive
		$('#approved_amount').val(approved_amount);
		$('#claim_amount').val(approved_amount);
	...
	var checkbox = document.getElementById('ClaimIncentive');
		if (checkbox.checked == true)
		{
			...
			$.ajax({
				type: "POST",
				url: 'ajax/ajaxIncentiveAddEditForm.php',
				data: 'selectType=1'+'&id='+id+'&id_hotel_md='+id_hotel_md+'&enquiryDate='+enquiryDate, 
				success: function (result) {
