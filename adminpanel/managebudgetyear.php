...
if($_REQUEST['action'] == 'change'){
	...
	if($_REQUEST['activeId'] != ''){
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_BUDGET_YEAR,'activate');

		$statusId = addslashes(encryptor('decrypt',$_REQUEST['activeId']));

		$statusSql = "	UPDATE `".TBL_BUDGET_YEAR."`
		...
		checkUserLevelPermission($_SESSION['userLevel'],TBL_BUDGET_YEAR,'deactivate');
	...
	if(executeSql($statusSql)){
		...
		$_SESSION['successMsg'] = 'Budget Year '.selectColumn(TBL_BUDGET_YEAR,'name'," WHERE `id` = '".$statusId."'").' status has been changed sucessfully.';

	}else{

		$err = 1;

		$_SESSION['errorMsg'] = 'Budget Year '.selectColumn(TBL_BUDGET_YEAR,'name'," WHERE `id` = '".$statusId."'").' status has not been changed sucessfully.';

	}

}else if($_REQUEST['action'] == 'delete' && $_REQUEST['delId'] != ''){

	checkUserLevelPermission($_SESSION['userLevel'],TBL_BUDGET_YEAR,'delete');

	$delSql = "DELETE FROM `".TBL_BUDGET_YEAR."` WHERE `id` = '".$_REQUEST['delId']."'";

	$sqlDelUserLevel = selectRow(TBL_BUDGET_YEAR," WHERE `id` = '".$_REQUEST['delId']."'");

	if(executeSql($delSql)){		

		$err = 0;

		$_SESSION['successMsg'] = 'One Budget Year '.$sqlDelUserLevel["name"].' has been deleted sucessfully.';

	}else{

		$err = 1;

		$_SESSION['errorMsg'] = 'Unable to delete Budget Year '.$sqlDelUserLevel["name"];
...
if($_REQUEST["act"] == "activate" && !empty($_REQUEST['ids'])){
	...
	$delSql = "DELETE FROM `".TBL_BUDGET_YEAR."` WHERE `id` IN (".addslashes($deleteIds).")";
...
$sql = " SELECT * FROM `".TBL_BUDGET_YEAR."` WHERE `id_shop` = '".addslashes($_SESSION['shop'])."' ";
	 ...
	 <div class="form-group has-error" align="center">
        ...
        <div class="box-header with-border">
		  ...
		  <div class="btn-group  pull-right">
							  ...
							  <a type="button" class="btn btn-success" href="editbudgetyear.php" >Add Budget Yaer</a>
							  ...
							  <ul class="dropdown-menu" role="menu">
								...
								<?php /*?><li><a title="Export to excel file" href="exportTable.php?fileType=xls&tableName=<?php echo TBL_BUDGET_YEAR;?>"><img src="images/excel-icon.jpg" width="20" height="20" />&nbsp;Export</a></li>

								<li><a title="Export to csv file" href="exportTable.php?fileType=csv&tableName=<?php echo TBL_BUDGET_YEAR;?>"><img  src="images/excel-csv-icon.jpg" width="20" height="20"  />&nbsp;Export</a></li><?php */?>
        ...
        <div class="col-xs-12">		     
          ...
          <div class="box">
            ...
            <div class="box-header">
              ...
              <h3 class="box-title">Budget Year List</h3>
            ...
            <div class="box-body table-responsive">
              ...
              <table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?=$row->status=='1'?'<span onclick="location.href=\'managebudgetyear.php?inactiveId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'" style="color:green;cursor:pointer;">Active</span>':'<span onclick="location.href=\'managebudgetyear.php?activeId='.encryptor('encrypt',$row->id).'&action=change&page='.$_REQUEST['page'].'\'"  style="color:red;cursor:pointer;">Inactive</span>';?>&nbsp;</td>			 

				  <td><img src="images/view_edit.gif" style="cursor:pointer;" title=" View / Edit " onClick="window.location.href='editbudgetyear.php?eId=<?=encryptor('encrypt',$row->id)?>&action=edit&page=<?=$_REQUEST['page']?>';" />&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/delete.gif" style="cursor:pointer;" title="Delete" name="<?php echo $row->name; ?>" id="<?php echo $row->id;?>" onClick="deleteMe(this.id,this.name);"/></td>
  		    ...
  		    if (this.readyState == 4 && this.status == 200) {
  		      ...
  		      if(this.responseText == 1){
  		      	...
  		      	if(confirm('Are you sure that you want to delete this record '+name+'?')){
  		      		window.location.href='managebudgetyear.php?delId='+id+'&action=delete&page=<?=$_REQUEST['page']?>';
  		      	}
  		      }
  		    }
  		  };
  		  xhttp.open("GET", "ajax/ajaxCheckCompanyDomain.php?id_bgt_yr="+id, true);
  		  xhttp.send();
  	}
  </script> 
