...
$setContent = selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND id="30" AND type=2');
if($setContent!=''){
/*$content .= "<table>
				<tr>
		...
		$content .=selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND id="30" AND type=2');	
}
...
$content .= "<table class='table table-striped text-center' style='border-spacing:0;width:40%;text-align:center;'>
  <tbody>
    <tr>
      <td style='border:1px solid #252525;'><b>Product Name </b></td>

     
    </tr>";
	 $sqlQuery = "SELECT id_product FROM `daily_pickup_details` WHERE id_daily_pickup='".$row->id."'  ";
	
$resw = mysqli_query($connNew,$sqlQuery);


	while($roww = mysqli_fetch_object($resw)){	
											
		$ProductName	=	selectColumn(TBL_HOTELS,'name','WHERE  id="'.$roww->id_product.'" ');
	$content .= "<tr>
        <td style='border:1px solid #252525;'>".$ProductName."</td>
...
$content .= " <table style='font-family:Georgia !imporant;font-size:8pt !imporant;'>
                <tr style='font-family:Georgia !imporant;font-size:8pt !imporant;' ><td style='font-family:Georgia !imporant;font-size:8pt !imporant;'><span style='font-family:Georgia !imporant;font-size:8pt !imporant;'>".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND type=3 AND id="31" ')."</span></td></tr></table>";
...
if($sent){
	...
	$updateDaily = "UPDATE daily_pickup SET count_thank_you=count_thank_you+1 WHERE id='".encryptor('decrypt',$_REQUEST['id'])."' ";
        ...
        <div class="box box-primary">
          ...
          <form action="" method="post" enctype="multipart/form-data" autocomplete="off" data-parsley-validate>
            <div class="box-body">
              ...
              <div class="form-group">
                <lable>Subject</lable>
                <input class="form-control" placeholder="Subject:" name="subject" value="<?php echo " THANK YOU  ".strtoupper($season) ?> ||  <?php echo strtoupper(selectColumn(TBL_SHOP,'name','WHERE id="'.$_SESSION['shop'].'" '));?>" data-parsley-required >
              </div>
              <div class="form-group">
                <label>Content</label>
