...
if($setContent!=''){
$content .= selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND id="25" AND type=2');
}
...
$content .= "<table class='table table-striped text-center' style='border-spacing:0;width:40%;text-align:center;'>
  <tbody>
    <tr>
      <td style='border:1px solid #252525;'><b>Date </b></td>
      <td style='border:1px solid #252525;'><b>Invoice No</b> </td>
      <td style='border:1px solid #252525;'><b>Pending Amount</b></td>
     
    </tr><tr>
        <td style='border:1px solid #252525;'>".date('d-m-Y',strtotime($row->doc_date))."</td>
      <td style='border:1px solid #252525;'>".$row->bill_no."</td>
      <td style='border:1px solid #252525;'>".$TotalValue	."</td>
...
$content .= " <table style='font-family:Georgia !imporant;font-size:8pt !imporant;'>
                <tr style='font-family:Georgia !imporant;font-size:8pt !imporant;' ><td style='font-family:Georgia !imporant;font-size:8pt !imporant;'><span style='font-family:Georgia !imporant;font-size:8pt !imporant;'>".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND type=3 AND id="29" ')."</span></td></tr></table>";
...
$mail->Debugoutput = function($str, $level) {
    error_log("PHPMailer: $str", 3, $_SERVER['DOCUMENT_ROOT']."/logs/mail.log");
};

if($sent){
	
	$updateDaily = "UPDATE daily_pickup SET count_payment_reminder=count_payment_reminder+1 WHERE id='".encryptor('decrypt',$_REQUEST['id'])."' ";
          ...
          <div class="box box-primary">
           ...
           <?php /*?> <img id="loadAni" src="loadingAni.gif"><?php */?>
            ...
            <div class="box-body">
              ...
              <div class="form-group">
            <lable>Subject</lable>
                <input class="form-control" placeholder="Subject:" name="subject" value="<?php echo " PAYMENT REMINDER  ".strtoupper($season) ?> ||  <?php echo strtoupper(selectColumn(TBL_SHOP,'name','WHERE id="'.$_SESSION['shop'].'" '));?>" data-parsley-required >
