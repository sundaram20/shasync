...
if($rowOrderDetail->id_company_person>0){  
  ...
  Greetings!!!&nbsp;<br />
  <br />
  This is in reference to your booking from '.dateformat_date($rowOrderDetail->checkin).' to '.dateformat_date($rowOrderDetail->checkout).' at '.$resultHotelDetail->name.', '.$resultHotelDetail->city.' wherein the '.selectColumn(TBL_ORDER_STATE,'template'," WHERE `id_order_state` = '".addslashes($rowOrderDetail->payment_status)."'").' is due by '.dateformat_date($rowOrderDetail->payment_date).' .&nbsp;<br />
