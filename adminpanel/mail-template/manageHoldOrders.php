...
$reminderDate = explode(' to ',$_REQUEST['reminder_date'] );
	...
	$sql .= " AND `".TBL_ORDERS."`.`payment_date` between '".date('Y-m-d',strtotime($reminderDate[0]) )."' and '".date('Y-m-d',strtotime($reminderDate[1]) )."'";

}else{
	$sql .= " AND `".TBL_ORDERS."`.`payment_date` between '".date('Y-m-d')."' and '".date('Y-m-d',(strtotime ( '+6 day' ,strtotime(date('d-m-Y')) ) ))."'";
}

if($_REQUEST['reminder_date'] !=''){
$reminderDate = explode(' to ',$_REQUEST['reminder_date'] );

	$link = " AND `".TBL_ORDERS."`.`payment_date` between '".date('Y-m-d',strtotime($reminderDate[0]) )."' and '".date('Y-m-d',strtotime($reminderDate[1]) )."'";

}else{
	$link = " AND `".TBL_ORDERS."`.`payment_date` between '".date('Y-m-d')."' and '".date('Y-m-d',(strtotime ( '+6 day' ,strtotime(date('d-m-Y')) ) ))."'";
}
