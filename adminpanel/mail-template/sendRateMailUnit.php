...
if($res){
  ...
  $market = selectColumn(TBL_RATE_MARKET,'name','WHERE id="'.$row->market.'" ');
...
$sqlUserDetail = $db->fetch_obj2(selectSql(TBL_USERS,"WHERE `id` = '".$id_user."'",''));
...
$handeledby = $sqlUserDetail->name;

$handeledbyEmail = $sqlUserDetail->email;

if($_SESSION['shop']==6){
    $HospitalityWithResponsibility  =" #HospitalityWithResponsibility  <br>";
}else{
    $HospitalityWithResponsibility  =" ";
}
...
"                .$HospitalityWithResponsibility.' '.$address1.','.$address2.','.$city.'-'.$zip."<br>
...
$sent =  $sendMail->sendRateMail('support@roomstatushub.com', $_POST['mailId'], $_POST['subject'],$_POST['sendcontent'],$_POST['ccId'],$_SERVER['DOCUMENT_ROOT']."/sync/adminpanel/mailattachunit/".$file_name,ucwords($handeledby),$handeledbyEmail);
...
if($sent){
 ...
 $updateSql = 'UPDATE '.TBL_RATE_UNIT.' SET id_mail_sent_by="'.$_SESSION['userId'].'" , mail_sent_at="'.date('Y-m-d H:i:s').'" WHERE id="'.encryptor('decrypt',$_REQUEST['id']).'" ';
  
  mysqli_query($conn,$updateSql);
  

  unlink($_SERVER['DOCUMENT_ROOT']."/sync/adminpanel/mailattachunit/".$file_name);
