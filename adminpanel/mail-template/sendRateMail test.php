...
$content .= "<table >
          ...
          font-size:10pt !important; '>
                ".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND type=2')."
            </td>
				</tr>
			</table>";
...
$content.="<table>
              <tr>
                  <td class='forTd' ><b>".ucwords($handeledby)."</b></td>
...
$content .= "<table style='width : 100%;'>
        <tr><td></td></tr>
            ...
            <td ><span style='color:green;font-weight:bold;'>".$companyNameUser."</span><span style='font-family:Georgia !imporant;font-size:9pt !imporant;'><pre><br>".$formalCompanyName."<br>
                ".$address1.','.$address2.','.$city.''.$zip."<br>
                M: ".$mobile." | T: ".$phone." | Email : ".$handeledbyEmail."</pre></span>               
            </td>
        </tr>
      </table>";

$content .= "<table>
                <tr><td><span style='font-family:Georgia !imporant;font-size:8pt !imporant;'><pre>".selectColumn(TBL_RATE_MAIL_FORMAT,'description','WHERE id_shop="'.$_SESSION['shop'].'" AND type=3')."</pre></span></td></tr></table>";
...
$file_name = $company.'-'.$season.'.pdf';
$sent =  $sendMail->sendRateMail('support@roomstatushub.com', $_POST['mailId'], $_POST['subject'],$_POST['sendcontent'],$_POST['ccId'],"../mailattach/".$file_name,ucwords($handeledby),$handeledbyEmail);

if($sent){
  $message = "Mail Sent Successfully";
  unlink("../mailattach/".$file_name);
}
else{
