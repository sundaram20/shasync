        ...
        <div class="box-body">
          <div class="row">
            <div class="form-group col-md-3">
			  ...
			  if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){
				  $report_date	= stripslashes(date('d-m-Y',strtotime($row->invoice_date))); 
				  }else{  
				  $report_date	=	date('d-m-Y');}	
// Calulating the difference in timestamps 
$date1	=date('d-m-Y');
    $diff = strtotime($row->invoice_date) - strtotime($date1); 
  ...
  <input type="text" class="form-control" placeholder="Enter Invoice No" id="invoice_no" name="invoice_no" value="<?php echo  $row->invoice_no; ?>"  data-parsley-required>
           ...
           <div class="form-group col-md-3">
             ...
             <select  class="form-control select2 itemName" name="id_company" id="id_company" data-parsley-errors-container="#idcompanyError"  data-parsley-required>
              ...
              <label for="invoiceno">Account Head:</label>
              <input type="text" class="form-control " placeholder="Enter Contact Person " id="contact_person" name="contact_person" value="<?php echo  $row->contact_person; ?>"  data-parsley-required>
              <?php echo $err_contact_person;?> </div>
              
              
              <div class="form-group col-md-3">
              

              <label for="invoiceno">Account Head Email:</label>
              <input type="text" class="form-control " placeholder="Enter Contact Email " id="contact_email" name="contact_email" value="<?php echo  $row->contact_email; ?>"  data-parsley-required>
              <?php echo $err_contact_email;?> </div>
              
              <div class="form-group col-md-3">
              

              <label for="invoiceno">Account Head Mobile:</label>
              <input type="text" class="form-control " placeholder="Enter Contact Mobile " id="contact_mobile" name="contact_mobile" value="<?php echo  $row->contact_mobile; ?>"  data-parsley-required>
              <?php echo $err_contact_mobile;?> </div>
           
                         
                            
              <div class="form-group col-sm-3">
            <label for="id_guest" >Guest Name</label>
             <input type="text" class="form-control " placeholder="Enter Guest Name " id="guest_name" name="guest_name" value="<?php echo  $row->guest_name; ?>"  data-parsley-required>
          ...
          <div class="form-group col-md-3">
              ...
              <input type="text" class="form-control pickerdate_addreport" placeholder="Enter Expected check in date" id="check_in_date" name="check_in_date" value="<?php  if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){ echo date('d-m-Y',strtotime($row->checkin)); }else{ echo date('d-m-Y'); }?>">
              <?php echo $err_check_in_date;?> 
        </div>
        
        
          <div class="form-group col-md-3">
              <label for="start_date">Check out:</label>
              <input type="text" class="form-control pickerdate_addreport" placeholder="Enter Expected check in date" id="check_out_date" name="check_out_date" value="<?php if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){ echo date('d-m-Y',strtotime($row->checkout)); }else{ echo date('d-m-Y'); }?>">
              <?php echo $err_check_in_date;?> 
        </div>     
             ...
             <input type="hidden" class="form-control " placeholder="Enter Balance" id="lead_followup_status" name="lead_followup_status" value="1"  >
              <?php echo $err_balance;?> </div>
			...
			<div class="form-group col-md-3">
              ...
              <input type="text" class="form-control pickerdate_addreport" placeholder="Enter Due Date" id="duedate" name="duedate" value="<?php if(!empty($_REQUEST['eId']) && $_REQUEST['action']=='edit'){ echo date('d-m-Y',strtotime($row->due_date)); }else{ echo date('d-m-Y'); } ?>"  data-parsley-required>
              <?php echo $err_duedate;?> </div>
              
              <div class="form-group col-md-3">
              <label for="invoiceno">Sales Manager :</label>
              <input type="text" class="form-control " placeholder="Enter Manager Name" id="sales_manager" name="sales_manager" value="<?php echo  $row->sales_manager; ?>"  data-parsley-required>
              <?php echo $err_sales_manager;?> </div>
          ...
          <div class="row">
            <div class="col-sm-3">
              ...
              <?php $max_editid=selectColumn(INVOICE_DETAILS,'MAX(id)',"WHERE  enquiry_id = '".addslashes(encryptor('decrypt',$_REQUEST['eId']))."' ");
			  if($max_editid==0){
			  ?> 
          ...
          <div class="box">
            ...
            <div id="showFolowup">
              <div class="box-body table-responsive">
				...
				$DateVisitList .= '<td id="ChangeButton_'.$FollowupSqlRow['id'].'"><button  data="'.$FollowupSqlRow['id'].'" class="btn '.$StatusEs.'" type="button" onclick="OpenPopup('.$FollowupSqlRow['lead_status'].','.$FollowupSqlRow['id'].','.$FollowupSqlRow['enquiry_id'].','.$FollowupSqlRow['lead_status'].','.$followupInvoiceType.','.$row->amount.','.$row->received.','.$row->balance.');"    >'.$ActiveINactive.'</button>';   
...
?><div  id="showFolowup"></div>
    ...
    <div id="enquiry_failed" class="well">
      ...
      <br />
      <button class="enquiry_failed_close btn btn-default pull-right">OK</button>
    </div>
      ...
      <div id="cars2" class="desc" style="display: none;">
        <form id="ColseSummaryPopUpForm" class="ColseSummaryPopUpForm" data-parsley-validate autocomplete="off">
         ...
         <div class="form-group">
            ...
            <div class="form-group">
              <textarea   name="followup_close_summary" id="followup_close_summary" class="form-control" placeholder="Received Summary"  data-parsley-required automcomplete="off"></textarea>
            </div>
...
<script type="text/javascript">
	...
	$( document ).ajaxStop(function() {
	 ...
	 function chkStatus(value){
		...
		$("#lead_followup_status").val(value);
  	}
	function addEqyFollowUp(followup_status){
			var edit_id_enquiry	=	$("#edit_id_enquiry").val();
		$.ajax({
			type: "POST",
		    url: 'ajax/ajaxAddInvoicePaymentFollowUp.php',
  		    data: 'followup_status='+followup_status+'&edit_id_enquiry='+edit_id_enquiry, 
 		    success: function (result) {		
	...
	function savefollowupDate() { 
	  ...
	  if(form.parsley().validate()){
		...
		var nextFollowup = form.serialize(); 
		 $.ajax({
		   type: "GET",
		   url: 'ajax/ajaxInvoicefollowupCalanderUpdate.php',
		   data: nextFollowup,  
		   success: function (result) {
	...
	function OpenPopup(followup_status,followup_id,daily_Visit_id,hotel_id,followup_type,amount,received,balance){
	     ...
	     $( "#my_popup_no" ).hide();
			$( ".my_popup_open" ).click();	
				result = JSON.parse(data);
					...
					$('#SuccessMessageEmail').html('<div><b>Invoice Details sent to : </b></div><br/>'+result.msg+'<div><br/></div>');
			       $( "#FollowUpNextUpdate" ).html(result);
...
function saveIncentiveExistingLeadform(){	
	...
	function saveAddFollowupPopUpform(){
		...
		var new_received1	=	$("#new_received1").val();
		var new_balance1	=	$("#new_balance1").val();
		var new_amount1	=	$("#new_amount1").val();
		$('#amount').val(new_amount1);
		$('#balance').val(new_balance1);
		$('#received').val(new_received1);
		
		//alert(FollowupCoditionType);
		...
		if(form.parsley().validate()){
			...
			$.ajax({
			   type: "POST",
			   url: 'ajax/ajaxDateInvoiceFollowupList.php',
		   ...
		   success: function (result) {
			 ...
			 if(result==7){
				$( ".enquiry_failed_open" ).click();
				 $( "#FollowUpNextUpdateerror" ).html('Please assign Follow Ups to continue');
