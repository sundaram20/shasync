...
<?php include_once("./../includes/header.php")?>
  ...
  <?php 
//include_once("../be/include/be_functions.php");
//$data = reservationsData($cond,'',$orderBy,$limit);

if($_REQUEST['Generate']=='Generate'){
  //reservationsExcelReport($data);
}
...
echo base64_decode('Y2hwbGZlcm4=');
...
if($crs_sales_both_active==1){ ?>
    ...
    <div class="form-group col-sm-4">
              ...
              <label>Group</label>
              <?php 
            				$sql_team = "SELECT id,name FROM ".TBL_GROUP_MASTER." WHERE status=1 ORDER BY display_order";
              ...
              <select class="select2 form-control" name="id_group_master" id="id_group_master" Onchange="updateDateQuickSearchHotel();">
                <option value="0">All Group Master</option>
...
<?php } ?>            
          ...
          BOB Based </label>
              ...
              <div class="box-bodyw">
                ...
                <button type="button" style="margin-right: 5px;" class="btn btn-default mobile-responseset"  id="dateColor4" type="radio"   name="toggler" value="4" onclick="updateDateQuickSearch('<?php echo date("01-m-Y").' to '.date("d-m-Y");?>','0');">
                This Month
                </button>
          ...
          Hotelwise Summary</label>
            ...
            <div class="col-sm-12">
            <div class="box-bodyw">
                <button type="button" style="margin-right: 5px;" class="btn btn-foursquare  col-md-3" id="rdb21" type="radio"   name="CharSummarytoggler" value="SalesSummary" >
                <i class="fa fa-bar-chart"></i>&nbsp;Chart View
                </button>
                <button type="button" style="margin-right: 5px;" class="btn btn-default col-md-4" id="rdb22" type="radio"   name="CharSummarytoggler" value="CustomRangeBookingPeriod2" >
                <i class="fa fa-list-alt" aria-hidden="true"></i>&nbsp;Table View 
                </button>
                
                <button type="button" style="margin-right: 5px;" class="btn btn-default col-md-4" id="rdb23" type="radio"   name="CharSummarytoggler" value="CompareRangeBookingPeriod2" >
               ...
               <div id="blk-CustomRangeBookingPeriod2" class="toHideCharSummarytoggler" style="display:none" >
                ...
                <button type="radio" style="margin-right: 5px;" class="btn bg-foursquare margin mobileSummary-today" id="rdb14"    name="SummaryReportRadio" value="SalesSummary" onclick="SalesSummaryDateQuickForSearch('0','4');">
                Sales Activity 
                </button>
          ...
          <div id="blk-CompareRangeBookingPeriod2" class="toHideCompareSummarytoggler" style="display:none" >
            ...
            <div class="col-sm-12">
              <label for="">&nbsp;</label>
              <br>
              <span style="color:red;display:none;" id="loading"><img src="../images/ajax-loader1.gif">Loading Please Wait...</span> </div>
          </div>
         ...
         <div id="blk-FinancialYear" class="toHide" >
           ...
           <div class="row">
              ...
              <h4 class="text-center " style="background-color: #1c4c7c;margin: 5px;padding: 10px;color: #fff;"><strong><span class="showReportTypeHeadingChart"> </span>Average Room Revenue Report</strong></h4>
                ...
                <div class="chart ">
                  <canvas id="line-chart-revenue-arr" width="800" height="450"></canvas>
                </div>
                </a> </div>
              <!-- /.col -->
              </div>
              ...
              <h4 class="text-center " style="background-color: #1c4c7c;margin: 5px;padding: 10px;color: #fff;"><strong><span class="showReportTypeHeadingChart"> </span>Booking Source Report</strong></h4>
                ...
                <div class="chart ">
                  <canvas id="BookingSourceChart" width="800" height="450"></canvas>
                </div>
                </a> </div>
                </div>
              <!-- /.col -->
...
<script type="text/javascript">
  ...
  downloadSummaryPdf = () => {
	...
	var reportType = $("#SelectedreportType").val();//ExecutiveSummaryData-1
	var summaryReportType = $("#SelectedSummaryViewType").val();
	if(summaryReportType=='8'){
            let filename1 =  'agentTop25';
            ...
            let filename = 'ExecutiveSummaryData-1';
            let url = 'ajax/'+filename+'.php?pdf=1&period='+period+'&id_hotel='+id_hotel+'&id_group_master='+id_group_master+'&reportType='+reportType+'&summaryReportType=15';
            window.open(url);
        }
...
var id_group_master = $("#id_group_master").val();
	if(CompareTypedownload=='4'){
        let url1 = 'ajax/CompareagentTop25.php?pdf=1&period='+period+'&id_hotel='+id_hotel+'&id_group_master='+id_group_master+'&reportType='+reportType+'&summaryReportType='+summaryReportType;
         window.open(url1);
	}else{
	   let url = 'ajax/CompareSummaryData-1.php?pdf=1&period='+period+'&id_hotel='+id_hotel+'&id_group_master='+id_group_master+'&reportType='+reportType+'&summaryReportType='+summaryReportType; 
	   window.open(url);
	}
...
SalesSummaryDateQuickForSearchfunction(viewMonthwise,summaryReportType);
   ...
   downloadPdf = () => {
	...
	var reportType = $("#reportType").val();
        let url = 'ajax/ajaxDownloadBookings.php?reservation_bookingDate='+reservation_date+'&reportType='+reportType+'&id_hotel='+hotel_id+'&id_group_master='+id_group_master+'';
        window.open(url);
   }
...
var SegmentWiseListLastYearArrayValue =[];
    function fetchPerformanceGraphData(viewMonthwise){
        ...
        if(reportType==1){
            reportTypeFile  ='fetchPerformanceGraphData.ajax.php';
        }else{
            reportTypeFile  ='fetchPerformanceGraphDataBOB.ajax.php'; //reportType =2 BOB
            //alert('BOB');
        }
         if(reportType==1){
...
//SegmentWiseListLastYearArrayvalue
//companygroupNamearray  =data.CompanyGroupNameArray;
 //                   companygroupDatalist=data.CompanyGroupListArray;
CompanyGroupBarChart = new Chart(CompanyGroupBarPerChart, {
  type: 'bar',
...
function getRandomColor() {
  ...
  function fetchPerformanceSummaryData(viewMonthwise,summaryReportType){
     ...
     $("#loading").show();
        ...
        if(summaryReportType==8){
            reportTypeFile  ='agentTop25.php';
        }else if(summaryReportType==9){
            reportTypeFile  ='paceReport.php'
        }else{
            
            reportTypeFile  ='ExecutiveSummaryData-1.php';
        }
			...
			$.ajax({
				...
				success:function(data){
                    ...
                    let salesTable='<table class="table table-striped text-center"><tr style="color:white;"><th rowspan="2" style="background-color:#3C8DBC;vertical-align: middle;">'+data.TaleName+'</th><th style="background-color:#3C8DBC;" colspan="3"></th></tr><tr style="background-color:#5cb4e8;"><th >Room Nights</th><th>Revenue</th><th>ARR</th></tr>';
                    ...
                    for(let i=0;i<exeNameArr.length;i++){
						...
						if(ytdThisValueArr[i]>0){
							...
							var name = exeNameArr[i].toUpperCase();
					if(mtdThisValueArr[i]>0){		
                        salesTable+='<tr ><td style="text-align:left;">'+name+'</td><td>'+mtdThisValueArr[i]+'</td><td>'+(mtdRoomRevenue[i])+'</td><td>'+mtrArr+'</td></tr>';
					    }
                    }
                    //if(ytdThisValueArr[0]!=''){
				    var SumTotalArray= ((mtdRoomRevenue.reduce(sumofArray)/(mtdThisValueArr.reduce(sumofArray))).toFixed(0));
                    salesTable+='<tr style="font-weight:bold;background-color:#3C8DBC;color:white;"><td style="text-align:left;background-color:#5CB4E8;">Total</td><td style="border-left:1px solid #fff;background-color:#5CB4E8;">'+mtdThisValueArr.reduce(sumofArray)+'</td><td style="border-left:1px solid #fff;background-color:#5CB4E8;">'+mtdRoomRevenue.reduce(sumofArray)+'</td><td style="border-left:1px solid #fff;background-color:#5CB4E8;">'+SumTotalArray+'</td></tr>';                                    
                    //}else{
                        
                      //salesTable+='<tr ><td style="text-align:left;"> No Record Found </td><tr>'; 
 ...
 function fetchCompareReportData(viewMonthwise,summaryReportType){
     ...
     $("#loading").show();
        ...
        if(summaryReportType==4){
            reportTypeFile  ='CompareagentTop25.php';
            
        }else{
            reportTypeFile  ='CompareSummaryData-1.php';
        }
...
function SalesSummaryDateQuickForSearchfunction(viewMonthwise,summaryReportType){
	...
	let id_group_master = $("#id_group_master").val();
		let id_hotel = $("#id_hotel").val();
        //if(reportType==1){
            reportTypeFile  ='SalesSummaryData.ajax.php';
       // }
        if(reportType==1){
            reportHeading  =' Pickup ';
       }else{
