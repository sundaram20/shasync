...
$lastYearPickUpsql =" SELECT `".TBL_ORDERS."`.id_hotel,`".TBL_ORDERS."`.id_company,`mst_team`.id_group,`fs_users`.myownteam_id as MyOwnteam,`".TBL_COMPANY."`.id_default_group,`".TBL_USERS."`.id AS id_executive,`".TBL_AREAS."`.id AS id_area,`".TBL_ORDERS."`.booking_status,`".TBL_USERS."`.ids_team,
       ...
       ".$cond." ".$allUser."AND `".TBL_GROUP."`.name='Travel Agent'  GROUP BY  `".TBL_ORDERS."`.id_company" ;
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_GROUP."`.name='Travel Agent' AND ( ( `fs_order_detail` .dated BETWEEN '".$FromLastYearDate."' And '".$ToLastYearDate."')) $condhotelAccess GROUP BY `".TBL_ORDERS."`.id_company ";
...
if(($_REQUEST['pdf']==1 && $_REQUEST['summaryReportType'] == '3') || ($_REQUEST['summaryReportType'] == '3' )){   //Hotel Wise  Summary PDF PICKUP Report
    ...
    $reportArray['Top 50 Agents'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearroomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
    $reportArray['Top 50 Agents'][strtolower($companyname==''?$emptytext7:$companyname)]['lastYearconfimed_revenue']=$rowlastYearList->confimed_revenue==''?0:round($rowlastYearList->confimed_revenue);
...
where `fs_orders`.`id_shop` = '".addslashes($_SESSION['shop'])."' AND `".TBL_GROUP."`.name='Travel Agent' AND ( ( `fs_order_detail` .dated BETWEEN '".date('Y-m-d',strtotime($from_book))."' And '".date('Y-m-d',strtotime($to_book))."')) $condhotelAccess GROUP BY `".TBL_ORDERS."`.id_company ";
	...
	while($rowList = mysqli_fetch_object($resultList)){
	  $companyname= selectColumn(TBL_COMPANY,'name'," WHERE `id_company` = '".$rowList->id_company."'");
	    ...
	    $reportArray['Top 50 Agents'][strtolower($companyname==''?$emptytext7:$companyname)]['roomnights']=$newConfirmednewTentative==''?'0':$newConfirmednewTentative;
	    $reportArray['Top 50 Agents'][strtolower($companyname==''?$emptytext7:$companyname)]['confimed_revenue']=$rowList->confimed_revenue==''?0:round($rowList->confimed_revenue);
...
foreach($reportArray as $maintitle=>$mainDatalist){
	...
	$content .='<tr><th colspan="8" style="vertical-align:central;text-align:center;color:#fff;background-color:#4f6228; font-size:16px !important"><b>Compare '.$maintitle.'  Breakup For Period '.$reportPeriod.'</b></th></tr>';
    $content .='<tr>
...
array_multisort($roomnights, SORT_DESC, $confimed_revenue, SORT_ASC, $mainDatalist);
	 $k=1;
...
if($subDataList['roomnights']>0 || $subDataList['lastYearroomnights']>0){
    ...
    <td style="text-align:left;">'.ucwords($teamGroup).' </td>
    <td style="border-left:1px solid #fff;text-align:center;">'.$TotalRoomNightlastYear.'</td>
    <td style="border-left:1px solid #fff;text-align:center;">'.$subDataList['roomnights'].'</td>
    <td style="border-left:1px solid #fff;text-align:center;'.$SumTotalGolyRoomNightsColor1.'">'.$SumTotalGolyRoomNights.'</td>
    
    <td style="border-left:1px solid #fff;text-align:center;">'.round($TotalConfimedRevenuelastYear/100000,2).'</td>
    <td style="border-left:1px solid #fff;text-align:center;">'.round($TotalConfimedRevenue/100000,2).'</td>
    <td style="border-left:1px solid #fff;text-align:center;'.$SumTotalGolyConfimedRevenueColor1.'">'.$SumTotalGolyConfimedRevenue.'</td>
    
    
    <td style="border-left:1px solid #fff;text-align:center;">'.$SumTotalArray.'</td>
