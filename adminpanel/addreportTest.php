...
<?php } 
 ...
 ?>
                  ...
                  <div  class="input-group enquirypage" id="showcompanyby"> 
                      <select  class="form-control select2 itemName"  onChange="getExecutiveName(this.value,''); areaExecutive();"  name="id_company" id="id_company" data-parsley-errors-container="#idcompanyError"  data-parsley-required>   
                      <?php
                        if($_REQUEST['eId']!=''){
                          $CompanyName	=	 selectColumn(TBL_COMPANY,'CONCAT(name," - ",city)'," WHERE `id_company` = '".$row->id_company."'");
                          echo '<option  '.$selected.' value="'.$row->id_company.'">'.$CompanyName.'</option>';
