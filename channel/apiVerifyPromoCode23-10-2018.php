...
if($NumberOfRows >0){
		...
		if($Fetch_Record['promo_code_status'] =='1' || $Fetch_Record['promo_code_status'] =='2'){  //PomoCode Issues or reissued
	
			
				//Email Code
				
					$updateEvocher = executeSql("UPDATE  `".TBL_PROMO_CODE_DETAILS."`  SET 
								`promo_code_status`='3'	,`generate_email`='".$filter_id['verifyemail']."',`generate_date`='".date('Y-m-d')."'							
								where 
...
.table td, .table > tbody > tr > td, .table > tbody > tr > th, .table > tfoot > tr > td, .table > tfoot > tr > th, .table > thead > tr > td, .table > thead > tr > th {
    ...
    padding: 7px !important;
		...
		<td colspan="3">  Greetings !!!<br><br>       
							...
							We are delighted to provide Evoucher Code mentioned below:<br><br>	  </td>';

		$content .=	'<tr align="middle" style="background-color:#4f81bc;color:#fff;font-color:#fff;">
						<td>
						Evoucher Code : '.addslashes(encryptor(decrypt,$filter_id['promocode'])).' 
						</td>
						...
						// Additional headers
						$headers[] = 'To:'.$Emails;
						$headers[] = 'From:test@welcomheritagehotels.in';
						//$headers[] = 'Cc:'.$Emails;
						//$headers[] = 'Bcc: ';
					...
					$Meassage	=	"Evoucher Code Generated sucessfully";
	
	
		}elseif($Fetch_Record['promo_code_status'] =='3'){  //PromoCode Is Generated
			$Meassage	=	"Evoucher code already Generated";						
