...
$CheckTaxISArray	=	array_key_exists('0',$xmlarray['HotelReservations']['HotelReservation']['ResGlobalInfo']['Total']['Taxes']);
		
		
		if($CheckTaxISArray	==1){
		
		$totaltaxamount 		        = $xmlarray['HotelReservations']['HotelReservation']['ResGlobalInfo']['Total']['Taxes'][0]['@attributes']['Amount'];		
		}else{
		
		
		$totaltaxamount 		        = $xmlarray['HotelReservations']['HotelReservation']['ResGlobalInfo']['Total']['Taxes']['Tax']['@attributes']['Amount'];		
...
$valid=	'1.'.$rowTax->tax_room;*/
		...
		$total_price_orderTable = 	($subtotal+$totaltaxamount);
		
		$Base_AmountBeforeTax	=	($subtotal/$no_of_days);
		
		$amount_received = 0;
		
		$balance = ($subtotal+$totaltaxamount);	
...
if($ChechGuestArray	==1){
		...
		$Customer_GivenName =$ChangeGuestArray[0]['Profiles']['ProfileInfo']['Profile']['Customer']['PersonName']['GivenName'];
			$Customer_Surname1 		= 	$ChangeGuestArray[0]['Profiles']['ProfileInfo']['Profile']['Customer']['PersonName']['Surname'];
		
		if (!empty($Customer_Surname1)) {
     // list is empty.
...
//GuestCounts-------------------------------------------------------
$GuestCountArray	=	array_key_exists('0',$RoomStayCount[$stay]['GuestCounts']['GuestCount']);

	if($GuestCountArray	==1){
		
				$GuestAgeQualifyingCode 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount'][0]['@attributes']['AgeQualifyingCode'];		
				$GuestCountsArray 		 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount'][0]['@attributes']['Count']; 
				...
				$GuestAgeQualifyingCode1 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount'][1]['@attributes']['AgeQualifyingCode'];		
				$GuestCountsArray 		 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount'][1]['@attributes']['Count']; 		
				if($GuestAgeQualifyingCode1 ==8){
					$GuestCountsArray1 		= $RoomStayCount[$stay]['GuestCounts']['GuestCount'][1]['@attributes']['Count']; 		
					$total_child   			=	$GuestCountsArray1;			
					$child 					= $GuestCountsArray1;
				}
		}else{
				$GuestCountArrayStep2	=	array_key_exists('0',$RoomStayCount[$stay]['GuestCounts']);
				
				if($GuestCountArrayStep2	==1){
					
					
							$GuestAgeQualifyingCode 	= $RoomStayCount[$stay]['GuestCounts'][0]['@attributes']['AgeQualifyingCode'];		
							$GuestCountsArray 		 	= $RoomStayCount[$stay]['GuestCounts'][0]['@attributes']['Count']; 
							...
							$GuestAgeQualifyingCode1 	= $RoomStayCount[$stay]['GuestCounts'][1]['@attributes']['AgeQualifyingCode'];		
							$GuestCountsArray 		 	= $RoomStayCount[$stay]['GuestCounts'][1]['@attributes']['Count']; 
							
							if($GuestAgeQualifyingCode1 ==8){
							$GuestCountsArray1 		 	= $RoomStayCount[$stay]['GuestCounts'][1]['@attributes']['Count']; 
							$total_child   =	$GuestCountsArray1;			
							$child 			= $GuestCountsArray1;
							}
				}else{
						...
						$GuestAgeQualifyingCode1 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount']['@attributes']['AgeQualifyingCode'];		
						$GuestCountsArray 		 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount']['@attributes']['Count']; 
						
						if($GuestAgeQualifyingCode1 ==8){
						$GuestCountsArray1 		 	= $RoomStayCount[$stay]['GuestCounts']['GuestCount']['@attributes']['Count']; 
						$total_child   =	$GuestCountsArray1;			
						$child 			= $GuestCountsArray1;
						}
...
/*if($total_child!=0){
$total_child	=	$total_child*$TotalRoomS['NumberOfUnits'];
}
$total_adults	=	$total_adults*$TotalRoomS['NumberOfUnits'];
*/	
...
$Base_CurrencyCode 		=  $RoomStayCount[$stay]['RoomRates']['RoomRate']['Rates']['Rate']['Base']['@attributes']['CurrencyCode'];
	...
	if($kkk1	==1){
		...
		$CheckPlanISArray	=	array_key_exists('0',$RoomStayCount[$stay]['RatePlans']);
		
		
		if($CheckPlanISArray	==1){
		
		
		$RatePlansMappingID = $RoomStayCount[$stay]['RatePlans'][0]['@attributes']['RatePlanCode'];		
		}else{
		...
		$SQLRateId 			= mysql_query("SELECT * FROM fs_rate_mapping WHERE booking_engine_id ='".addslashes($RatePlansMappingID)."' and channel_id='1'");
		$FetchArrayRateId 	= mysql_fetch_object($SQLRateId);
...
$AmendmentSQL = "INSERT INTO  `".TBL_AMENDMENT_COUNT."` SET 				 
								...
								`amount_received`	='".$amount_received."',
								`balance`			='".$balance."',
								`total_products`	='".$TotalRoom_NumberOfUnits."',				
								`total_adults`		='".$total_adults."',
								`total_child`		='".$total_child."',
								`payment_date`		='".$payment_date."',
								`checkin`			='".$checkin."',
